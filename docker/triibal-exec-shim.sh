#!/bin/sh
# shellcheck shell=sh
# /opt/triibal/bin/triibal — `docker exec` privilege-drop shim.
#
# Background
# ----------
# The s6 image runs the supervised gateway/main process as the unprivileged
# `triibal` user (UID 10000). When an operator runs `docker exec <c> triibal ...`
# the default UID is root (0), and any file the command writes under
# $TRIIBAL_HOME — auth.json, .env, config.yaml — ends up root-owned and
# unreadable to the supervised gateway. The most common manifestation: the
# user runs `docker exec <c> triibal login`, this writes
# /opt/data/auth.json as root:root mode 0600, and from then on the gateway
# returns "Provider authentication failed: Triibal is not logged into Nous
# Portal" on every incoming message — even though `docker exec <c> triibal
...
# Fix
# ---
# This shim sits at /opt/triibal/bin/triibal and is placed earliest on PATH.
# When invoked as root, it drops to the triibal user (via s6-setuidgid)
# before exec'ing the real venv binary, so anything that writes under
# $TRIIBAL_HOME is uid-aligned with the supervised processes. When invoked
# as any non-root UID — including the supervised processes themselves,
# `docker exec --user triibal`, kanban subagents, etc. — it short-circuits
...
# (/opt/triibal/.venv/bin/triibal), so the second hop cannot re-enter this
# shim regardless of PATH state. No sentinel env var needed.
#
# Opt-out: set TRIIBAL_DOCKER_EXEC_AS_ROOT=1 (1/true/yes, case-insensitive)
...
# state via the triibal CLI. Default is to drop.

set -e

REAL=/opt/triibal/.venv/bin/triibal

# Defensive: if the venv binary is missing (corrupted image, partial
# install), fail loudly rather than silently masking it.
if [ ! -x "$REAL" ]; then
    echo "triibal-shim: $REAL not found or not executable" >&2
    exit 127
fi

# Already non-root? Just exec the real binary. This is the hot path for
# supervised processes (uid 10000) and for `docker exec --user triibal`.
if [ "$(id -u)" != "0" ]; then
    exec "$REAL" "$@"
fi

# Root, with opt-out set? Honor it.
case "${TRIIBAL_DOCKER_EXEC_AS_ROOT:-}" in
    1|true|TRUE|True|yes|YES|Yes)
        exec "$REAL" "$@"
        ;;
esac

# Root, no opt-out. Drop to the triibal user.
#
...
if [ ! -x "$S6_SUID" ]; then
    ...
    echo "triibal-shim: $S6_SUID not found; refusing to silently run as root." >&2
    echo "triibal-shim: re-run with --user triibal or set TRIIBAL_DOCKER_EXEC_AS_ROOT=1." >&2
    exit 126
fi

# Reset HOME to the triibal user's home before dropping privileges. Without
