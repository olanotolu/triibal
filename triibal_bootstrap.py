"""Windows UTF-8 bootstrap for Triibal entry points.
...
should be imported at the very top of every Triibal entry point
(``triibal``, ``triibal-agent``, ``triibal-acp``, ``python -m gateway.run``,
...
# Apply on import — entry points just need ``import triibal_bootstrap``
# (or ``from triibal_bootstrap import apply_windows_utf8_bootstrap``) at
