...
Maps each role archetype to the Triibal skills it should `always_load` and the
toolsets it needs. Only references skills that ship in the public triibal-agent
...
## Triibal skills relevant to video production

### Visual / rendering skills (`triibal-agent/skills/creative/`)
...
### Design / pre-production skills (`triibal-agent/skills/creative/`)
...
### Audio / media skills (`triibal-agent/skills/creative/` + `skills/media/`)
...
### Kanban infrastructure (`triibal-agent/skills/devops/`)
...
These are **not** Triibal skills but external CLIs / APIs that profiles invoke.
...
## Built-in Triibal tools for media review

These are native Triibal tools — not invoked via terminal but through their own
...
key is present in `~/.triibal/.env` (or macOS Keychain) before firing the kanban.
...
| Anthropic | `ANTHROPIC_API_KEY` | every Triibal profile (Claude) |

If a key is missing, prompt the user to add it. Storage methods, in order of
preference: macOS Keychain → `~/.triibal/.env` → environment variable.
...
When a new Triibal-public video skill ships:
