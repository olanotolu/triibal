"""
Triibal MCP Server — expose messaging conversations as MCP tools.
...
Plus: channels_list (Triibal-specific extra)

Usage:
    triibal mcp serve
    triibal mcp serve --verbose
...
logger = logging.getLogger("triibal.mcp_serve")
...
def _get_sessions_dir() -> Path:
    """Return the sessions directory using TRIIBAL_HOME."""
    try:
        from triibal_constants import get_triibal_home
        return get_triibal_home() / "sessions"
    except ImportError:
        return Path(os.environ.get("TRIIBAL_HOME", Path.home() / ".triibal")) / "sessions"
...
def _load_channel_directory() -> dict:
    ...
    try:
        from triibal_constants import get_triibal_home
        directory_file = get_triibal_home() / "channel_directory.json"
    except ImportError:
        directory_file = Path(
            os.environ.get("TRIIBAL_HOME", Path.home() / ".triibal")
...
class EventBridge:
    ...
    This is the Triibal equivalent of OpenClaw's WebSocket gateway bridge.
    ...
    def _poll_once(self, db):
        ...
        try:
            from triibal_constants import get_triibal_home
            db_file = get_triibal_home() / "state.db"
        except ImportError:
            db_file = Path(os.environ.get("TRIIBAL_HOME", Path.home() / ".triibal")) / "state.db"
...
def create_mcp_server(event_bridge: Optional[EventBridge] = None) -> "FastMCP":
    """Create and return the Triibal MCP server with all tools registered."""
    ...
    mcp = FastMCP(
        "triibal",
        instructions=(
            "Triibal Agent messaging bridge. Use these tools to interact with "
...
def run_mcp_server(verbose: bool = False) -> None:
    """Start the Triibal MCP server on stdio."""
    if not _MCP_SERVER_AVAILABLE:
        print(
