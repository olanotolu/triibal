...
curl -s -X POST \
  ...
  -d "{
    \"commit_id\": \"$HEAD_SHA\",
    \"event\": \"COMMENT\",
    \"body\": \"Code review from Triibal Agent\",
    \"comments\": [
...
```bash
source "${TRIIBAL_HOME:-$HOME/.triibal}/skills/github/github-auth/scripts/gh-env.sh"
...
```bash
# 若无问题——批准
gh pr review $PR_NUMBER --approve --body "Reviewed by Triibal Agent. Code looks clean — good test coverage, no security concerns."
