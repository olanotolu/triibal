...
def rewrite_relative_links(body: str, meta: dict[str, Any]) -> str:
    ...
    base = f"https://github.com/Triibal/triibal/blob/main/{source_dir}/{meta['rel_path']}"
...
def render_skill_page(
    ...
    triibal_meta = (fm.get("metadata") or {}).get("triibal") or {}
    tags = triibal_meta.get("tags") or []
    related = triibal_meta.get("related_skills") or []
    ...
    if meta["source_kind"] == "bundled":
        ...
        info_rows.append(
            (
                "Source",
                "Optional — install with `triibal skills install official/"
                + meta["category"]
                + "/"
                + meta["slug"]
                + "`",
...
def build_catalog_md_bundled(entries: list[tuple[dict[str, Any], dict[str, Any]]]) -> str:
    ...
    lines = [
        ...
        'description: "Catalog of bundled skills that ship with Triibal Agent"',
        "---",
        "",
        "# Bundled Skills Catalog",
        "",
        "Triibal ships with a large built-in skill library copied into `~/.triibal/skills/` on install. Each skill below links to a dedicated page with its full definition, setup, and usage.",
        "",
        "Triibal also syncs bundled skills on `triibal update`, but the sync manifest respects local deletions and user edits. If a skill listed here is missing from your profile's `~/.triibal/skills/` tree, it is still shipped with Triibal; restore it with `triibal skills reset <name> --restore`.",
        "",
