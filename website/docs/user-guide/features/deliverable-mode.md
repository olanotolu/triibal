...
When Triibal Agent runs inside a messaging gateway (Slack, Discord, Telegram,
...
persona in `~/.triibal/SOUL.md`, or as a named preset under
`agent.personalities` in `~/.triibal/config.yaml` (switchable per session
via `/personality`).
...
If you use Triibal' kanban multi-agent workflow, workers can attach
...
Install MCP servers via `~/.triibal/config.yaml` under the `mcp_servers`
...
back into the thread as a native attachment. Triibal Agent's deliverable
