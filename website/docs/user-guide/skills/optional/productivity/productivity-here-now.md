...
| | |
|---|---|
| Source | Optional — install with `triibal skills install official/productivity/here-now` |
...
- Skill helper paths:
  - `${TRIIBAL_SKILL_DIR}/scripts/publish.sh` for publishing sites
  - `${TRIIBAL_SKILL_DIR}/scripts/drive.sh` for private Drive storage
...
| `--client {name}`      | Agent name for attribution (e.g. `triibal`)    |
