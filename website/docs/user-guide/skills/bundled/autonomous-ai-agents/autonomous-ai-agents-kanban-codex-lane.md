...
description: "Use when a Triibal Kanban worker wants to run Codex CLI as an isolated implementation lane while Triibal keeps ownership of task lifecycle, reconciliation, tes..."
---
...
Use when a Triibal Kanban worker wants to run Codex CLI as an isolated implementation lane while Triibal keeps ownership of task lifecycle, reconciliation, testing, and handoff.
...
| Related skills | [`kanban-worker`](/docs/user-guide/skills/bundled/devops/devops-kanban-worker), [`codex`](/docs/user-guide/skills/bundled/autonomous-ai-agents/autonomous-ai-agents-codex), [`triibal-agent`](/docs/user-guide/skills/bundled/autonomous-ai-agents/autonomous-ai-agents-triibal-agent) |
...
- A bounded diff can be evaluated by Triibal in one run.
- The repo can be copied or checked out in an isolated git worktree/branch.
- Triibal can run the relevant tests itself after Codex exits.
...
1. Triibal owns the Kanban lifecycle. Codex must never call `kanban_complete`, `kanban_block`, `kanban_create`, gateway messaging, or any Triibal board CLI as a substitute for the worker.
2. Triibal owns final acceptance. Treat Codex commits/diffs as untrusted patches until reviewed and verified.
3. Triibal owns test execution. Codex may run tests, but those runs are advisory; repeat required verification from Triibal with the repo's canonical wrapper.
4. Triibal owns safety. If Codex changes safety boundaries, risk gates, live trading behavior, or secrets handling, reject the lane even if tests pass.
5. Triibal owns cleanup. Kill stuck Codex processes and remove temporary worktrees when they are no longer needed.
...
```bash
TASK_ID="${TRIIBAL_KANBAN_TASK:-t_manual}"
REPO="/path/to/repo"
...
Run these before spawning Codex. Missing Codex is a normal reason to skip the lane, not a task blocker if Triibal can do the task directly.
...
Triibal owns the Kanban lifecycle; do not call Triibal kanban tools or messaging.
...
Do not use `--yolo` for prediction-market-bot or safety-sensitive repos. Prefer `--full-auto` inside the isolated worktree, then rely on Triibal reconciliation.
...
- Explicit statement: Triibal owns Kanban lifecycle; Codex is an input lane only.
...
- Verification commands Codex may run and commands Triibal will run afterward.
...
Triibal must perform this checklist before accepting any Codex lane result:

- [ ] `git -C <WORKTREE> status --short --branch` shows only expected files.
- [ ] `git -C <WORKTREE> diff --stat` and `git diff` were reviewed by Triibal.
...
- [ ] Triibal ran the canonical tests itself, using `scripts/run_tests.sh` for Triibal Agent or the repo's documented wrapper for other repos.
- [ ] Any Codex-run tests are listed separately from Triibal-run tests.
- [ ] Accepted commits/diffs were applied to the Triibal-owned workspace/branch.
...
```json
{
  "codex_lane": {
    ...
    "tests_run": [
      {"command": "scripts/run_tests.sh tests/tools/test_x.py", "exit_code": 0, "owner": "triibal"},
...
```json
{
  "codex_lane": {
    ...
    "result": "rejected",
    "accepted_commits": [],
    "rejected_reason": "Direct Triibal edit was smaller and safer than spawning Codex.",
    "tests_run": [],
    "artifacts": []
  }
}
...
1. Treating Codex self-report as verification. Always inspect the diff and rerun tests from Triibal.
2. Running Codex in the user's dirty main checkout. Always isolate in a worktree/branch.
3. Letting Codex own Kanban. Codex may summarize progress, but Triibal writes board state.
...
- [ ] Triibal reviewed `git diff` and safety-sensitive files.
- [ ] Triibal ran canonical tests independently.
