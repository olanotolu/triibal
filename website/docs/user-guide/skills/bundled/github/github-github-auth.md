...
- Give it a name like "triibal-agent"
- Select scopes:
...
- Give it a title like "triibal-agent-&lt;machine-name>"
...
elif [ -f ~/.triibal/.env ] && grep -q "^GITHUB_TOKEN=" ~/.triibal/.env; then
  export GITHUB_TOKEN=$(grep "^GITHUB_TOKEN=" ~/.triibal/.env | head -1 | cut -d= -f2 | tr -d '\n\r')
  echo "AUTH_METHOD=curl"
