---
sidebar_position: 6
title: "Use MCP with Triibal"
description: "A practical guide to connecting MCP servers to Triibal Agent, filtering their tools, and using them safely in real workflows"
---

# Use MCP with Triibal

This guide shows how to actually use MCP with Triibal Agent in day-to-day workflows.
...
Use MCP when:
- a tool already exists in MCP form and you do not want to build a native Triibal tool
- you want Triibal to operate against a local or remote system through a clean RPC layer
- you want fine-grained per-server exposure control
- you want to connect Triibal to internal APIs, databases, or company systems without modifying Triibal core

Do not use MCP when:
- a built-in Triibal tool already solves the job well
...
- Triibal remains the agent
- MCP servers contribute tools
- Triibal discovers those tools at startup or reload time
...
If you installed Triibal with the standard install script, MCP support is already included (the installer runs `uv pip install -e ".[all]"`).
...
- Triibal banner/status should show MCP integration when configured
- ask Triibal what tools it has available
...
## WSL2: bridge Triibal in WSL to Windows Chrome

This is the practical setup when:

- Triibal runs inside WSL2
...
In this setup, Triibal does **not** connect to Chrome directly. Instead:

- Triibal runs in WSL
- Triibal starts a local stdio MCP server
...
```text
Triibal (WSL) -> MCP stdio bridge -> Windows Chrome
```

### Why this mode is useful

- you keep your real Windows browser profile, cookies, and logins
- Triibal stays in its supported Unix environment (WSL2)
- browser control is exposed as MCP tools instead of relying on Triibal core browser transport
...
```bash
triibal mcp add chrome-devtools-win --command cmd.exe --args /c npx -y chrome-devtools-mcp@latest --autoConnect --no-usage-statistics
```

After saving the server:

```bash
triibal mcp test chrome-devtools-win
```

Then start a fresh Triibal session or run:
...
Once loaded, Triibal can use the MCP-prefixed browser tools directly. For example:
...
If Triibal runs in WSL and Chrome runs on Windows, `/browser connect` may fail even though Chrome is open and debuggable.
...
- Start Triibal from a Windows-mounted path like `/mnt/c/Users/<you>` or `/mnt/c/workspace/...` when using Windows stdio executables through MCP.
- If you start Triibal from `/root` or `/home/...`, Windows may emit a `UNC` current-directory warning before the MCP server starts.
...
There are two categories of MCP-exposed functionality in Triibal:
...
2. Triibal-added utility wrappers
- filtered with:
  - `tools.resources`
  - `tools.prompts`
...
So Triibal will not pretend a server has resources/prompts if it does not.

## Common patterns

### Pattern 1: local project assistant

Use MCP for a repo-local filesystem or git server when you want Triibal to reason over a bounded workspace.
...
Now Triibal can combine them:

```text
Inspect the local project files, then create a GitHub issue summarizing the bug you find.
```

That is where MCP gets powerful: multi-system workflows without changing Triibal core.
...
Because Triibal now respects your per-server policy and capability-aware registration. That is expected, and usually desirable.
