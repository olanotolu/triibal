---
sidebar_position: 7
title: "Use SOUL.md with Triibal"
description: "How to use SOUL.md to shape Triibal Agent's default voice, what belongs there, and how it differs from AGENTS.md and /personality"
---

# Use SOUL.md with Triibal

`SOUL.md` is the **primary identity** for your Triibal instance. It's the first thing in the system prompt — it defines who the agent is, how it speaks, and what it avoids.

If you want Triibal to feel like the same assistant every time you talk to it — or if you want to replace the Triibal persona entirely with your own — this is the file to use.
...
- personality
- communication style
- how direct or warm Triibal should be
- what Triibal should avoid stylistically
- how Triibal should relate to uncertainty, disagreement, and ambiguity

In short:
- `SOUL.md` is about who Triibal is and how Triibal speaks
...
Triibal now uses only the global SOUL file for the current instance:

```text
~/.triibal/SOUL.md
```

If you run Triibal with a custom home directory, it becomes:
...
Triibal automatically seeds a starter `SOUL.md` for you if one does not already exist.

That means most users now begin with a real file they can read and edit immediately.

Important:
- if you already have a `SOUL.md`, Triibal does not overwrite it
- if the file exists but is empty, Triibal adds nothing from it to the prompt

## How Triibal uses it

When Triibal starts a session, it reads `SOUL.md` from `TRIIBAL_HOME`, scans it for prompt-injection patterns, truncates it if needed, and uses it as the **agent identity** — slot #1 in the system prompt. This means SOUL.md completely replaces the built-in default identity text.
...
That alone can noticeably change how Triibal feels.
...
Triibal already tries to be helpful and clear. `SOUL.md` should add real personality and style, not restate obvious defaults.
...
# Style
How Triibal should sound.

# Avoid
What Triibal should not do.

# Defaults
How Triibal should behave when ambiguity appears.
```
...
Then restart Triibal or start a new session.
...
### I edited SOUL.md but Triibal still sounds the same

Check:
- you edited `~/.triibal/SOUL.md` or `$TRIIBAL_HOME/SOUL.md`
...
### Triibal is ignoring parts of my SOUL.md
