...
This tutorial walks you through setting up a Telegram bot powered by Triibal Agent that multiple team members can use. By the end, your team will have a shared AI assistant they can message for help with code, research, system administration, and anything else — secured with per-user authorization.
...
- **Triibal Agent installed** on a server or VPS (not your laptop — the bot needs to stay running). Follow the [installation guide](/getting-started/installation) if you haven't yet.
- **A Telegram account** for yourself (the bot owner)
- **An LLM provider configured** — at minimum, an API key for OpenAI, Anthropic, or another supported provider in `~/.triibal/.env`

:::tip
A $5/month VPS is plenty for running the gateway. Triibal itself is lightweight — the LLM API calls are what cost money, and those happen remotely.
:::
...
2. **Send `/newbot`** — BotFather will ask you two things:
   - **Display name** — what users see (e.g., `Team Triibal Assistant`)
   - **Username** — must end in `bot` (e.g., `myteam_triibal_bot`)
...
4. **Set a description** (optional but recommended):
   ...
   ```
   Team AI assistant powered by Triibal Agent. DM me for help with code, research, debugging, and more.
   ```
...
Add these lines to `~/.triibal/.env`:
...
```
[Gateway] Starting Triibal Gateway...
...
# View live logs
journalctl --user -u triibal-gateway -f
...
triibal gateway start
triibal gateway stop
tail -f ~/.triibal/logs/gateway.log
```

:::tip macOS PATH
The launchd plist captures your shell PATH at install time so gateway subprocesses can find tools like Node.js and ffmpeg. If you install new tools later, re-run `triibal gateway install` to update the plist.
:::
...
```bash
triibal gateway stop && triibal gateway start
```
...
triibal pairing revoke telegram 987654321
...
**Option 2:** Set it manually in `~/.triibal/.env`:
...
Control how much detail the bot shows when using tools. In `~/.triibal/config.yaml`:
...
Customize how the bot communicates by editing `~/.triibal/SOUL.md`:

For a full guide, see [Use SOUL.md with Triibal](/guides/use-soul-with-triibal).
...
```bash
# From the CLI
triibal cron list          # View all scheduled jobs
triibal cron status        # Check if scheduler is running
...
| What | Location |
|------|----------|
| Gateway logs | `journalctl --user -u triibal-gateway` (Linux) or `~/.triibal/logs/gateway.log` (macOS) |
| Cron job output | `~/.triibal/cron/output/{job_id}/{timestamp}.md` |
| Cron job definitions | `~/.triibal/cron/jobs.json` |
| Pairing data | `~/.triibal/pairing/` |
| Session history | `~/.triibal/sessions/` |
