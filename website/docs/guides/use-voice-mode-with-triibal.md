---
sidebar_position: 8
title: "Use Voice Mode with Triibal"
description: "A practical guide to setting up and using Triibal voice mode across CLI, Telegram, Discord, and Discord voice channels"
---

# Use Voice Mode with Triibal
...
- you want Triibal sitting in a Discord voice channel for live conversation
...
There are really three different voice experiences in Triibal.
...
## Step 1: make sure normal Triibal works first
...
Triibal supports both local and cloud speech stacks.
...
### If you use `triibal setup`

If you choose NeuTTS in the setup wizard, Triibal checks whether `neutts` is already installed. If it is missing, the wizard tells you NeuTTS needs the Python package `neutts` and the system package `espeak-ng`, offers to install them for you, installs `espeak-ng` with your platform package manager, and then runs:
...
4. Triibal transcribes and responds
...
- asking Triibal to structure your thoughts in real time

#### Accessibility / low-typing sessions

If typing is inconvenient, voice mode is one of the fastest ways to stay in the full Triibal loop.

## Tuning CLI behavior

### Silence threshold

If Triibal starts/stops too aggressively, tune:
...
Triibal stays a normal chat bot, but can speak replies.
...
- you want Triibal to function like a portable research or ops assistant
...
Triibal joins a Discord VC, listens to user speech, transcribes it, runs the normal agent pipeline, and speaks replies back into the channel.
...
- Triibal detects speech boundaries
- transcripts are posted in the associated text channel
- Triibal responds in text and audio
...
1. get text Triibal working
2. install `triibal-agent[voice]`
