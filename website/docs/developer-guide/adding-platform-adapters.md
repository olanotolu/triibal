...
This guide covers adding a new messaging platform to the Triibal gateway. A platform adapter connects Triibal to an external messaging service (Telegram, Discord, WeCom, etc.) so users can interact with the agent through that service.

:::tip
There are two ways to add a platform:
- **Plugin** (recommended for community/third-party): Drop a plugin directory into `~/.triibal/plugins/` — zero core code changes needed. See [Plugin Path](#plugin-path-recommended) below.
...
The plugin system lets you add a platform adapter without modifying any core Triibal code. Your plugin is a directory with two files:

```
~/.triibal/plugins/my-platform/
...
Plugin metadata. The `requires_env` and `optional_env` blocks auto-populate `triibal config` UI entries (see [Surfacing Env Vars](#surfacing-env-vars-in-triibal-config) below).
...
| `triibal config` UI entries | `requires_env` / `optional_env` in `plugin.yaml` auto-populate |
...
| `triibal status` | Shows plugin platforms with `(plugin)` tag |
| `triibal gateway setup` | Plugin platforms appear in setup menu |
| `triibal tools` / `triibal skills` | Plugin platforms in per-platform config |
...
Most users set up a platform by dropping env vars into `~/.triibal/.env` rather than editing `config.yaml`. The `env_enablement_fn` hook lets your plugin pick those env vars up **before** the adapter is constructed, so `triibal gateway status`, `get_connected_platforms()`, and cron delivery see the correct state without instantiating the platform SDK.
...
`cron_deliver_env_var` makes your platform a recognized `deliver=` target. To make the actual send succeed when the cron job runs in a separate process from the gateway (i.e., `triibal cron run` separate from `triibal gateway`), register a `standalone_sender_fn`:
...
## Surfacing Env Vars in `triibal config`

`triibal_cli/config.py` scans `plugins/platforms/*/plugin.yaml` at import time and auto-populates `OPTIONAL_ENV_VARS` from `requires_env` and (optional) `optional_env` blocks. Use the rich-dict form to contribute proper descriptions, prompts, password flags, and URLs — the CLI setup UI picks them up for free.
...
version: 1.0.0
description: >
  My Platform gateway adapter for Triibal Agent.
author: Your Name
requires_env:
  ...
  - name: MY_PLATFORM_CHANNEL
    description: "Channel to join (e.g. #triibal)"
    prompt: "Channel"
    password: false
optional_env:
  - name: MY_PLATFORM_HOME_CHANNEL
...
:::note
This checklist is for adding a platform directly to the Triibal core codebase — typically done by core contributors for officially supported platforms. Community/third-party platforms should use the [Plugin Path](#plugin-path-recommended) above.
:::
...
1. **`triibal_cli/config.py`** — Add all `NEWPLAT_*` vars to `_EXTRA_ENV_KEYS`
2. **`triibal_cli/gateway.py`** — Add entry to `_PLATFORMS` list with key, label, emoji, token_var, setup_instructions, and vars
3. **`triibal_cli/platforms.py`** — Add `PlatformInfo` entry with label and default_toolset (used by `skills_config` and `tools_config` TUIs)
4. **`triibal_cli/setup.py`** — Add `_setup_newplat()` function (can delegate to `gateway.py`) and add tuple to the messaging platforms list
5. **`triibal_cli/status.py`** — Add platform detection entry: `"NewPlat": ("NEWPLAT_TOKEN", "NEWPLAT_HOME_CHANNEL")`
6. **`triibal_cli/dump.py`** — Add `"newplat": "NEWPLAT_TOKEN"` to platform detection dict
...
1. **`toolsets.py`** — Add `"triibal-newplat"` toolset definition with `_TRIIBAL_CORE_TOOLS`
2. **`toolsets.py`** — Add `"triibal-newplat"` to the `"triibal-gateway"` includes list
...
| `website/docs/reference/toolsets-reference.md` | triibal-newplat toolset |
