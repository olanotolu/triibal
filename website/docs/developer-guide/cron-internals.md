...
description: "How Triibal stores, schedules, edits, pauses, skill-loads, and delivers cron jobs"
---
...
| `triibal_cli/cron.py` | CLI `triibal cron` subcommands |
...
Jobs are stored in `~/.triibal/cron/jobs.json` with atomic write semantics (write to temp file, then rename). Each job record contains:
...
In CLI mode, cron jobs only fire when `triibal cron` commands are run or during active CLI sessions.
...
```python
# ~/.triibal/scripts/check_competitors.py
...
2. **Environment variable** — `TRIIBAL_CRON_SCRIPT_TIMEOUT`
...
| Local file | `local` | Save to `~/.triibal/cron/output/` |
...
The scheduler uses cross-process file-based locking (`fcntl.flock` on Unix, `msvcrt.locking` on Windows) to prevent overlapping ticks from executing the same due-job batch twice — even between the gateway's in-process ticker and a standalone `triibal cron` / manual `tick()` call. If the lock cannot be acquired, `tick()` returns 0 immediately.

## CLI Interface

The `triibal cron` CLI provides direct job management:

```bash
triibal cron list                    # Show all jobs
triibal cron create                  # Interactive job creation (alias: add)
triibal cron edit <job_id>           # Edit job configuration
triibal cron pause <job_id>          # Pause a running job
triibal cron resume <job_id>         # Resume a paused job
triibal cron run <job_id>            # Trigger immediate execution
triibal cron remove <job_id>         # Delete a job
```
