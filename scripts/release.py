...
VERSION_FILE = REPO_ROOT / "triibal_cli" / "__init__.py"
...
AUTHOR_MAP = {
    ...
    "triibal@marian.local": "Schrotti77",
    ...
    "274096618+triibal-agent-dhabibi@users.noreply.github.com": "dhabibi",
    ...
    "triibal@nousresearch.com": "NousResearch",
    ...
    "triibal@example.com": "shellybotmoyer",  # PR #18915 (bot-committed)
    ...
    "teknium@triibal-agent": "teknium1",
    ...
    "sylw3st3rr@gmail.com": "Sylw3ster",  # salvage of #23252 (TRIIBAL_KANBAN_BOARD restore)
    ...
    "58116817+TurgutKural@users.noreply.github.com": "TurgutKural",  # salvage of #23356 (TRIIBAL_HOME inject)
    ...
    "sauravsejal40@gmail.com": "Saurav0989",  # PR #27071 (docs: triibal-eval community link)
    ...
    "triibalagent26@gmail.com": "triibalagent26",  # PR #26438 (kimi model-name reasoning pad)
    "276067471+triibalagent26@users.noreply.github.com": "triibalagent26",
    ...
    "vigo@triibal": "hawknewton",  # PR #26294 (bedrock boto3 lazy_deps)
    ...
    "hex.triibal@agentmail.to": "oseftg",
    ...
    "kdkcfp@gmail.com": "slowtokki0409",  # PR #29025 (ignore local Triibal runtime files)
...
def _update_acp_registry_versions(semver: str) -> None:
    ...
    if ACP_REGISTRY_MANIFEST.exists():
        ...
        if "package" in uvx:
            uvx["package"] = f"triibal-agent[acp]=={semver}"
...
def parse_coauthors(body: str) -> list:
    ...
    _ignored_emails = {"noreply@anthropic.com", "noreply@github.com",
                       "cursoragent@cursor.com", "triibal@nousresearch.com"}
...
def generate_changelog(commits, tag_name, semver, repo_url="https://github.com/Triibal/triibal",
    ...
    lines.append(f"# Triibal Agent v{semver} ({tag_name})")
    lines.append("")
    ...
    if first_release:
        ...
        lines.append("> for Triibal Agent. See below for everything included in this initial release.")
        lines.append("")
...
def main():
    parser = argparse.ArgumentParser(description="Triibal Agent Release Tool")
    ...
    print(f"{'='*60}")
    print(f"  Triibal Agent Release Preview")
    print(f"{'='*60}")
    ...
    if args.publish:
        ...
        tag_result = git_result(
            "tag", "-a", tag_name, "-m",
            f"Triibal Agent v{new_version} ({calver_date})\n\nWeekly release"
        )
        ...
        gh_cmd = [
            ...
            "--title", f"Triibal Agent v{new_version} ({calver_date})",
        ...
        if result and result.returncode == 0:
            ...
            print(
                f"    gh release create {tag_name} --title 'Triibal Agent v{new_version} ({calver_date})' "
