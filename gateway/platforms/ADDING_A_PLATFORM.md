...
There are two ways to add a platform to the Triibal gateway:

## Plugin Path (Recommended for Community/Third-Party)

Create a plugin directory in `~/.triibal/plugins/` (or under `plugins/platforms/`
...
**zero changes to core Triibal code**.
...
- `env_enablement_fn: () -> Optional[dict]` — seeds `PlatformConfig.extra`
  ...
  `triibal gateway status` or `get_connected_platforms()` until the SDK
  instantiates.
...
- `plugin.yaml` `requires_env` / `optional_env` rich-dict entries —
  auto-populate `OPTIONAL_ENV_VARS` in `triibal_cli/config.py` so the setup
...
Checklist for integrating a platform directly into the Triibal core.
...
And add it to the `triibal-gateway` composite:

```python
"triibal-gateway": {
    "includes": [..., "triibal-your-platform"]
}
```
...
## 12. Status Display (`triibal_cli/status.py`)
...
## 13. Gateway Setup Wizard (`triibal_cli/gateway.py`)
...
grep -r "telegram\|discord\|whatsapp\|slack" gateway/ tools/ agent/ cron/ triibal_cli/ toolsets.py \
  --include="*.py" -l | sort -u
