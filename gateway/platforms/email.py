"""
Email platform adapter for the Triibal gateway.

Allows users to interact with Triibal by sending emails.
...
def _send_imap_id(imap: "imaplib.IMAP4") -> None:
    ...
    try:
        ...
        imap.xatom(
            "ID",
            f'("name" "triibal-agent" "version" "{_triibal_version}" '
            '"vendor" "NousResearch" '
...
class EmailAdapter(BasePlatformAdapter):
    ...
    def _send_email(
        ...
        subject = ctx.get("subject", "Triibal Agent")
        ...
        msg_id = f"<triibal-{uuid.uuid4().hex[:12]}@{self._address.split('@')[1]}>"
