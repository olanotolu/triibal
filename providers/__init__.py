...
1. Bundled plugins: ``plugins/model-providers/<name>/`` (shipped with triibal-agent)
2. User plugins: ``$TRIIBAL_HOME/plugins/model-providers/<name>/``
...
def register_provider(profile: ProviderProfile) -> None:
    ...
    plugins under ``$TRIIBAL_HOME/plugins/model-providers/`` can override
...
def _user_plugins_dir() -> Path | None:
    """Return ``$TRIIBAL_HOME/plugins/model-providers/`` if it exists."""
    try:
        from triibal_constants import get_triibal_home

        d = get_triibal_home() / "plugins" / "model-providers"
...
def _import_plugin_dir(plugin_dir: Path, source: str) -> None:
    ...
    # multiple TRIIBAL_HOME profiles don't alias each other.
    safe_name = plugin_dir.name.replace("-", "_")
    if source == "bundled":
        module_name = f"plugins.model_providers.{safe_name}"
    else:
        module_name = f"_triibal_user_provider_{safe_name}"
...
def _discover_providers() -> None:
    ...
    Order:
      ...
      2. User plugins at ``$TRIIBAL_HOME/plugins/model-providers/<name>/``
    ...
    # 1. Bundled plugins — shipped with triibal-agent.
    ...
    # 2. User plugins — under $TRIIBAL_HOME/plugins/model-providers/<name>/.
