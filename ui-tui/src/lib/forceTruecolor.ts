...
export function shouldForceTruecolor(env: NodeJS.ProcessEnv = process.env): boolean {
  const override = (env.TRIIBAL_TUI_TRUECOLOR ?? '').trim()
...
if (shouldForceTruecolor()) {
  ...
  // incorrectly. Keep Triibal on the safer TERM-driven 256-color path unless
  // users explicitly opt back in via TRIIBAL_TUI_TRUECOLOR=1.
