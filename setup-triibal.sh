...
# 5. Symlinks the 'triibal' CLI command into a user-facing bin dir
...
echo ""
echo -e "${CYAN}⚕ Triibal Agent Setup${NC}"
echo ""
...
if is_termux; then
    ...
    if [ -n "$UV_CMD" ]; then
        ...
        _uv_log="$(mktemp 2>/dev/null || echo "/tmp/triibal-uv-install.$$.log")"
...
# ============================================================================
# PATH setup — symlink triibal into a user-facing bin dir
# ============================================================================

echo -e "${CYAN}→${NC} Setting up triibal command..."

TRIIBAL_BIN="$SCRIPT_DIR/venv/bin/triibal"
COMMAND_LINK_DIR="$(get_command_link_dir)"
COMMAND_LINK_DISPLAY_DIR="$(get_command_link_display_dir)"
mkdir -p "$COMMAND_LINK_DIR"
ln -sf "$TRIIBAL_BIN" "$COMMAND_LINK_DIR/triibal"
echo -e "${GREEN}✓${NC} Symlinked triibal → $COMMAND_LINK_DISPLAY_DIR/triibal"
...
# ============================================================================
# Seed bundled skills into ~/.triibal/skills/
# ============================================================================

TRIIBAL_SKILLS_DIR="${TRIIBAL_HOME:-$HOME/.triibal}/skills"
mkdir -p "$TRIIBAL_SKILLS_DIR"

echo ""
echo "Syncing bundled skills to ~/.triibal/skills/ ..."
if "$SCRIPT_DIR/venv/bin/python" "$SCRIPT_DIR/tools/skills_sync.py" 2>/dev/null; then
    echo -e "${GREEN}✓${NC} Skills synced"
else
    # Fallback: copy if sync script fails (missing deps, etc.)
    if [ -d "$SCRIPT_DIR/skills" ]; then
        cp -rn "$SCRIPT_DIR/skills/"* "$TRIIBAL_SKILLS_DIR/" 2>/dev/null || true
...
echo "Other commands:"
echo "  triibal status        # Check configuration"
if is_termux; then
    echo "  triibal gateway       # Run gateway in foreground"
else
    echo "  triibal gateway install # Install gateway service (messaging + cron)"
fi
echo "  triibal cron list     # View scheduled jobs"
echo "  triibal doctor        # Diagnose issues"
echo ""
...
if [[ $REPLY =~ ^[Yy]$ ]] || [[ -z $REPLY ]]; then
    ...
    "$SCRIPT_DIR/venv/bin/python" -m triibal_cli.main setup
fi
