"""Shell completion script generation for triibal CLI.
...
def generate_bash(parser: argparse.ArgumentParser) -> str:
    ...
    for cmd in sorted(tree["subcommands"]):
        ...
        if cmd == "profile" and info["subcommands"]:
            ...
            cases.append(
                ...
                f"                    COMPREPLY=($(compgen -W \"$(_triibal_profiles)\" -- \"$cur\"))\n"
                f"                    return\n"
                f"                    ;;\n"
                f"            esac\n"
                f"            ;;"
    ...
    return f"""# Triibal Agent bash completion
# Add to ~/.bashrc:
#   eval "$(triibal completion bash)"

_triibal_profiles() {{
    local profiles_dir="$HOME/.triibal/profiles"
...
_triibal_completion() {{
    ...
    if [[ "$prev" == "-p" || "$prev" == "--profile" ]]; then
        COMPREPLY=($(compgen -W "$(_triibal_profiles)" -- "$cur"))
        return
    fi
...
complete -F _triibal_completion triibal
"""
...
def generate_zsh(parser: argparse.ArgumentParser) -> str:
    ...
    return f"""#compdef triibal
# Triibal Agent zsh completion
# Add to ~/.zshrc:
...
_triibal_profiles() {{
    local -a profiles
    profiles=(default)
    if [[ -d "$HOME/.triibal/profiles" ]]; then
        profiles+=("${{(@f)$(ls $HOME/.triibal/profiles 2>/dev/null)}}")
    fi
...
_triibal() {{
    ...
    _arguments -C \\
        ...
        '(-)'{{-p,--profile}}'[Profile name]:profile:_triibal_profiles' \\
        '1:command:->commands' \\
        '*::arg:->args'
...
{top_cmds_str}
            ...
            _describe 'triibal command' subcmds
            ;;
        args)
            case ${{line[1]}} in
{sub_cases_str}
...
def generate_fish(parser: argparse.ArgumentParser) -> str:
    ...
    lines: list[str] = [
        "# Triibal Agent fish completion",
        "# Add to your config:",
        "#   triibal completion fish | source",
        "",
        "# Helper: list available profiles",
        "function __triibal_profiles",
        "    echo default",
        "    if test -d $HOME/.triibal/profiles",
        "        ls $HOME/.triibal/profiles 2>/dev/null",
        "    end",
        "end",
        "",
        "# Disable file completion by default",
        "complete -c triibal -f",
        "",
        "# Complete profile names after -p / --profile",
        "complete -c triibal -f -s p -l profile"
        " -d 'Profile name' -xa '(__triibal_profiles)'",
        "",
        ...
        if cmd == "profile":
            for action in sorted(profile_name_actions):
                lines.append(
                    ...
                    f"-a '(__triibal_profiles)' -d 'Profile name'"
                )
