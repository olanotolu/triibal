...
Add, remove, or reorder entries here — both `triibal setup` and
`triibal` provider-selection will pick up the change automatically.
"""
...
_TRIIBAL_USER_AGENT = f"triibal-cli/{_TRIIBAL_VERSION}"
...
def _codex_curated_models() -> list[str]:
    ...
    This keeps the gateway /model picker in sync with the CLI `triibal model`
    flow without maintaining a separate static list.
    """
    from triibal_cli.codex_models import DEFAULT_CODEX_MODELS, _add_forward_compat_models
...
# $TRIIBAL_HOME/models_dev_cache.json as of 2026-04-28. Whenever xAI renames
...
def _xai_curated_models() -> list[str]:
    ...
    Reads $TRIIBAL_HOME/models_dev_cache.json directly (no network) so this
    runs at import time without blocking. Falls back to ``_XAI_STATIC_FALLBACK``
    when the cache is empty or unreadable. Triibal refreshes the cache from
...
def union_with_portal_free_recommendations(
    ...
    *,
    ...
    older Triibal that doesn't ship that model in its hardcoded curated
...
def union_with_portal_paid_recommendations(
    ...
    *,
    ...
    picker even if the user is running an older Triibal that doesn't ship
...
# this list:  triibal model, /model, list_authenticated_providers.
#
# Fields:
#   slug        — internal provider ID (used in config.yaml, --provider flag)
#   label       — short display name
#   tui_desc    — longer description for the `triibal model` interactive picker
# ---------------------------------------------------------------------------

class ProviderEntry(NamedTuple):
    slug: str
    label: str
    tui_desc: str   # detailed description for `triibal model` TUI
...
def get_default_model_for_provider(provider: str) -> str:
    ...
    model a user would be offered first in the ``triibal model`` picker.

    Used as a fallback when the user has configured a provider but never
    selected a model (e.g. ``triibal auth add openai-codex`` without
    ``triibal model``).
    """
...
def _openrouter_model_supports_tools(item: Any) -> bool:
    ...
    triibal-agent is tool-calling-first — every provider path assumes the model
...
def fetch_openrouter_models(
    ...
    *,
    ...
    try:
        from triibal_cli.model_catalog import get_curated_openrouter_models
    ...
    for preferred_id in preferred_ids:
        ...
        if live_item is None:
            continue
        # Hide models that don't advertise tool-calling support — triibal-agent
...
def get_curated_nous_model_ids() -> list[str]:
    ...
    """
    try:
        from triibal_cli.model_catalog import get_curated_nous_models
...
def _resolve_nous_pricing_credentials() -> tuple[str, str]:
    ...
    """
    try:
        from triibal_cli.auth import resolve_nous_runtime_credentials
...
def list_available_providers() -> list[dict[str, str]]:
    ...
    source of truth shared with ``triibal model``, ``/model``, etc.).
    """
    ...
    for pid in provider_order:
        ...
        has_creds = False
        try:
            from triibal_cli.auth import get_auth_status, has_usable_secret
            if pid == "custom":
...
def _resolve_static_model_alias(
    ...
    try:
        from triibal_cli.model_switch import MODEL_ALIASES
    except Exception:
        return None
...
def normalize_provider(provider: Optional[str]) -> str:
    """Normalize provider aliases to Triibal' canonical provider ids.

    Note: ``"auto"`` passes through unchanged — use
    ``triibal_cli.auth.resolve_provider()`` to resolve it to a concrete
...
def model_supports_fast_mode(model_id: Optional[str]) -> bool:
    """Return whether Triibal should expose the /fast toggle for this model."""
...
def _resolve_copilot_catalog_api_key() -> str:
    ...
    Resolution order:
      ...
      2. ``read_credential_pool("copilot")`` — a token (typically a
         ...
         populated by ``triibal auth add copilot`` and by ``_seed_from_env``
         when the env var is set in ``~/.triibal/.env``.
    ...
    """
    try:
        from triibal_cli.auth import resolve_api_key_provider_credentials
    ...
    try:
        from triibal_cli.auth import read_credential_pool
        from triibal_cli.copilot_auth import (
...
def provider_model_ids(provider: Optional[str], *, force_refresh: bool = False) -> list[str]:
    ...
    on the platform appear in ``/model`` without a Triibal release.
    """
    normalized = normalize_provider(provider)
    if normalized == "openrouter":
    ...
    if normalized == "openai-codex":
        ...
        # a Triibal release). Falls back to the hardcoded catalog if no token
    ...
    if normalized == "nous":
        ...
        try:
            from triibal_cli.auth import fetch_nous_models, resolve_nous_runtime_credentials
            creds = resolve_nous_runtime_credentials()
            if creds:
        ...
        # added Portal models still surface without a Triibal release.
...
#
# Cache strategy:
#   - One JSON file at $TRIIBAL_HOME/provider_models_cache.json
...
def _provider_models_cache_path() -> Path:
    ...
    return get_triibal_home() / "provider_models_cache.json"
...
def _credential_fingerprint(provider: str) -> str:
    ...
    relevant tokens live in ``$TRIIBAL_HOME/auth.json`` and external
...
def _lmstudio_request_headers(api_key: Optional[str] = None) -> dict:
    ...
    headers = {"User-Agent": _TRIIBAL_USER_AGENT}
...
_COPILOT_MODEL_ALIASES = {
    ...
    # Dash-notation fallbacks: Triibal' default Claude IDs elsewhere use
...
def probe_api_models(
    ...
    tried: list[str] = []
    headers: dict[str, str] = {"User-Agent": _TRIIBAL_USER_AGENT}
...
def _ollama_cloud_cache_path() -> Path:
    ...
    from triibal_constants import get_triibal_home
    return get_triibal_home() / "ollama_cloud_models_cache.json"
    ...
    *,
    ...
    if normalized == "custom" or normalized.startswith("custom:"):
        ...
        message = (
            ...
            f"Triibal will still save `{requested}`, but the endpoint should expose `/models` for verification."
        )
        if api_mode == "anthropic_messages":
            message += (
        ...
        if catalog_models:
            ...
            return {
                ...
                "message": (
                    ...
                    f"{suggestion_text}"
                    "\n  MiniMax does not expose a /models endpoint, so Triibal cannot verify the model name."
