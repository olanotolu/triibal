...
the existing module-level functions in triibal_cli.gateway and
triibal_cli.gateway_windows directly. This protocol is a thin facade
...
``triibal gateway start/stop/restart`` when running inside a container.
"""
...
def detect_service_manager() -> ServiceManagerKind:
    ...
    in ``triibal gateway start/stop/restart``).
    """
...
def _s6_running() -> bool:
    ...
    Detection has to work for **both** root and the unprivileged triibal
    ...
    detection always fails. Since every Triibal runtime call inside the
    container drops to triibal via ``s6-setuidgid``, that silent failure
...
# in ``triibal_cli.gateway`` (systemd/launchd) and ``triibal_cli.gateway_windows``
...
# active (set via the ``triibal -p <profile>`` flag before the call). This
...
class SystemdServiceManager(_RegistrationUnsupportedMixin):
    """Thin wrapper around the ``systemd_*`` functions in triibal_cli.gateway.
    ...
    def start(self, name: str) -> None:
        from triibal_cli.gateway import systemd_start
        systemd_start()

    def stop(self, name: str) -> None:
        from triibal_cli.gateway import systemd_stop
        systemd_stop()

    def restart(self, name: str) -> None:
        from triibal_cli.gateway import systemd_restart
        systemd_restart()

    def is_running(self, name: str) -> bool:
        from triibal_cli.gateway import _probe_systemd_service_running
        _, running = _probe_systemd_service_running()
        return running


class LaunchdServiceManager(_RegistrationUnsupportedMixin):
    """Thin wrapper around the ``launchd_*`` functions in triibal_cli.gateway."""

    kind: ServiceManagerKind = "launchd"

    def start(self, name: str) -> None:
        from triibal_cli.gateway import launchd_start
        launchd_start()

    def stop(self, name: str) -> None:
        from triibal_cli.gateway import launchd_stop
        launchd_stop()

    def restart(self, name: str) -> None:
        from triibal_cli.gateway import launchd_restart
        launchd_restart()

    def is_running(self, name: str) -> bool:
        from triibal_cli.gateway import _probe_launchd_service_running
        return _probe_launchd_service_running()


class WindowsServiceManager(_RegistrationUnsupportedMixin):
    """Thin wrapper around ``triibal_cli.gateway_windows`` (Scheduled Task /
    ...
    def is_running(self, name: str) -> bool:
        from triibal_cli import gateway_windows
        from triibal_cli.gateway import find_gateway_pids
...
#
# Per-profile gateways are registered dynamically when `triibal profile create`
# runs inside the container (Phase 4). Static services (main-triibal, dashboard)
...
# UID/GID of the in-image ``triibal`` user. Hardcoded to match what
...
def _seed_supervise_skeleton(svc_dir: Path) -> None:
    ...
    inside a service directory, owned by the triibal user.
    ...
    (the ``triibal`` user — UID 10000 — running every Triibal runtime
    ...
    is dispatched as the triibal user.
    ...
    with triibal ownership before triggering ``s6-svscanctl -a``,
    s6-supervise inherits our layout and never touches it.

    Layout produced
    ---------------
    ``svc_dir/``                           triibal:triibal, 0755 (parent must already exist)
    ``svc_dir/event/``                     triibal:triibal, 03730   (setgid + g+rwx + sticky)
    ``svc_dir/supervise/``                 triibal:triibal, 0755
    ``svc_dir/supervise/event/``           triibal:triibal, 03730
    ``svc_dir/supervise/control``          triibal:triibal, 0660    (FIFO)
    ...
    slot's supervise/ was triibal-owned.
    ...
    def _mkdir_owned(path: Path, mode: int) -> None:
        ...
        path.chmod(mode)
        try:
            os.chown(path, _TRIIBAL_UID, _TRIIBAL_GID)
        except PermissionError:
            # Running as the triibal user already — directory is triibal-
    ...
    if not control.exists():
        ...
        control.chmod(0o660)
        try:
            os.chown(control, _TRIIBAL_UID, _TRIIBAL_GID)
        except PermissionError:
            pass
    ...
    # when the parent slot's supervise/ is triibal-owned.
    log_dir = svc_dir / "log"
    if log_dir.is_dir():
        ...
        if not log_control.exists():
            ...
            try:
                os.chown(log_control, _TRIIBAL_UID, _TRIIBAL_GID)
            except PermissionError:
                pass
...
class GatewayNotRegisteredError(S6Error):
    ...
    Most commonly: ``triibal -p typo gateway start`` when no profile
    ...
    def __init__(self, profile: str) -> None:
        ...
        super().__init__(
            ...
            f"`triibal profile create {profile}` first, or pass "
...
class S6ServiceManager:
    ...
    ``S6_DYNAMIC_SCANDIR``. Static services (main-triibal, dashboard)
    ...
    def _render_run_script(
        ...
        The script:
          1. Sources TRIIBAL_HOME (and any extra env) via with-contenv —
             so e.g. ``-e TRIIBAL_HOME=/data/triibal`` is honored at run
          ...
          4. Drops to the triibal user and exec's
             ``triibal -p <profile> gateway run`` (or just ``triibal
             gateway run`` for the default profile — see below).

        Special case: ``profile == "default"`` emits ``triibal gateway
        run`` with **no** ``-p`` flag. This is the sentinel for "the
        root TRIIBAL_HOME profile" (the implicit profile that exists at
        the top of $TRIIBAL_HOME, not under profiles/). It must be
        spelled this way because ``_profile_suffix()`` returns the
        empty string for the root profile, and the dispatcher in
        ``triibal_cli.gateway`` maps that empty string to the
        ``gateway-default`` service slot. Passing ``-p default`` here
        would instead look up ``$TRIIBAL_HOME/profiles/default/`` — a
        ...
        ``triibal_cli.profiles._allocate_gateway_port``). PR #30136
        ...
        # guard.
        lines.append("export TRIIBAL_S6_SUPERVISED_CHILD=1")
        if profile == "default":
            lines.append("exec s6-setuidgid triibal triibal gateway run")
        else:
            lines.append(
                f"exec s6-setuidgid triibal triibal -p {shlex.quote(profile)} gateway run"
            )
        return "\n".join(lines) + "\n"
    ...
    def _render_log_run(profile: str) -> str:
        ...
        OQ8-C: persist to ``${TRIIBAL_HOME}/logs/gateways/<profile>/``.
        CRITICAL: the TRIIBAL_HOME path is sourced from the runtime env
        via with-contenv — NOT Python-substituted at registration time
        — so a container started with ``-e TRIIBAL_HOME=/data/triibal``
        gets its logs under /data/triibal/logs/..., not the build-time
        default.
        ...
        per line, in order:
          ...
          2. ``T <log_dir>`` — also write a timestamped copy to the
             ...
             files). This is what ``triibal logs`` reads and what
        ...
        return (
            ...
            f': "${{TRIIBAL_HOME:=/opt/data}}"\n'
            f'log_dir="$TRIIBAL_HOME/logs/gateways/{prof}"\n'
            f'mkdir -p "$log_dir"\n'
            f'chown -R triibal:triibal "$log_dir" 2>/dev/null || true\n'
            f'exec s6-setuidgid triibal s6-log 1 n10 s1000000 T "$log_dir"\n'
        )
    ...
    def register_profile_gateway(
        ...
        *,
        ...
        try:
            ...
            # Pre-create the supervise/ skeleton with triibal ownership
            ...
            # the triibal user) won't hit EACCES on root-owned 0700
    ...
    def unregister_profile_gateway(self, profile: str) -> None:
        ...
        **before** ``rmtree`` so s6-svscan reaps the supervise child
        ...
        # is triibal-owned (see ``_seed_supervise_skeleton``), and on
