...
Usage:
    triibal pairing list              # Show all pending + approved users
    triibal pairing approve <platform> <code>  # Approve a pairing code
    triibal pairing revoke <platform> <user_id> # Revoke user access
    triibal pairing clear-pending     # Clear all expired/pending codes
"""

def pairing_command(args):
    """Handle triibal pairing subcommands."""
    ...
    if action == "list":
        ...
        print("Usage: triibal pairing {list|approve|revoke|clear-pending}")
        print("Run 'triibal pairing --help' for details.")
...
def _cmd_approve(store, platform: str, code: str):
    ...
    if result:
        ...
        print(
            ...
            "~/.triibal/platforms/pairing/_rate_limits.json\n".format(platform)
        )
    else:
        print(f"\n  Code '{code}' not found or expired for platform '{platform}'.")
        print("  Run 'triibal pairing list' to see pending codes.\n")
