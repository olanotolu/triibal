...
on Japanese locales, etc.).  Triibal's banners, tool output feed, and slash
...
Entry points (``cli.py`` ``main``, ``triibal_cli/main.py`` CLI dispatch,
...
def _flip_console_code_page_to_utf8() -> None:
    ...
    is silent — if there's no attached console (e.g. Triibal is running
...
def configure_windows_stdio() -> bool:
    ...
    Set ``TRIIBAL_DISABLE_WINDOWS_UTF8=1`` in the environment to opt out
    ...
    if os.environ.get("TRIIBAL_DISABLE_WINDOWS_UTF8") in {"1", "true", "True", "yes"}:
        _CONFIGURED = True
        return False
    ...
    # Augment PATH with the Triibal-managed Git install directories so
    ...
    # already-running shells don't see the change — which means triibal
    ...
    # install without Triibal-managed PortableGit).
    ...
    # stdin is re-configured for completeness; Triibal's interactive
...
def _default_windows_editor() -> str:
    ...
    The prompt_toolkit buffer's ``open_in_editor`` and Triibal's
    ``triibal config edit`` both honour ``$EDITOR``.  Users who prefer a
    ...
    Set this before launching Triibal (User env var in Windows Settings, or
    export in a PowerShell profile) and Triibal picks it up automatically.
    """
    import shutil
...
def _augment_path_with_known_tools() -> None:
    """Prepend well-known Triibal-managed tool directories to os.environ['PATH'].
    ...
    ``%LOCALAPPDATA%\\triibal\\git\\bin`` to the User PATH via
    ...
    *spawned* processes only — already-running shells (including the one the
    user invokes ``triibal`` from right after install) retain their old PATH.

    Any subprocess Triibal spawns — bash, ``rg``, ``grep``, ``npm`` — inherits
    ...
    Patch-up strategy: add the known Triibal-managed tool directories to our
    ...
    # see on next launch.
    candidate_dirs = [
        os.path.join(local_appdata, "triibal", "git", "cmd"),
        os.path.join(local_appdata, "triibal", "git", "bin"),
        os.path.join(local_appdata, "triibal", "git", "usr", "bin"),
        # Triibal venv Scripts directory — host of the triibal.exe shim itself,
        # also where any pip-installed console scripts land.  Usually already
        # on PATH when the user invokes triibal, but harmless to include.
        os.path.join(local_appdata, "triibal", "triibal-agent", "venv", "Scripts"),
