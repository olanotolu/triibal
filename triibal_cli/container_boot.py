...
``$TRIIBAL_HOME/profiles/<name>/`` live on the persistent VOLUME, and
...
Dockerfile (Phase 4 Task 4.0). Runs as root after 01-triibal-setup
(the stage2 hook) has chowned the volume and seeded $TRIIBAL_HOME, but
...
def reconcile_profile_gateways(
    *,
    ...
    (the implicit profile that lives at the top of ``$TRIIBAL_HOME``,
    not under ``profiles/``). The dispatcher in ``triibal_cli.gateway``
    maps an empty profile suffix to ``gateway-default``, so this slot
    is what ``triibal gateway start`` (no ``-p``) targets. Without it,
    bare ``triibal gateway start`` inside the container would land on
    ...
    ``$TRIIBAL_HOME/gateway_state.json`` (sibling to the profile root,
    not under ``profiles/``); stale runtime files there are swept the
    same way as for named profiles.

    Args:
        triibal_home: The container's TRIIBAL_HOME (typically /opt/data).
            Profiles live under ``<triibal_home>/profiles/<name>/``;
            the default profile lives at ``<triibal_home>`` itself.
    ...
    # ``triibal gateway start`` (no ``-p``) has somewhere to land;
    # auto-up only when the prior state was "running" (same rule as
    # named profiles).
    default_prior_state = _read_prior_state(triibal_home)
    default_should_start = default_prior_state in _AUTOSTART_STATES
    if not dry_run:
        _cleanup_stale_runtime_files(triibal_home)
    ...
    profiles_root = triibal_home / "profiles"
    if profiles_root.is_dir():
        for entry in sorted(profiles_root.iterdir()):
            if not entry.is_dir():
                continue
            # SOUL.md is always seeded by `triibal profile create` (config.yaml
            # is not — that comes later via `triibal setup`). Use it as the
            ...
            # via ``triibal -p default-named gateway start`` if they
    ...
    if not dry_run:
        _write_reconcile_log(triibal_home, actions)
    return actions
...
def _register_service(scandir: Path, profile: str, *, start: bool) -> None:
    ...
    try:
        ...
        # it up explicitly with `triibal -p <profile> gateway start`
        ...
        # spawns will EEXIST our dirs/FIFOs and inherit triibal
        # ownership, so runtime s6-svc / s6-svstat / s6-svwait calls
        # (all dispatched as the triibal user) won't hit EACCES. See
...
def _write_reconcile_log(
    triibal_home: Path, actions: list[ReconcileAction],
) -> None:
    """Append one line per profile to $TRIIBAL_HOME/logs/container-boot.log.
...
def main() -> int:
    ...
    triibal_home = Path(os.environ.get("TRIIBAL_HOME", "/opt/data"))
    scandir = Path(os.environ.get("S6_PROFILE_GATEWAY_SCANDIR", "/run/service"))
    actions = reconcile_profile_gateways(
        triibal_home=triibal_home, scandir=scandir,
    )
    for a in actions:
        print(
