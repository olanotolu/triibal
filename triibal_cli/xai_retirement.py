...
Pure logic: walks a Triibal config dict, returns issues for any reference
to a retired xAI model. No I/O, no CLI dependencies — testable in isolation
and reusable from both `triibal doctor` and a future `triibal migrate xai`.
"""
...
class RetirementIssue:
    """A reference to a retired xAI model found in a Triibal config."""
...
def find_retired_xai_refs(config: Dict[str, Any]) -> List[RetirementIssue]:
    """Walk all model slots in a Triibal config and return retirement issues.
