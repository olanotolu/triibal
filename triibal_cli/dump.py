"""
Dump command for triibal CLI.

Outputs a compact, plain-text summary of the user's Triibal setup
...
from triibal_cli.config import get_triibal_home, get_env_path, get_project_root, load_config
...
def _get_git_commit(project_root: Path) -> str:
    ...
    to ``<project_root>/.triibal_build_sha`` by the Dockerfile's
    ``TRIIBAL_GIT_SHA`` build-arg (see ``triibal_cli/build_info.py``).
...
def _redact(value: str) -> str:
    ...
    ``triibal dump`` formats empty values as blank, not as ``"(not set)"``).
    """
...
def _gateway_status() -> str:
    ...
    try:
        from triibal_cli.gateway import get_gateway_runtime_snapshot
...
def _count_skills(triibal_home: Path) -> int:
...
def _cron_summary(triibal_home: Path) -> str:
    """Return cron jobs summary."""
    jobs_file = triibal_home / "cron" / "jobs.json"
...
def run_dump(args):
    ...
    lines.append(f"triibal_home:      {display_triibal_home()}")
    ...
    toolsets = config.get("toolsets", ["triibal-cli"])
    ...
    lines.append(f"  cron_jobs:          {_cron_summary(triibal_home)}")
    lines.append(f"  skills:             {_count_skills(triibal_home)}")
