"""`triibal checkpoints` CLI subcommand.

Gives users direct visibility and control over the filesystem checkpoint
store at ``~/.triibal/checkpoints/``.  Actions:

    triibal checkpoints               # same as `status`
    triibal checkpoints status        # total size, project count, breakdown
    triibal checkpoints list          # per-project checkpoint counts + workdir
    triibal checkpoints prune [opts]  # force a sweep (ignores the 24h marker)
    triibal checkpoints clear [-f]    # nuke the entire base (asks first)
    triibal checkpoints clear-legacy  # delete just the legacy-* archives

Examples::

    triibal checkpoints
    triibal checkpoints prune --retention-days 3 --max-size-mb 200
    triibal checkpoints clear -f
...
def cmd_status(args: argparse.Namespace) -> int:
    ...
    if legacy:
        ...
        print()
        print("Clear with: triibal checkpoints clear-legacy")
    return 0
...
def register_cli(parser: argparse.ArgumentParser) -> None:
    """Wire subcommands onto the ``triibal checkpoints`` parser."""
    parser.set_defaults(func=cmd_status)  # bare `triibal checkpoints` → status
