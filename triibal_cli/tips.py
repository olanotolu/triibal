...
TIPS = [
    ...
    "/title \"my project\" names your session — resume it later with /resume or triibal -c.",
    ...
    "Ctrl+Z suspends Triibal to the background — run fg in your shell to resume.",
    ...
    # --- CLI Flags ---
    "triibal -c resumes your most recent CLI session. triibal -c \"project name\" resumes by title.",
    "triibal -w creates an isolated git worktree — perfect for parallel agent workflows.",
    "triibal -w -q \"Fix issue #42\" combines worktree isolation with a one-shot query.",
    "triibal chat -t web,terminal enables only specific toolsets for a focused session.",
    "triibal chat -s github-pr-workflow preloads a skill at launch.",
    "triibal chat -q \"query\" runs a single non-interactive query and exits.",
    "triibal chat --max-turns 200 overrides the default 90-iteration limit per turn.",
    "triibal chat --checkpoints enables filesystem snapshots before every destructive file change.",
    "triibal --yolo bypasses all dangerous command approval prompts for the entire session.",
    "triibal chat --source telegram tags the session for filtering in triibal sessions list.",
    "triibal -p work chat runs under a specific profile without changing your default.",

    # --- CLI Subcommands ---
    "triibal doctor --fix diagnoses and auto-repairs config and dependency issues.",
    "triibal dump outputs a compact setup summary — great for bug reports.",
    "triibal config set KEY VALUE auto-routes secrets to .env and everything else to config.yaml.",
    "triibal config edit opens config.yaml in your default editor.",
    "triibal config check scans for missing or stale configuration options.",
    "triibal sessions browse opens an interactive session picker with search.",
    "triibal sessions stats shows session counts by platform and database size.",
    "triibal sessions prune --older-than 30 cleans up old sessions.",
    "triibal skills search react --source skills-sh searches the skills.sh public directory.",
    "triibal skills check scans installed hub skills for upstream updates.",
    "triibal skills tap add myorg/skills-repo adds a custom GitHub skill source.",
    "triibal skills snapshot export setup.json exports your skill configuration for backup or sharing.",
    "triibal mcp add github --command npx adds MCP servers from the command line.",
    "triibal mcp serve runs Triibal itself as an MCP server for other agents.",
    "triibal auth add lets you add multiple API keys for credential pool rotation.",
    "triibal completion bash >> ~/.bashrc enables tab completion for all commands and profiles.",
    "triibal logs -f follows agent.log in real time. --level WARNING --since 1h filters output.",
    "triibal backup creates a zip backup of your entire Triibal home directory.",
    "triibal profile create coder creates an isolated profile that becomes its own command.",
    "triibal profile create work --clone copies your current config and keys to a new profile.",
    "triibal update syncs new bundled skills to ALL profiles automatically.",
    "triibal gateway install sets up Triibal as a system service (systemd/launchd).",
    "triibal memory setup lets you configure an external memory provider (Honcho, Mem0, etc.).",
    "triibal webhook subscribe creates event-driven webhook routes with HMAC validation.",
    "Save money: triibal tools disables unused tools, triibal skills config trims skills down.",
    "/reasoning low or /reasoning minimal cuts thinking depth below the default (medium) — faster, cheaper responses.",
    "triibal models routes vision, compression, and aux tasks to cheaper models — cuts background token cost 85%+ without downgrading your main chat model.",
    ...
    "Set worktree: true in config.yaml to always create a git worktree (same as triibal -w).",
    ...
    "TRIIBAL_TIMEZONE overrides the server timezone with any IANA timezone string.",
    ...
    # --- Tools & Capabilities ---
    "execute_code runs Python scripts that call Triibal tools programmatically — results stay out of context.",
    ...
    "Profile names become shell commands — 'triibal profile create coder' creates the 'coder' command.",
    "triibal profile export coder -o backup.tar.gz creates a portable profile archive.",
    ...
    "When exiting, Triibal prints a resume command with session ID and stats.",
    "triibal sessions export backup.jsonl exports all sessions for backup or analysis.",
    "triibal -r SESSION_ID resumes any specific past session by its ID.",
    ...
    "triibal skills install official/security/1password installs optional skills from the repo.",
    ...
    "The plan skill saves markdown plans under .triibal/plans/ in the active workspace.",

    # --- Cron & Scheduling ---
    "Cron jobs can attach skills: triibal cron add --skill blogwatcher \"Check for new posts\".",
    ...
    # --- Gateway & Messaging ---
    "Triibal runs on 21 messaging platforms: Telegram, Discord, Slack, WhatsApp, Signal, Matrix, IRC, Microsoft Teams, email, and more.",
    "triibal gateway install sets it up as a system service that starts on boot.",
    "DingTalk uses Stream Mode — no webhooks or public URL needed.",
    "BlueBubbles brings iMessage to Triibal via a local macOS server.",
    ...
    "Context files (.triibal.md, AGENTS.md) are security-scanned for prompt injection before loading.",
    ...
    "SOUL.md at ~/.triibal/SOUL.md is the agent's primary identity — customize it to shape behavior.",
    "Triibal loads project context from .triibal.md, AGENTS.md, CLAUDE.md, or .cursorrules (first match).",
    ...
    # --- MCP ---
    "triibal mcp opens an interactive picker of Nous-approved MCPs you can install in one keystroke.",
    "triibal mcp catalog lists Nous-approved MCP servers shipped with the repo.",
    "triibal mcp install <name> installs a catalog entry, prompts for credentials, and lets you pick which of its tools to enable.",
    ...
    "MCP servers auto-generate toolsets at runtime — triibal tools can toggle them per platform.",
    ...
    "Checkpoints use shadow repos in ~/.triibal/checkpoints/ — your project's .git is never touched.",

    # --- Batch & Data ---
    "batch_runner.py processes hundreds of prompts in parallel for training data generation.",
    "triibal chat -Q enables quiet mode for programmatic use — suppresses banner and spinner.",
    ...
    "triibal plugins install owner/repo installs plugins directly from GitHub.",
    ...
    "triibal acp runs Triibal as an ACP server for VS Code, Zed, and JetBrains integration.",
    "Custom providers: save named endpoints in config.yaml under custom_providers.",
    "TRIIBAL_EPHEMERAL_SYSTEM_PROMPT injects a system prompt that's never persisted to history.",
    "credential_pool_strategies supports fill_first, round_robin, least_used, and random rotation.",
    "triibal auth add nous or triibal auth add openai-codex sets up OAuth-based providers.",
    ...
    "triibal status --deep runs deeper diagnostic checks across all components.",
    ...
    "Cron scripts live in ~/.triibal/scripts/ and run before the agent — perfect for data collection pipelines.",
    "prefill_messages_file in config.yaml injects few-shot examples into every API call, never saved to history.",
    "SOUL.md completely replaces the agent's default identity — rewrite it to make Triibal your own.",
    "SOUL.md is auto-seeded with a default personality on first run. Edit ~/.triibal/SOUL.md to customize.",
    ...
    "Before a gateway session reset, Triibal auto-flushes important facts to memory in the background.",
    ...
    "The skills audit log at ~/.triibal/skills/.hub/audit.log tracks every install and removal operation.",
    "Stale git worktrees are auto-cleaned: 24-72h old with no unpushed commits get pruned on startup.",
    "Each profile gets its own subprocess HOME at TRIIBAL_HOME/home/ — isolated git, ssh, npm, gh configs.",
    "TRIIBAL_HOME_MODE env var (octal, e.g. 0701) sets custom directory permissions for web server traversal.",
    "Container mode: place .container-mode in TRIIBAL_HOME and the host CLI auto-execs into the container.",
    "Ctrl+C has 5 priority tiers: cancel recording → cancel prompts → cancel picker → interrupt agent → exit.",
    "Every interrupt during an agent run is logged to ~/.triibal/interrupt_debug.log with timestamps.",
    ...
    "Profile names cannot collide with existing PATH binaries — 'triibal profile create ls' would be rejected.",
    "triibal profile create backup --clone-all copies everything (config, keys, SOUL.md, memories, skills, sessions).",
    ...
    "triibal claw migrate --dry-run previews OpenClaw migration without writing anything.",
    ...
    "TRIIBAL_DEV=1 bypasses container mode detection for local development.",
    "Each MCP server gets its own toolset (mcp-servername) that can be toggled independently via triibal tools.",
    "MCP ${ENV_VAR} placeholders in config are resolved at server spawn — including vars from ~/.triibal/.env.",
    "Skills from trusted repos (NousResearch) get a 'trusted' security level; community skills get extra scanning.",
    "The skills quarantine at ~/.triibal/skills/.hub/quarantine/ holds skills pending security review.",

    # --- Advanced Slash Commands ---
    '/steer <prompt> injects a note after the next tool call — nudge direction mid-task without interrupting.',
    '/goal <text> sets a standing Ralph-loop objective — Triibal auto-continues turn after turn until a judge says done.',
    '/snapshot create [label] saves a full state snapshot of Triibal config; /snapshot restore <id> reverts later.',
    ...
    '/busy queue|steer|interrupt controls what pressing Enter does while Triibal is working.',
    ...
    '/reload reloads ~/.triibal/.env into the running session — pick up new API keys without restarting.',
    ...
    "TRIIBAL_CRON_MAX_PARALLEL (default 4) caps how many cron jobs run per tick so bursts don't saturate your keys.",

    # --- Gateway Hooks ---
    'Gateway hooks live under ~/.triibal/hooks/<name>/ with HOOK.yaml + handler.py — handler must be named `handle`.',
    'Hook events include gateway:startup, session:start, agent:step, and command:* wildcard subscriptions.',
    'Drop a ~/.triibal/BOOT.md checklist and a gateway:startup hook runs it as a one-shot agent every boot.',

    # --- Curator ---
    'triibal curator run --dry-run previews what the curator would archive or consolidate without mutating anything.',
    "triibal curator pin <skill> hard-fences a skill against both auto-archival and the agent's skill_manage tool.",
    'triibal curator rollback restores skills from a pre-run snapshot — backups live under skills/.curator_backups/.',

    # --- Credential Pools & Routing ---
    'triibal auth reset <provider> clears all cooldowns and exhaustion flags on a credential pool.',
    ...
    # --- TUI & Dashboard ---
    'TRIIBAL_TUI_RESUME=1 auto-re-attaches to the most recent TUI session on launch — handy after SSH drops.',
    "TRIIBAL_TUI_THEME=light|dark|<hex> forces the TUI theme on terminals that don't set COLORFGBG.",
    ...
    'triibal dashboard launches a local web UI at 127.0.0.1:9119 — zero data leaves localhost.',
    'triibal dashboard --tui embeds the full Triibal TUI in your browser via xterm.js and a WebSocket PTY.',
    'Drop a YAML in ~/.triibal/dashboard-themes/ with two palette colors to reskin the entire dashboard.',
    'Dashboard plugins are drop-in: manifest.json + JS bundle in ~/.triibal/dashboard-plugins/ — no npm build required.',
    ...
    'TRIIBAL_BACKGROUND_NOTIFICATIONS=result only pings when background tasks finish (vs all/error/off).',
    'TRIIBAL_WRITE_SAFE_ROOT restricts write_file and patch to a directory prefix; writes outside require approval.',
    'TRIIBAL_IGNORE_RULES skips auto-injection of AGENTS.md, SOUL.md, .cursorrules, memory, and preloaded skills.',
    'TRIIBAL_ACCEPT_HOOKS auto-approves unseen shell hooks declared in config.yaml without a TTY prompt.',
    ...
    # --- TTS ---
    'tts.provider: piper runs 44-language local TTS on CPU — voices auto-download to ~/.triibal/cache/piper-voices/.',
    ...
    '/reload-skills re-scans ~/.triibal/skills/ so drop-in skills appear without restarting the session.',
    ...
    'triibal -z "<prompt>" is the purest one-shot: final answer on stdout, nothing else — ideal for piping in scripts.',
    'triibal chat --pass-session-id injects the session ID into the system prompt so the agent can self-reference it.',
    'triibal chat --image path/to/pic.png attaches a local image to a single -q query without a separate upload step.',
    'triibal chat --ignore-user-config skips ~/.triibal/config.yaml — reproducible bug reports and CI runs.',
    "triibal chat --source tool tags programmatic chats so they don't clutter triibal sessions list.",
    'triibal dump --show-keys includes redacted API key fingerprints for deeper support debugging.',
    'triibal sessions rename <ID> "new title" renames any past session; triibal sessions delete <ID> removes one.',
    'triibal import restores a session export or profile archive produced by sessions export or profile export.',
    'triibal fallback manages the fallback_model chain interactively — no hand-editing config.yaml.',
    'triibal pairing rotates the DM pairing token — the first messager after rotation claims access to the bot.',
    'triibal setup walks first-time users through provider, keys, and platform wiring in one interactive flow.',
    'triibal status --deep runs the full health sweep across every component; plain triibal status is the quick view.',

    # --- Agent Behavior Env Vars ---
    'TRIIBAL_AGENT_TIMEOUT=0 disables the gateway inactivity kill for a running agent — use for long research runs.',
    'TRIIBAL_ENABLE_PROJECT_PLUGINS=1 auto-loads repo-local plugins from ./.triibal/plugins/ — trust-gated by design.',
    "TRIIBAL_DISABLE_FILE_STATE_GUARD=1 turns off the 'file changed since you read it' guard on patch and write_file.",
    'TRIIBAL_ALLOW_PRIVATE_URLS=true lets web tools hit localhost and private networks — off by default in gateway mode.',
    'TRIIBAL_OPTIONAL_SKILLS=name1,name2 auto-installs extra optional-catalog skills on first run per profile.',
    'TRIIBAL_BUNDLED_SKILLS points at a custom bundled-skill tree — used by Homebrew and Nix packaging.',
    'TRIIBAL_DUMP_REQUEST_STDOUT=1 dumps every API request payload to stdout instead of log files.',
    'TRIIBAL_OAUTH_TRACE=1 logs redacted OAuth token exchange and refresh attempts for debugging provider auth.',
    'TRIIBAL_STREAM_RETRIES (default 3) controls mid-stream reconnect attempts on transient network errors.',

    # --- Gateway Behavior Env Vars ---
    'TRIIBAL_GATEWAY_BUSY_ACK_ENABLED=false silences the ⚡/⏳/⏩ ack messages when a user messages a busy agent.',
    'TRIIBAL_AGENT_NOTIFY_INTERVAL (default 180s) sets how often the gateway pings with progress on long turns.',
    'TRIIBAL_RESTART_DRAIN_TIMEOUT (default 900s) caps how long /restart waits for in-flight runs before forcing.',
    'TRIIBAL_CHECKPOINT_TIMEOUT (default 30s) caps filesystem checkpoint creation — raise it on huge monorepos.',
    ...
    # --- Security ---
    'security.tirith_fail_open: false makes Triibal block commands when the tirith scanner itself errors out.',
    ...
    '--source tool chats are excluded from triibal sessions list by default — set --source explicitly to see them.',
    ...
    'Dashboard plugins are served from /dashboard-plugins/<name>/ — drop files into ~/.triibal/dashboard-plugins/.',
]
