"""CLI handlers for ``triibal migrate ...``."""
...
def cmd_migrate(args: Any) -> int:
    """Dispatcher for ``triibal migrate <subtype>``."""
    sub = getattr(args, "migrate_type", None)
    if sub == "xai":
        return cmd_migrate_xai(args)

    print("usage: triibal migrate {xai} [options]", file=sys.stderr)
    return 2
...
def cmd_migrate_xai(args: Any) -> int:
    ...
    if not apply:
        ...
        print(color(
            "Re-run with `triibal migrate xai --apply` to rewrite "
    ...
    print()
    print(color(
        "Run `triibal doctor` to confirm no retired xAI models remain.",
        Colors.DIM,
    ))
    return 0
