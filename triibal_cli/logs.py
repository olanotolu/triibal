"""``triibal logs`` — view and filter Triibal log files.
...
Usage examples::
    ...
    triibal logs                    # last 50 lines of agent.log
    triibal logs -f                 # follow agent.log in real time
    triibal logs errors             # last 50 lines of errors.log
    triibal logs gateway -n 100    # last 100 lines of gateway.log
    triibal logs --level WARNING    # only WARNING+ lines
    triibal logs --session abc123   # filter by session ID substring
    triibal logs --component tools  # only tool-related lines
    triibal logs --since 1h         # lines from the last hour
    triibal logs --since 30m -f     # follow, starting 30 min ago
"""
...
def tail_log(
    ...
    *,
    ...
    log_path = get_triibal_home() / "logs" / filename
    if not log_path.exists():
        print(f"Log file not found: {log_path}")
        print(f"(Logs are created when Triibal runs — try 'triibal chat' first)")
        sys.exit(1)
    ...
    if follow:
        print(f"--- {display_triibal_home()}/logs/{filename}{filter_desc} (Ctrl+C to stop) ---")
    else:
        print(f"--- {display_triibal_home()}/logs/{filename}{filter_desc} (last {num_lines}) ---")
...
def list_logs() -> None:
    ...
    if not log_dir.exists():
        print(f"No logs directory at {display_triibal_home()}/logs/")
        return

    print(f"Log files in {display_triibal_home()}/logs/:\n")
    found = False
    ...
    if not found:
        print("  (no log files yet — run 'triibal chat' to generate logs)")
