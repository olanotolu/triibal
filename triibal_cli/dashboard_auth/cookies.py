...
Three cookies in play:
  - triibal_session_at:   the OAuth access token
                         (HttpOnly, lifetime = token TTL)
  - triibal_session_rt:   the OAuth refresh token
  ...
  - triibal_session_pkce: short-lived PKCE state + CSRF nonce + provider
...
https://datatracker.ietf.org/doc/html/draft-west-cookie-prefixes):
  ...
  * Gated HTTPS, direct deploy (Path=/) — ``__Host-`` prefix. Binds the
    ...
    guarantee.
  * Gated HTTPS, behind a reverse-proxy prefix (Path=/triibal) —
    ``__Secure-`` prefix. ``__Host-`` is disallowed when Path != "/";
    ``__Secure-`` keeps the Secure-required hardening without the
    Path constraint, and the explicit ``Path=/triibal`` covers
...
setter wrote ``__Secure-triibal_session_at`` would never find the value.
...
SESSION_AT_COOKIE = "triibal_session_at"
SESSION_RT_COOKIE = "triibal_session_rt"
PKCE_COOKIE = "triibal_session_pkce"
...
def _cookie_path(prefix: str) -> str:
    ...
    Under ``X-Forwarded-Prefix: /triibal`` we want ``Path=/triibal`` so:
...
def set_session_cookies(
    ...
    *,
    ...
    ``prefix`` is the normalised X-Forwarded-Prefix value (e.g. ``/triibal``)
