...
Profile-aware location: ``$TRIIBAL_HOME/logs/dashboard-auth.log``.
...
from ``triibal_constants`` or other triibal_cli modules — so it can be
...
def _resolve_log_path() -> Path:
    """``$TRIIBAL_HOME/logs/dashboard-auth.log`` with the standard fallback.

    Mirrors ``triibal_constants.get_triibal_home`` semantics: env var wins,
    else ``~/.triibal``. A local copy avoids an import cycle with the
    middleware which lives below ``triibal_cli``.
    """
    home = os.environ.get("TRIIBAL_HOME") or str(Path.home() / ".triibal")
