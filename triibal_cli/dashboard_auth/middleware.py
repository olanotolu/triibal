...
from triibal_cli.dashboard_auth.base import ProviderError
from triibal_cli.dashboard_auth.cookies import read_session_cookies
...
def _unauth_response(request: Request, *, reason: str) -> Response:
    ...
    Under a reverse proxy with ``X-Forwarded-Prefix: /triibal``, the
    ``login_url`` is prefixed (``/triibal/login?next=...``) so the
...
async def gated_auth_middleware(
    ...
    if session is None:
        ...
        from triibal_cli.dashboard_auth.cookies import clear_session_cookies
        from triibal_cli.dashboard_auth.prefix import prefix_from_request
