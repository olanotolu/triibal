...
Used by ``triibal profile describe <name> --auto`` and the dashboard's
...
Design notes
------------
- Mirrors the shape of ``triibal_cli/kanban_specify.py``: lazy aux
...
_SYSTEM_PROMPT = """You are a profile-describer for the Triibal Agent kanban board.
...
Rules:
  ...
  - Never write "Triibal Agent profile" or other meta-narration.
    ...
    *,
    ...
    try:
        if canon == "default":
            ...
            profile_dir = Path(get_triibal_home())
        else:
