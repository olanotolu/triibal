"""
Triibal Agent Uninstaller.

Provides options for:
- Full uninstall: Remove everything including configs and data
- Keep data: Remove code but keep ~/.triibal/ (configs, sessions, logs)
"""
...
def remove_path_from_shell_configs():
    """Remove Triibal PATH entries from shell configuration files."""
    ...
    for config_path in configs:
        try:
            ...
            # Remove lines containing triibal-agent or triibal PATH entries
            new_lines = []
            skip_next = False
            
            for line in content.split('\n'):
                # Skip the "# Triibal Agent" comment and following line
                if '# Triibal Agent' in line or '# triibal-agent' in line:
                    skip_next = True
                    continue
                if skip_next and ('triibal' in line.lower() and 'PATH' in line):
                    skip_next = False
                    continue
                skip_next = False
                
                # Remove any PATH line containing triibal
                if 'triibal' in line.lower() and ('PATH=' in line or 'path=' in line.lower()):
                    continue
...
def remove_wrapper_script():
    """Remove the triibal wrapper script if it exists."""
    wrapper_paths = [
        Path.home() / ".local" / "bin" / "triibal",
        Path("/usr/local/bin/triibal"),
    ]
    ...
    for wrapper in wrapper_paths:
        if wrapper.exists():
            try:
                # Check if it's our wrapper (contains triibal_cli reference)
                content = wrapper.read_text()
                if 'triibal_cli' in content or 'triibal-agent' in content:
                    wrapper.unlink()
...
def uninstall_gateway_service():
    ...
    - All platforms: standalone ``triibal gateway run`` processes
    ...
    try:
        from triibal_cli.gateway import kill_gateway_processes, find_gateway_pids
    ...
    # 3. macOS: uninstall launchd plist
        ...
        try:
            from triibal_cli.gateway import get_launchd_plist_path
...
#
#   1. Sets User-scope env vars ``TRIIBAL_HOME`` and ``TRIIBAL_GIT_BASH_PATH``
...
#      like ``%LOCALAPPDATA%\triibal\git\cmd``, ``%LOCALAPPDATA%\triibal\git\bin``,
#      ``%LOCALAPPDATA%\triibal\git\usr\bin``, ``%LOCALAPPDATA%\triibal\node``.
#      Again not in any rc file — only accessible via the registry or the
#      .NET [Environment] API.
#   3. Downloads PortableGit to ``%LOCALAPPDATA%\triibal\git\`` and Node to
#      ``%LOCALAPPDATA%\triibal\node\`` as user-scoped, isolated copies.
#      These are ~200MB combined and serve no purpose after uninstall.
#   4. On the ``triibal dashboard`` + gateway paths, drops files into
#      ``%LOCALAPPDATA%\triibal\gateway-service\`` and sometimes
...
def _triibal_path_markers(triibal_home: Path) -> list[str]:
    """Path-entry substrings that identify Triibal-owned User-PATH entries."""
    root = str(triibal_home).rstrip("\\/")
    # Match on prefix so sub-entries (git\cmd, git\bin, git\usr\bin, node, etc.)
    # all get swept.  Also match the bare triibal-agent install dir.
    markers = [root + "\\triibal-agent", root + "\\git", root + "\\node", root + "\\venv"]
    # Also match if TRIIBAL_HOME was customised to somewhere else — find-and-nuke
    # any entry whose path component contains "triibal".  We don't want to catch
    # unrelated entries like "ctriibal-foo" or "ephermeral", so we look for
    # backslash-triibal as a word-ish boundary.
    return markers


def remove_path_from_windows_registry(triibal_home: Path) -> list[str]:
    """Strip Triibal-owned entries from User-scope PATH in the registry.
    ...
    try:
        with winreg.OpenKey(winreg.HKEY_CURRENT_USER, key_path, 0,
            ...
            markers = _triibal_path_markers(triibal_home)
            kept: list[str] = []
...
def remove_triibal_env_vars_windows() -> list[str]:
    """Delete TRIIBAL_HOME and TRIIBAL_GIT_BASH_PATH from User-scope env vars."""
    try:
        import winreg
    except ImportError:
        return []
    ...
    try:
        with winreg.OpenKey(winreg.HKEY_CURRENT_USER, "Environment", 0,
                            winreg.KEY_READ | winreg.KEY_WRITE) as key:
            for name in ("TRIIBAL_HOME", "TRIIBAL_GIT_BASH_PATH"):
                try:
...
def _is_default_triibal_home(triibal_home: Path) -> bool:
    """Return True when ``triibal_home`` points at the default (non-profile) root."""
    try:
        from triibal_constants import get_default_triibal_root
        return triibal_home.resolve() == get_default_triibal_root().resolve()
    except Exception:
        return False
...
def _uninstall_profile(profile) -> None:
    ...
    remove its alias wrapper, and wipe its TRIIBAL_HOME directory.

    We shell out to ``triibal -p <name> gateway stop|uninstall`` because
    service names, unit paths, and plist paths are all derived from the
    current TRIIBAL_HOME and can't be easily switched in-process.
    """
    import sys as _sys
    name = profile.name
    ...
    #    Use `python -m triibal_cli.main` so we don't depend on a `triibal`
    #    wrapper that may be half-removed mid-uninstall.
    triibal_invocation = [_sys.executable, "-m", "triibal_cli.main", "--profile", name]
    for subcmd in ("stop", "uninstall"):
        try:
            subprocess.run(
                triibal_invocation + ["gateway", subcmd],
                capture_output=True,
                text=True,
                timeout=60,
                check=False,
    ...
    # 3. Wipe the profile's TRIIBAL_HOME directory.
    try:
...
def run_uninstall(args):
    ...
    Options:
    - Full uninstall: removes code + ~/.triibal/ (configs, data, logs)
    - Keep data: removes code but keeps ~/.triibal/ for future reinstall
    """
    project_root = get_project_root()
    triibal_home = get_triibal_home()

    # Detect named profiles when uninstalling from the default root —
    # offer to clean them up too instead of leaving zombie TRIIBAL_HOMEs
    # and systemd units behind.
    is_default_profile = _is_default_triibal_home(triibal_home)
    named_profiles = _discover_named_profiles() if is_default_profile else []

    print()
    print(color("┌─────────────────────────────────────────────────────────┐", Colors.MAGENTA, Colors.BOLD))
    print(color("│            ⚕ Triibal Agent Uninstaller                  │", Colors.MAGENTA, Colors.BOLD))
    print(color("└─────────────────────────────────────────────────────────┘", Colors.MAGENTA, Colors.BOLD))
    print()
    ...
    print(f"  Config:  {triibal_home / 'config.yaml'}")
    print(f"  Secrets: {triibal_home / '.env'}")
    print(f"  Data:    {triibal_home / 'cron/'}, {triibal_home / 'sessions/'}, {triibal_home / 'logs/'}")
    print()
    ...
    # their alias wrappers, and wiping their TRIIBAL_HOME dirs. Otherwise
    ...
    print()
    if full_uninstall:
        print(color("⚠️  WARNING: This will permanently delete ALL Triibal data!", Colors.RED, Colors.BOLD))
        ...
        print("This will remove the Triibal code but keep your configuration and data.")
    ...
    if _is_windows():
        ...
        # Expand %LOCALAPPDATA% etc. in triibal_home so the marker matching is
        # against fully resolved paths — installer writes literal strings
        # like C:\Users\<u>\AppData\Local\triibal\git\cmd, not %LOCALAPPDATA%.
        removed_path_entries = remove_path_from_windows_registry(Path(os.path.expandvars(str(triibal_home))))
        if removed_path_entries:
            for entry in removed_path_entries:
                log_success(f"Removed from User PATH: {entry}")
        else:
            log_info("No Triibal-owned PATH entries in User environment")

        log_info("Removing TRIIBAL_HOME / TRIIBAL_GIT_BASH_PATH User env vars...")
        removed_env = remove_triibal_env_vars_windows()
        if removed_env:
            for name in removed_env:
                log_success(f"Removed User env var: {name}")
        else:
            log_info("No Triibal-set User env vars to remove")
    
    # 3. Remove wrapper script
    log_info("Removing triibal command...")
    ...
    try:
        if project_root.exists():
            # If the install is inside ~/.triibal/, just remove the triibal-agent subdir
            if triibal_home in project_root.parents or project_root.parent == triibal_home:
    ...
    #     under TRIIBAL_HOME but they're install tooling, not config — safe to
    #     remove even in "keep data" mode.  If we're doing a full uninstall
    #     the step-5 rmtree(triibal_home) would sweep them anyway; calling
    ...
    if _is_windows():
        ...
        removed_artifacts = remove_portable_tooling_windows(triibal_home)
    ...
    # 5. Optionally remove ~/.triibal/ data directory (and named profiles)
    if full_uninstall:
        # 5a. Stop and remove each named profile's gateway service and
        #     alias wrapper. The profile TRIIBAL_HOME dirs live under
        ...
        try:
            if triibal_home.exists():
                shutil.rmtree(triibal_home)
                log_success(f"Removed {triibal_home}")
        except Exception as e:
            log_warn(f"Could not fully remove {triibal_home}: {e}")
            log_info("You may need to manually remove it")
    else:
        log_info(f"Keeping configuration and data in {triibal_home}")
    ...
    if not full_uninstall:
        ...
        print(f"  {triibal_home}/")
        print()
        print("To reinstall later with your existing settings:")
        if _is_windows():
            print(color("  iex (irm https://raw.githubusercontent.com/Triibal/triibal/main/scripts/install.ps1)", Colors.DIM))
        else:
            print(color("  curl -fsSL https://raw.githubusercontent.com/Triibal/triibal/main/scripts/install.sh | bash", Colors.DIM))
        print()
    ...
    print()
    print("Thank you for using Triibal Agent! ⚕")
    print()
