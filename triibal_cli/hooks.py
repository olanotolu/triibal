"""triibal hooks — inspect and manage shell-script hooks.

Usage::

    triibal hooks list
    triibal hooks test <event> [--for-tool X] [--payload-file F]
    triibal hooks revoke <command>
    triibal hooks doctor

Consent records live under ``~/.triibal/shell-hooks-allowlist.json`` and
hook definitions come from the ``hooks:`` block in ``~/.triibal/config.yaml``
...
def hooks_command(args) -> None:
    """Entry point for ``triibal hooks`` — dispatches to the requested action."""
    sub = getattr(args, "hooks_action", None)

    if not sub:
        print("Usage: triibal hooks {list|test|revoke|doctor}")
        print("Run 'triibal hooks --help' for details.")
        return
...
def _cmd_list(_args) -> None:
    ...
    if not specs:
        print("No shell hooks configured in ~/.triibal/config.yaml.")
        print("See `triibal hooks --help` or")
                ...
                if entry and entry.get("approved_at"):
                    ...
                    if mtime_now and mtime_at and mtime_now > mtime_at:
                        print(
                            ...
                            f"run `triibal hooks doctor` to re-validate"
                        )
        print()
...
# stdin a script sees under `triibal hooks test` and `triibal hooks doctor`
...
def _print_run_result(result: Dict[str, Any]) -> None:
    ...
    if parsed:
        print(f"      parsed (Triibal wire shape): {json.dumps(parsed)}")
    else:
...
def _doctor_one(spec, shell_hooks) -> int:
    ...
    if entry and entry.get("script_mtime_at_approval"):
        ...
        if mtime_now and mtime_at and mtime_now > mtime_at:
            ...
            print(f"      ⚠ script modified since approval "
                  ...
                  f"then `triibal hooks revoke` + re-approve to refresh")
    ...
    # is already allowlisted.  Otherwise `triibal hooks doctor` would execute
    ...
    if not entry:
        print("      ℹ skipped JSON smoke test — not allowlisted yet. "
              ...
              "then re-run `triibal hooks doctor`.")
