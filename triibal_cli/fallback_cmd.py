"""
triibal fallback — manage the fallback provider chain.
...
Subcommands:
  triibal fallback [list]   Show the current fallback chain (default when no subcommand)
  triibal fallback add      Pick provider + model via the same picker as `triibal model`,
                           then append the selection to the chain
  triibal fallback remove   Pick an entry to delete from the chain
  triibal fallback clear    Remove all fallback entries

Storage: ``fallback_providers`` in ``~/.triibal/config.yaml`` (top-level, list of
...
def _restore_auth_active_provider(value: Any) -> None:
    ...
    try:
        from triibal_cli.auth import _auth_store_lock, _load_auth_store, _save_auth_store
        ...
        # `triibal model` to fix it.  Don't fail the fallback add.
        pass
...
def cmd_fallback_list(args) -> None:  # noqa: ARG001
    ...
    if not chain:
        ...
        print()
        print("  Add one with:  triibal fallback add")
        print()
        return
    ...
    print("  Docs: https://triibal.dev/docs/user-guide/features/fallback-providers")
    print()
...
def cmd_fallback_add(args) -> None:
    """Launch the same picker as `triibal model`, then append the selection to the chain."""
    from triibal_cli.main import _require_tty, select_provider_and_model
    ...
    print("  `triibal model` — select the provider + model you want as a fallback.")
    print()
    ...
    print()
    print("  Run `triibal fallback list` to view, or `triibal fallback remove` to delete.")
...
def cmd_fallback(args) -> None:
    """Top-level dispatcher for ``triibal fallback [subcommand]``."""
