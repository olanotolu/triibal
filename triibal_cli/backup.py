"""
Backup and import commands for triibal CLI.

`triibal backup` creates a zip archive of the entire ~/.triibal/ directory
(excluding the triibal-agent repo and transient files).

`triibal import` restores from a backup zip, overlaying onto the current
TRIIBAL_HOME root.
"""
...
from triibal_constants import get_default_triibal_root, get_triibal_home, display_triibal_home
...
_EXCLUDED_DIRS = {
    "triibal-agent",     # the codebase repo — re-clone instead
...
def _should_exclude(rel_path: Path) -> bool:
    """Return True if *rel_path* (relative to triibal root) should be skipped."""
...
def _should_skip_backup_file(abs_path: Path, rel_path: Path, out_path: Path) -> bool:
    ...
    # write can copy data from outside TRIIBAL_HOME.
...
def run_backup(args) -> None:
    """Create a zip backup of the Triibal home directory."""
    triibal_root = get_default_triibal_root()

    if not triibal_root.is_dir():
        print(f"Error: Triibal home directory not found at {triibal_root}")
        sys.exit(1)
    ...
    if args.output:
        ...
        if out_path.is_dir():
            ...
            out_path = out_path / f"triibal-backup-{stamp}.zip"
    else:
        stamp = datetime.now().strftime("%Y-%m-%d-%H%M%S")
        out_path = Path.home() / f"triibal-backup-{stamp}.zip"
    ...
    # Collect files
    print(f"Scanning {display_triibal_home()} ...")
    files_to_add: list[tuple[Path, Path]] = []  # (absolute, relative)
    skipped_dirs = set()

    for dirpath, dirnames, filenames in os.walk(triibal_root, followlinks=False):
        dp = Path(dirpath)
        rel_dir = dp.relative_to(triibal_root)
        ...
        for fname in filenames:
            fpath = dp / fname
            rel = fpath.relative_to(triibal_root)
    ...
    print(f"\nRestore with: triibal import {out_path.name}")
...
def _validate_backup_zip(zf: zipfile.ZipFile) -> tuple[bool, str]:
    """Check that a zip looks like a Triibal backup.
    ...
    # Look for telltale files that a triibal home would have
    ...
    if not found:
        return False, (
            "zip does not appear to be a Triibal backup "
...
def _detect_prefix(zf: zipfile.ZipFile) -> str:
    ...
    Some tools zip as `.triibal/config.yaml` instead of `config.yaml`.
    ...
    if len(first_parts) == 1:
        ...
        # Only strip if it looks like a triibal dir name
        if prefix in {".triibal", "triibal"}:
            return prefix + "/"
...
def run_import(args) -> None:
    ...
    with zipfile.ZipFile(zip_path, "r") as zf:
        ...
        print(f"Target: {display_triibal_home()}")

        if prefix:
            print(f"Detected archive prefix: {prefix!r} (will be stripped)")

        # Check for existing installation
        has_config = (triibal_root / "config.yaml").exists()
        has_env = (triibal_root / ".env").exists()

        if (has_config or has_env) and not args.force:
            print()
            print("Warning: Target directory already has Triibal configuration.")
        ...
        for member in members:
            ...
            target = triibal_root / rel

            # Security: reject absolute paths and traversals
            try:
                target.resolve().relative_to(triibal_root.resolve())
            except ValueError:
        ...
        profiles_dir = triibal_root / "profiles"
        restored_profiles = []
        if profiles_dir.is_dir():
            try:
                ...
                # triibal_cli.profiles might not be available (fresh install)
                if any(profiles_dir.iterdir()):
                    print(f"\n  Profiles detected but aliases could not be created.")
                    print(f"  Run: triibal profile list  (after installing triibal)")

        # Guidance
        print()
        if not (triibal_root / "triibal-agent").is_dir():
            print("Note: The triibal-agent codebase was not included in the backup.")
            print("  If this is a fresh install, run: triibal update")

        if restored_profiles:
            gw_profiles = [n for n, _ in restored_profiles]
            print("\nTo re-enable gateway services for profiles:")
            for pname in gw_profiles:
                print(f"  triibal -p {pname} gateway install")

        print("Done. Your Triibal configuration has been restored.")


# ---------------------------------------------------------------------------
# Quick state snapshots (used by /snapshot slash command and triibal backup --quick)
# ---------------------------------------------------------------------------

# Critical state files to include in quick snapshots (relative to TRIIBAL_HOME).
...
# — `triibal update` snapshots this set before pulling so approved-user lists
...
def _quick_snapshot_root(triibal_home: Optional[Path] = None) -> Path:
    home = triibal_home or get_triibal_home()
    return home / _QUICK_SNAPSHOTS_DIR


def create_quick_snapshot(
    label: Optional[str] = None,
    triibal_home: Optional[Path] = None,
...
def list_quick_snapshots(
    ...
    root = _quick_snapshot_root(triibal_home)
    if not root.exists():
        return []
...
def prune_quick_snapshots(
    ...
    return _prune_quick_snapshots(_quick_snapshot_root(triibal_home), keep=keep)


def run_quick_backup(args) -> None:
    """CLI entry point for triibal backup --quick."""
    label = getattr(args, "label", None)
    snap_id = create_quick_snapshot(label=label)
    if snap_id:
        print(f"State snapshot created: {snap_id}")
        snaps = list_quick_snapshots()
        print(f"  {len(snaps)} snapshot(s) stored in {display_triibal_home()}/state-snapshots/")
...
def _write_full_zip_backup(out_path: Path, triibal_root: Path) -> Optional[Path]:
    """Write a full zip snapshot of ``triibal_root`` to ``out_path``.
...
def _pre_update_backup_dir(triibal_home: Optional[Path] = None) -> Path:
    home = triibal_home or get_triibal_home()
...
def create_pre_update_backup(
    ...
    """Create a full zip backup of TRIIBAL_HOME under ``backups/``.

    Mirrors :func:`run_backup` (same exclusion rules, same SQLite safe-copy)
    but writes to ``<TRIIBAL_HOME>/backups/pre-update-<timestamp>.zip`` and
    auto-prunes old pre-update backups.

    Returns the path to the created zip, or ``None`` if no files were
    found or the backup could not be created.  Never raises — the caller
    (``triibal update``) should continue even if the backup fails.
    """
    triibal_root = triibal_home or get_default_triibal_root()
    if not triibal_root.is_dir():
        return None

    backup_dir = _pre_update_backup_dir(triibal_root)
    try:
    ...
    result = _write_full_zip_backup(out_path, triibal_root)
    if result is None:
        return None
...
# ---------------------------------------------------------------------------
# Pre-migration auto-backup (used by `triibal claw migrate`)
# ---------------------------------------------------------------------------
...
def create_pre_migration_backup(
    ...
    """Create a full zip backup of TRIIBAL_HOME under ``backups/`` before a
    ``triibal claw migrate`` apply.

    Shares implementation with :func:`create_pre_update_backup` via
    ``_write_full_zip_backup`` — same exclusions, same SQLite safe-copy,
    restorable with ``triibal import <archive>``.  Writes to
    ``<TRIIBAL_HOME>/backups/pre-migration-<timestamp>.zip`` and auto-prunes
    ...
    # Reuses the shared backups/ directory so `triibal import` and the
