"""
Interactive setup wizard for Triibal Agent.
...
from triibal_cli.nous_subscription import get_nous_subscription_features
...
_DOCS_BASE = "https://triibal.dev/docs"
...
# display_triibal_home imported lazily at call sites (stale-module safety during triibal update)
...
from triibal_cli.cli_output import (  # noqa: E402
    print_error,
    print_info,
    print_success,
    print_warning,
)
from triibal_cli.secret_prompt import masked_secret_prompt  # noqa: E402
...
def print_noninteractive_setup_guidance(reason: str | None = None) -> None:
    ...
    print()
    print(color("⚕ Triibal Setup — Non-interactive mode", Colors.CYAN, Colors.BOLD))
    print()
    if reason:
        print_info(reason)
    print_info("The interactive wizard cannot be used here.")
    print()
    print_info("Configure Triibal using environment variables or config commands:")
    print_info("  triibal config set model.provider custom")
    print_info("  triibal config set model.base_url http://localhost:8080/v1")
    print_info("  triibal config set model.default your-model-name")
    print()
    print_info("Or set OPENROUTER_API_KEY / OPENAI_API_KEY in your environment.")
    print_info("Run 'triibal setup' in an interactive terminal to use the full wizard.")
    print()
...
def _curses_prompt_choice(question: str, choices: list, default: int = 0, description: str | None = None) -> int:
    ...
    from triibal_cli.curses_ui import curses_radiolist
...
def prompt_checklist(title: str, items: list, pre_selected: list = None) -> list:
    ...
    from triibal_cli.curses_ui import curses_checklist
...
def _prompt_api_key(var: dict):
    ...
    if value:
        ...
        print_warning("  Skipped (configure later with 'triibal setup')")


def _print_setup_summary(config: dict, triibal_home):
    ...
    if _vision_backends:
        ...
        tool_status.append(("Vision (image analysis)", False, "run 'triibal setup' to configure"))
    ...
    # Video generation — opt-in via `triibal tools` → Video Generation.
    # Only show the row when a plugin reports available so we don't badger
    # users who don't care about video gen with a "missing" status line.
    try:
        from agent.video_gen_registry import list_providers as _list_video_providers
        from triibal_cli.plugins import _ensure_plugins_discovered as _ensure_plugins
        _ensure_plugins()
        _video_backend = None
    ...
    if subscription_features.tts.managed_by_nous:
        ...
        if neutts_ok:
            ...
            tool_status.append(("Text-to-Speech (NeuTTS — not installed)", False, "run 'triibal setup tts'"))
        ...
        if kittentts_ok:
            ...
            tool_status.append(("Text-to-Speech (KittenTTS — not installed)", False, "run 'triibal setup tts'"))
    else:
    ...
    if subscription_features.modal.managed_by_nous:
        ...
        if subscription_features.modal.direct_override:
            ...
            tool_status.append(("Modal Execution", False, "run 'triibal setup terminal'"))
    ...
    # Spotify (OAuth via triibal auth spotify — check auth.json, not env vars)
    try:
        from triibal_cli.auth import get_provider_auth_state
    ...
    if disabled_tools:
        print_warning(
            "Some tools are disabled. Run 'triibal setup tools' to configure them,"
        )
    ...
    print(
        f"   {color('Data:', Colors.YELLOW)}      {triibal_home}/cron/, sessions/, logs/"
    )
    print()
    ...
    print()
    print(f"   {color('triibal setup', Colors.GREEN)}          Re-run the full wizard")
    print(f"   {color('triibal setup model', Colors.GREEN)}    Change model/provider")
    print(f"   {color('triibal setup terminal', Colors.GREEN)} Change terminal backend")
    print(f"   {color('triibal setup gateway', Colors.GREEN)}  Configure messaging")
    print(f"   {color('triibal setup tools', Colors.GREEN)}    Configure tool providers")
    print()
    print(f"   {color('triibal config', Colors.GREEN)}         View current settings")
    print(
        f"   {color('triibal config edit', Colors.GREEN)}    Open config in your editor"
    )
    print(f"   {color('triibal config set <key> <value>', Colors.GREEN)}")
    ...
    print()
    print(f"   {color('triibal', Colors.GREEN)}              Start chatting")
    print(f"   {color('triibal gateway', Colors.GREEN)}      Start messaging gateway")
    print(f"   {color('triibal doctor', Colors.GREEN)}       Check for issues")
    print()
...
# between `triibal tools` and `triibal setup tools`).
...
def setup_model_provider(config: dict, *, quick: bool = False):
    ...
    Delegates to ``cmd_model()`` (the same flow used by ``triibal model``)
    ...
    provider added to ``triibal model`` is automatically available here.
    ...
    # Delegate to the shared triibal model flow — handles provider picker,
    # credential prompting, model selection, and config persistence.
    from triibal_cli.main import select_provider_and_model
    try:
        ...
        print_info("You can try again later with: triibal model")
    ...
    if not quick and _supports_same_provider_pool_setup(selected_provider):
        try:
            ...
            from triibal_cli.auth_commands import auth_add_command
            ...
            print_info(
                "Triibal can keep multiple credentials for one provider and rotate between"
            )
            print_info(
    ...
    if _vision_needs_setup:
        ...
        print_info("doesn't currently provide one Triibal can auto-use for vision,")
        ...
        if _vision_idx == 0:  # OpenRouter
            ...
            print_info("Skipped — add later with 'triibal setup' or configure AUXILIARY_VISION_* settings")
...
def _xai_oauth_logged_in_for_setup() -> bool:
    ...
    through ``triibal model`` -> xAI Grok OAuth (SuperGrok / Premium+).
    """
    try:
        from triibal_cli.auth import get_xai_oauth_auth_status
...
def _setup_tts_provider(config: dict):
    ...
    if selected == "nous-openai":
        ...
        if get_env_value("VOICE_TOOLS_OPENAI_KEY") or get_env_value("OPENAI_API_KEY"):
            print_warning(
                "Direct OpenAI credentials are still configured and may take precedence until removed from ~/.triibal/.env."
            )
    ...
    if selected == "neutts":
        ...
        # via the Triibal auth store) > existing XAI_API_KEY > prompt the user.
            ...
            if choice_idx == 0:
                ...
                if api_key:
                    ...
                    print_warning(
                        ...
                        f"via triibal setup model or {_dhh()}/.env to use xAI TTS. "
                        "Falling back to Edge TTS."
                    )
                    selected = "edge"
            else:
...
def setup_tts(config: dict):
    """Standalone TTS setup (for 'triibal setup tts')."""
    _setup_tts_provider(config)
...
def setup_terminal_backend(config: dict):
    ...
    print_info("Choose where Triibal runs shell commands and code.")
...
def _apply_default_agent_settings(config: dict):
    ...
    # bridges it into TRIIBAL_MAX_ITERATIONS at startup. We no longer write
    ...
    remove_env_value("TRIIBAL_MAX_ITERATIONS")
    ...
    print_info("  Run `triibal setup agent` later to customize.")
...
def setup_agent_settings(config: dict):
    ...
    try:
        ...
        if max_iter > 0:
            ...
            # TRIIBAL_MAX_ITERATIONS from agent.max_turns at startup.
...
def _setup_telegram():
    ...
    print()
    print_info("📬 Home Channel: where Triibal delivers cron job results,")
...
def _setup_slack():
    ...
    if existing:
        ...
        if not prompt_yes_no("Reconfigure Slack?", False):
            ...
            if prompt_yes_no(
                ...
                "list? (recommended after `triibal update`)",
                True,
            ):
    ...
    print()
    print_info("   Full guide: https://triibal.dev/docs/user-guide/messaging/slack/")
    print()
...
def _write_slack_manifest_and_instruct():
    """Generate the Slack manifest, write it under TRIIBAL_HOME, and print
    ...
    """
    try:
        from triibal_cli.slack_cli import _build_full_manifest
        from triibal_constants import get_triibal_home

        manifest = _build_full_manifest(
            bot_name="Triibal",
            bot_description="Your Triibal agent on Slack",
        )
        target = Path(get_triibal_home()) / "slack-manifest.json"
        ...
        print_info(
            "   Re-run `triibal slack manifest --write` anytime to refresh after "
            "Triibal adds new commands."
        )
...
def _setup_matrix():
    ...
    if token or get_env_value("MATRIX_PASSWORD"):
        ...
        print()
        print_info("📬 Home Room: where Triibal delivers cron job results and notifications.")
...
def _setup_bluebubbles():
    ...
    print_info("Connects Triibal to iMessage via BlueBubbles — a free, open-source")
...
def _setup_qqbot():
    ...
    from triibal_cli.gateway import _setup_qqbot as _gateway_setup_qqbot
...
def _setup_webhooks():
    ...
    print()
    print_info("   Full guide: https://triibal.dev/docs/user-guide/messaging/webhooks/")
    print()
    ...
    print_info("   https://triibal.dev/docs/user-guide/messaging/webhooks/#configuring-routes")
    print()
    print_info("   Open config in your editor:  triibal config edit")
    print_info("   Open config in your editor:  triibal config edit")


def setup_gateway(config: dict):
    """Configure messaging platform integrations."""
    from triibal_cli.gateway import _all_platforms, _platform_status, _configure_platform

    print_header("Messaging Platforms")
    print_info("Connect to messaging platforms to chat with Triibal from anywhere.")
    ...
    if not selected:
        print_info("No platforms selected. Run 'triibal setup gateway' later to configure.")
        return
    ...
    if any_messaging:
        ...
        if missing_home:
            ...
            for plat in missing_home:
                print_info(
                    f"     triibal config set {plat.upper()}_HOME_CHANNEL <channel_id>"
                )
        ...
        if supports_systemd and has_legacy_triibal_units():
            print_legacy_unit_warning()
            print()
        ...
        if service_running:
            ...
            if prompt_yes_no(
                ...
                print_info("  Or run in foreground:  triibal gateway")
        else:
            from triibal_constants import is_container
            if is_container():
                print_info("Start the gateway to bring your bots online:")
                print_info("   triibal gateway run          # Run as container main process")
                print_info("")
                ...
                print_info("Start the gateway to bring your bots online:")
                print_info("   triibal gateway              # Run in foreground")
...
def setup_tools(config: dict, first_install: bool = False):
    ...
    Both `triibal setup tools` and `triibal tools` use the same flow:
...
def _model_section_has_credentials(config: dict) -> bool:
    ...
    Sources of truth:
      * ``PROVIDER_REGISTRY`` in ``triibal_cli.auth`` — lists every supported
    ...
    """
    try:
        from triibal_cli.auth import get_active_provider
...
def _get_section_config_summary(config: dict, section_key: str) -> Optional[str]:
    ...
    ``get_env_value`` is the module-level import from triibal_cli.config
    ...
    if section_key == "model":
        ...
        from triibal_cli.gateway import _all_platforms, _platform_status
...
_OPENCLAW_SCRIPT = (
    ...
    / "openclaw-migration"
    / "scripts"
    / "openclaw_to_triibal.py"
)


def _load_openclaw_migration_module():
    """Load the openclaw_to_triibal migration script as a module.
    ...
    spec = importlib.util.spec_from_file_location(
        "openclaw_to_triibal", _OPENCLAW_SCRIPT
    )
...
# Config values may have different semantics between OpenClaw and Triibal.
# Instruction/context files (.md) can contain incompatible setup procedures.
_HIGH_IMPACT_KIND_KEYWORDS = {
    "gateway": "⚠ Gateway/messaging — this will configure Triibal to use your OpenClaw messaging channels",
    "telegram": "⚠ Telegram — this will point Triibal at your OpenClaw Telegram bot",
    "slack": "⚠ Slack — this will point Triibal at your OpenClaw Slack workspace",
    "discord": "⚠ Discord — this will point Triibal at your OpenClaw Discord bot",
    "whatsapp": "⚠ WhatsApp — this will point Triibal at your OpenClaw WhatsApp connection",
    "config": "⚠ Config values — OpenClaw settings may not map 1:1 to Triibal equivalents",
...
def _print_migration_preview(report: dict):
    ...
    if conflict_items:
        print(color("  Would overwrite (conflicts with existing Triibal config):", Colors.YELLOW))
    ...
    if warnings_shown:
        ...
        print()
        print(color("  Note: OpenClaw config values may have different semantics in Triibal.", Colors.YELLOW))
        print(color("  For example, OpenClaw's tool_call_execution: \"auto\" ≠ Triibal's yolo mode.", Colors.YELLOW))
        print(color("  Instruction files (.md) from OpenClaw may contain incompatible procedures.", Colors.YELLOW))
        print()


def _offer_openclaw_migration(triibal_home: Path) -> bool:
    ...
    print_info("Triibal can preview what would be imported before making any changes.")
    print()

    if not prompt_yes_no("Would you like to see what can be imported?", default=True):
        print_info(
            "Skipping migration. You can run it later with: triibal claw migrate --dry-run"
        )
        return False
    ...
    try:
        ...
        dry_migrator = mod.Migrator(
            ...
            target_root=triibal_home.resolve(),
    ...
    if not prompt_yes_no("Proceed with migration?", default=False):
        print_info(
            "Migration cancelled. You can run it later with: triibal claw migrate"
        )
        print_info(
    ...
    # Execute the migration — overwrite=False so existing Triibal configs are
    ...
    try:
        migrator = mod.Migrator(
            ...
            execute=True,
            workspace_target=None,
            overwrite=False,  # preserve existing Triibal config
            migrate_secrets=True,
            output_dir=None,
            selected_options=selected,
            preset_name="full",
    ...
    if conflicts:
        print_info(f"Skipped {conflicts} item(s) that already exist in Triibal (use triibal claw migrate --overwrite to force).")
    if skipped:
...
def _run_portal_one_shot(config: dict) -> None:
    ...
    Wired into ``triibal setup --portal``. Does NOT prompt for anything
    besides what the underlying OAuth + Tool Gateway prompts already need.
    Designed to be shareable as a single command (``triibal setup --portal``)
    that gets a brand-new user from zero to a fully working Triibal session
    ...
    from triibal_cli.auth import get_nous_auth_status
    ...
    print(color("│     ⚕ Triibal Setup — Nous Portal (one-shot)             │", Colors.MAGENTA))
    print(
        color(
            "└─────────────────────────────────────────────────────────┘",
            Colors.MAGENTA,
    ...
    # runs `triibal setup --portal` after a successful first run).
    ...
    if already_logged_in:
        ...
        # identical to `triibal auth add nous --type oauth`. SimpleNamespace
        ...
        try:
            ...
            print_info("  You can retry later with `triibal auth add nous --type oauth`.")
            return
        except (KeyboardInterrupt, EOFError):
            print()
    ...
    # from `triibal model` after picking Nous.
    print()
    try:
    ...
    print_info("  Run `triibal portal status` to inspect routing.")
    print_info("  Run `triibal` to start chatting.")
...
def run_setup_wizard(args):
    ...
    Supports full, quick, and section-specific setup:
      triibal setup           — full or quick (auto-detected)
      triibal setup model     — just model/provider
      triibal setup tts       — just text-to-speech
      triibal setup terminal  — just terminal backend
      triibal setup gateway   — just messaging platforms
      triibal setup tools     — just tool configuration
      triibal setup agent     — just agent settings
    """
    from triibal_cli.config import is_managed, managed_error
    if is_managed():
    ...
    if section:
        for key, label, func in SETUP_SECTIONS:
            if key == section:
                ...
                print(color(f"│     ⚕ Triibal Setup — {label:<34s} │", Colors.MAGENTA))
                print(
                    color(
                        "└─────────────────────────────────────────────────────────┘",
                        Colors.MAGENTA,
    ...
    print(
        color(
            "│             ⚕ Triibal Agent Setup Wizard                │", Colors.MAGENTA
        )
    )
    print(
        color(
    ...
    print(
        color(
            "│  Let's configure your Triibal Agent installation.       │", Colors.MAGENTA
        )
    )
    print(
        color(
    ...
    if is_existing:
        ...
        if quick_requested:
            _run_quick_setup(config, triibal_home)
            return

        print()
        print_header("Reconfigure")
        print_success("You already have Triibal configured.")
        print_info("Running the full wizard — each prompt shows your current value.")
        print_info("Press Enter to keep it, or type a new value to change it.")
        print_info("")
        print_info("Tip: jump straight to a section with 'triibal setup model|terminal|")
        ...
        migration_ran = _offer_openclaw_migration(triibal_home)
        if migration_ran:
            config = load_config()

        setup_mode = prompt_choice("How would you like to set up Triibal?", [
            "Quick setup — provider, model & messaging (recommended)",
            "Full setup — configure everything",
        ], 0)

        if setup_mode == 0:
            _run_first_time_quick_setup(config, triibal_home, is_existing)
            return
    ...
    print_info(f"Data folder:  {triibal_home}")
    print_info(f"Install dir:  {PROJECT_ROOT}")
    print()
    print_info("You can edit these files directly or use 'triibal config edit'")
    ...
    _print_setup_summary(config, triibal_home)


def _run_first_time_quick_setup(config: dict, triibal_home, is_existing: bool):
    ...
    the user can customize later via ``triibal setup <section>``.
    """
    ...
    gateway_choice = prompt_choice(
        ...
        [
            ...
            "Skip — set up later with 'triibal setup gateway'",
        ],
        0,
    )
    ...
    print()
    print_info("  Configure all settings:    triibal setup")
    if gateway_choice != 0:
        print_info("  Connect Telegram/Discord:  triibal setup gateway")
    print()

    _print_setup_summary(config, triibal_home)


def _run_quick_setup(config: dict, triibal_home):
    ...
    if not has_anything_missing:
        ...
        print()
        print_info("Run 'triibal setup' and choose 'Full Setup' to reconfigure,")
    ...
    if missing_messaging:
        print()
        print_header("Messaging Platforms")
        print_info("Connect Triibal to messaging apps to chat from anywhere.")
        print_info("You can configure these later with 'triibal setup gateway'.")
