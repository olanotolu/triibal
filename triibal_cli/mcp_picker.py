"""MCP picker — interactive `triibal mcp picker` (also the default `triibal mcp`).

Lists every catalog entry plus any custom MCP servers the user has added via
``triibal mcp add``, lets them pick one, and routes to install / enable /
disable / uninstall / configure-tools flows.

Mirrors the `triibal plugin` picker UX: arrow keys to navigate, ENTER on a row
...
from triibal_cli.cli_output import prompt_yes_no
from triibal_cli.curses_ui import curses_single_select
from triibal_cli.mcp_catalog import (
    CatalogEntry,
    CatalogError,
    catalog_diagnostics,
    install_entry,
...
def _enable_disable(name: str, *, enable: bool) -> None:
    ...
    print(color(
        ...
        "Start a new Triibal session for changes to take effect.",
        Colors.GREEN,
    ))
...
def _configure_tools(name: str) -> None:
    ...
    """
    import argparse
    from triibal_cli.mcp_config import cmd_mcp_configure
...
def _print_rows_text(rows: List[_Row]) -> None:
    ...
    as the default output of `triibal mcp catalog`."""
    if not rows:
        print()
    ...
    print()
    print(color(
        "  Install: triibal mcp install <name>    Picker: triibal mcp",
        Colors.DIM,
    ))

    # Surface manifest-version warnings so users know when their Triibal is
    ...
    if future:
        ...
        for name, _, msg in future:
            print(color(
                f"  ⚠ '{name}' requires a newer Triibal — run `triibal update` "
...
def show_catalog() -> None:
    """`triibal mcp catalog` — print the curated list + custom servers, no interaction."""
    _print_rows_text(_build_rows())


def run_picker() -> None:
    """`triibal mcp picker` (and default `triibal mcp`) — interactive selector.
...
def install_by_name(identifier: str) -> int:
    """`triibal mcp install <name>` — non-interactive entry-point.
    ...
    if entry is None:
        print(color(
            ...
            "Run `triibal mcp catalog` to see available entries.",
            Colors.RED,
        ))
        return 1
    try:
