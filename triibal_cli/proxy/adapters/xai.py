...
# xAI's public API is OpenAI-compatible for the endpoints Triibal commonly
# uses. The Responses endpoint is included because Triibal' native xAI runtime
...
class XAIGrokAdapter(UpstreamAdapter):
    """Proxy upstream for xAI Grok via Triibal-managed OAuth credentials."""

    auth_hint = "triibal auth add xai-oauth --type oauth"
        ...
        with self._lock:
            ...
            if pool is None or not pool.has_credentials():
                raise RuntimeError(
                    ...
                    "`triibal auth add xai-oauth --type oauth` first."
                )
            ...
            if entry is None:
                raise RuntimeError(
                    ...
                    "`triibal auth reset xai-oauth` or re-authenticate with "
                    "`triibal auth add xai-oauth --type oauth`."
                )
    ...
    def _credential_from_entry(self, entry: PooledCredential) -> UpstreamCredential:
        ...
        if not bearer:
            raise RuntimeError(
                ...
                "Re-authenticate with `triibal auth add xai-oauth --type oauth`."
            )
