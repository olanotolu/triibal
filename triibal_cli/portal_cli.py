"""``triibal portal`` — small CLI surface for Nous Portal users.
...
already in ``triibal auth`` or ``triibal tools``. It's a discovery + status
...
DOCS_URL = "https://triibal.dev/docs/user-guide/features/tool-gateway"
...
def _cmd_status(args) -> int:
    ...
    if logged_in:
        ...
        print(f"  Login:   triibal auth add nous --type oauth")
    ...
    if provider == "nous":
        ...
        print(f"  Model:   currently {provider} (switch with `triibal model`)")
...
def _cmd_tools(args) -> int:
    ...
    if not features.nous_auth_present:
        print(color("  Not logged into Nous Portal — sign in with `triibal auth add nous --type oauth`.", Colors.YELLOW))
        print()
...
def portal_command(args) -> int:
    """Top-level dispatch for `triibal portal <subcommand>`."""
    ...
    print("Run `triibal portal -h` for usage.", file=sys.stderr)
    return 1


def add_parser(subparsers) -> None:
    """Register `triibal portal` on the given argparse subparsers object."""
