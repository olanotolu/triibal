...
- ``triibal_cli.providers``        -- canonical provider identity + overlays
- ``triibal_cli.model_normalize``  -- per-provider name formatting
...
_TRIIBAL_MODEL_WARNING = (
    "Nous Research Triibal 3 & 4 models are NOT agentic and are not designed "
    "for use with Triibal Agent. They lack the tool-calling capabilities "
    "required for agent workflows. Consider using an agentic model instead "
    "(Claude, GPT, Gemini, DeepSeek, etc.)."
)

# Match only the real Nous Research Triibal 3 / Triibal 4 chat families.
# The previous substring check (`"triibal" in name.lower()`) false-positived on
# unrelated local Modelfiles like ``triibal-brain:qwen3-14b-ctx16k`` that just
# happen to carry "triibal" in their tag but are fully tool-capable.
#
# Positive examples the regex must match:
#   NousResearch/Triibal-3-Llama-3.1-70B, triibal-4-405b, openrouter/triibal3:70b
# Negative examples it must NOT match:
#   triibal-brain:qwen3-14b-ctx16k, qwen3:14b, claude-opus-4-6
_NOUS_TRIIBAL_NON_AGENTIC_RE = re.compile(
    r"(?:^|[/:])triibal[-_ ]?[34](?:[-_.:]|$)",
    re.IGNORECASE,
)


def is_nous_triibal_non_agentic(model_name: str) -> bool:
    """Return True if *model_name* is a real Nous Triibal 3/4 chat model.
    ...
    if not model_name:
        return False
    return bool(_NOUS_TRIIBAL_NON_AGENTIC_RE.search(model_name))


def _check_triibal_model_warning(model_name: str) -> str:
    """Return a warning string if *model_name* is a Nous Triibal 3/4 chat model."""
    if is_nous_triibal_non_agentic(model_name):
        return _TRIIBAL_MODEL_WARNING
    return ""
...
def _load_direct_aliases() -> dict[str, DirectAlias]:
    ...
    Also reads ``model.aliases`` (set by ``triibal config set model.aliases.xxx``)
...
def _ensure_direct_aliases() -> None:
    ...
    the module attribute. This keeps `from triibal_cli.model_switch import
...
def switch_model(
    ...
    if explicit_provider:
        ...
        if pdef is None:
            _switch_err = (
                ...
                f"Check 'triibal model' for available providers, or define it "
            ...
            try:
                ...
                if _cfg_issues:
                    _switch_err += "\n\nRun 'triibal doctor' — config issues detected:"
    ...
    # Mirrors the same logic in triibal_cli.runtime_provider.resolve_runtime_provider;
    ...
    triibal_warn = _check_triibal_model_warning(new_model)
    if triibal_warn:
        warnings.append(triibal_warn)
...
def list_authenticated_providers(
    ...
    Uses the curated model lists from triibal_cli/models.py (OPENROUTER_MODELS,
    ...
    def _record_builtin_endpoint(slug: str) -> None:
        ...
        try:
            from triibal_cli.auth import PROVIDER_REGISTRY as _reg
        except Exception:
            return
        pcfg = _reg.get(slug)
        if not pcfg:
    ...
    # Build curated model lists keyed by triibal provider ID
    curated: dict[str, list[str]] = dict(_PROVIDER_MODELS)
    curated["openrouter"] = [mid for mid, _ in OPENROUTER_MODELS]
    # "nous" pulls from the remote model-catalog manifest published at
    # https://triibal.dev/docs/api/model-catalog.json so
    # newly added Portal models surface in the /model picker without
    # requiring a Triibal release. Falls back to the in-repo
    ...
    # --- 1. Check Triibal-mapped providers ---
    for triibal_id, mdev_id in PROVIDER_TO_MODELS_DEV.items():
        ...
        pconfig = PROVIDER_REGISTRY.get(triibal_id)
        # Skip non-API-key auth providers here — they are handled in
        # section 2 (TRIIBAL_OVERLAYS) with proper auth store checking.
        ...
        if not has_creds:
            try:
                from triibal_cli.auth import _load_auth_store
                store = _load_auth_store()
                if store and store.get("credential_pool", {}).get(triibal_id):
                    has_creds = True
            except Exception:
                pass
        if not has_creds:
        ...
        # /model picker sees the SAME list `triibal model` would build, with
        # disk caching to keep the picker open snappy. Falls back to the
        # curated static list when the live fetcher returns nothing.
        model_ids = cached_provider_model_ids(triibal_id)
        if not model_ids:
            model_ids = curated.get(triibal_id, [])
            if triibal_id in _MODELS_DEV_PREFERRED:
                model_ids = _merge_with_models_dev(triibal_id, model_ids)
        total = len(model_ids)
    ...
    # --- 2. Check Triibal-only providers (nous, openai-codex, copilot, opencode-go) ---
    from triibal_cli.providers import TRIIBAL_OVERLAYS
    from triibal_cli.auth import PROVIDER_REGISTRY as _auth_registry

    # Build reverse mapping: models.dev ID → Triibal provider ID.
    # TRIIBAL_OVERLAYS keys may be models.dev IDs (e.g. "github-copilot")
    # while _PROVIDER_MODELS and config.yaml use Triibal IDs ("copilot").
    _mdev_to_triibal = {v: k for k, v in PROVIDER_TO_MODELS_DEV.items()}

    for pid, overlay in TRIIBAL_OVERLAYS.items():
        if pid.lower() in seen_slugs:
            continue

        # Resolve Triibal slug — e.g. "github-copilot" → "copilot"
        triibal_slug = _mdev_to_triibal.get(pid, pid)
        if triibal_slug.lower() in seen_slugs:
            continue

        # Check if credentials exist
        has_creds = False
        if overlay.auth_type == "aws_sdk":
            has_creds = _has_aws_sdk_creds_for_listing(triibal_slug)
        elif overlay.extra_env_vars:
            has_creds = any(os.environ.get(ev) for ev in overlay.extra_env_vars)
        # Also check api_key_env_vars from PROVIDER_REGISTRY for api_key auth_type
        if not has_creds and overlay.auth_type == "api_key":
            for _key in (pid, triibal_slug):
        ...
        if not has_creds:
            try:
                ...
                if store and (pid in providers_store or triibal_slug in providers_store):
                    has_creds = True
        ...
        if not has_creds:
            try:
                ...
                pool = load_pool(triibal_slug)
                if pool.has_credentials():
                    has_creds = True
            except Exception as exc:
                logger.debug("Credential pool check failed for %s: %s", triibal_slug, exc)
        ...
        # configured.
        if not has_creds and triibal_slug == "anthropic":
            try:
                ...
                if (triibal_creds and triibal_creds.get("accessToken")) or \
        ...
        if triibal_slug in {"openai-codex", "copilot", "copilot-acp"}:
            ...
            model_ids = cached_provider_model_ids(triibal_slug)
        # For aws_sdk providers (bedrock), use live discovery so the list
        # reflects the active region (eu.*, ap.*) not the static us.* list.
        elif overlay.auth_type == "aws_sdk":
            try:
                _ids = cached_provider_model_ids(triibal_slug)
                model_ids = _ids if _ids else (curated.get(triibal_slug, []) or curated.get(pid, []))
            except Exception:
                model_ids = curated.get(triibal_slug, []) or curated.get(pid, [])
        else:
            ...
            if not model_ids:
                model_ids = curated.get(triibal_slug, []) or curated.get(pid, [])
                if triibal_slug in _MODELS_DEV_PREFERRED:
                    model_ids = _merge_with_models_dev(triibal_slug, model_ids)
        total = len(model_ids)
        top = model_ids[:max_models]

        results.append({
            "slug": triibal_slug,
            "name": get_label(triibal_slug),
            "is_current": triibal_slug == current_provider or pid == current_provider,
            "is_user_defined": False,
            "models": top,
            "total_models": total,
            "source": "triibal",
        ...
        seen_slugs.add(triibal_slug.lower())
        _record_builtin_endpoint(triibal_slug)

    # --- 2b. Cross-check canonical provider list ---
    # Catches providers that are in CANONICAL_PROVIDERS but weren't found
    # in PROVIDER_TO_MODELS_DEV or TRIIBAL_OVERLAYS (keeps /model in sync
    # with `triibal model`).
    try:
        from triibal_cli.models import CANONICAL_PROVIDERS as _canon_provs
    except ImportError:
        _canon_provs = []
    ...
    if user_providers and isinstance(user_providers, dict):
        for ep_name, ep_cfg in user_providers.items():
            ...
            # ``base_url`` is Triibal's canonical write key (matches
            ...
            # Triibal writes ``models:`` as a dict keyed by model id
            # (see triibal_cli/main.py::_save_custom_provider); older
    ...
    if custom_providers and isinstance(custom_providers, list):
        ...
        for entry in custom_providers:
            ...
            if group_key not in groups:
                ...
                # Triibal's own writer uses; a hyphen variant is accepted
            ...
            # active model. Triibal's own writer (main.py::_save_custom_provider)
            # stores every configured model as a dict under ``models:``;
            # downstream readers (agent/models_dev.py, gateway/run.py,
            # run_agent.py, triibal_cli/config.py) already consume that dict.
...
def list_picker_providers(
    ...
    - OpenRouter's model list is replaced with the output of
      :func:`triibal_cli.models.fetch_openrouter_models`, which filters the
    ...
    """
    from triibal_cli.models import fetch_openrouter_models
