...
def has_xai_credentials() -> bool:
    ...
    hot-paint paths (``triibal tools`` repaint, tool-registration scans,
    ...
    2. ``~/.triibal/auth.json`` has a non-empty ``providers.xai-oauth.tokens.access_token``
    ...
    try:
        ...
        auth_path = get_triibal_home() / "auth.json"
...
def get_env_value(name: str, default=None):
    """Read ``name`` from ``~/.triibal/.env`` first, then ``os.environ``.

    Wraps :func:`triibal_cli.config.get_env_value` so tests can patch
    ``tools.xai_http.get_env_value`` to inject dotenv-only secrets into the
    xAI credential resolver.
    """
    try:
        from triibal_cli.config import get_env_value as _triibal_get_env_value

        value = _triibal_get_env_value(name)
...
def triibal_xai_user_agent() -> str:
    """Return a stable Triibal-specific User-Agent for xAI HTTP calls."""
    try:
        from triibal_cli import __version__
    except Exception:
        __version__ = "unknown"
    return f"Triibal-Agent/{__version__}"


def resolve_xai_http_credentials(*, force_refresh: bool = False) -> Dict[str, str]:
    """Resolve bearer credentials for direct xAI HTTP endpoints.

    Prefers Triibal-managed xAI OAuth credentials when available, then falls back
    to ``XAI_API_KEY`` resolved via ``triibal_cli.config.get_env_value`` so keys
    stored in ``~/.triibal/.env`` (the standard Triibal location) are honored —
