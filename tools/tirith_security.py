...
it is automatically downloaded from GitHub releases to $TRIIBAL_HOME/bin/tirith.
...
def _get_triibal_home() -> str:
    """Return the Triibal home directory, respecting TRIIBAL_HOME env var."""
    return str(get_triibal_home())


def _failure_marker_path() -> str:
    """Return the path to the install-failure marker file."""
    return os.path.join(_get_triibal_home(), ".tirith-install-failed")
...
def _triibal_bin_dir() -> str:
    """Return $TRIIBAL_HOME/bin, creating it if needed."""
    d = os.path.join(_get_triibal_home(), "bin")
...
def _install_tirith(*, log_failures: bool = True) -> tuple[str | None, str]:
    """Download and install tirith to $TRIIBAL_HOME/bin/tirith.
    ...
    try:
        ...
        dest = os.path.join(_triibal_bin_dir(), "tirith")
        try:
...
def _resolve_tirith_path(configured_path: str) -> str:
    ...
    2. $TRIIBAL_HOME/bin/tirith (previously auto-installed)
    3. Auto-install from GitHub releases → $TRIIBAL_HOME/bin/tirith
    ...
    triibal_bin = os.path.join(_triibal_bin_dir(), "tirith")
    if os.path.isfile(triibal_bin) and os.access(triibal_bin, os.X_OK):
        _resolved_path = triibal_bin
        _install_failure_reason = ""
        _clear_install_failed()
        return triibal_bin
