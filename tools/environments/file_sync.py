...
_FORCE_SYNC_ENV = "TRIIBAL_FORCE_FILE_SYNC"
...
def iter_sync_files(container_base: str = "/root/.triibal") -> list[tuple[str, str]]:
    ...
    the hardcoded /root/.triibal to *container_base* because the remote
...
class FileSyncManager:
    ...
    def sync(self, *, force: bool = False) -> None:
        ...
        or ``TRIIBAL_FORCE_FILE_SYNC=1`` is set.
    ...
    def sync_back(self, triibal_home: Path | None = None) -> None:
        """Pull remote changes back to the host filesystem.

        Downloads the remote ``.triibal/`` directory as a tar archive,
    ...
    def _sync_back_impl(self) -> None:
        ...
        with tempfile.NamedTemporaryFile(suffix=".tar") as tf:
            ...
            with tempfile.TemporaryDirectory(prefix="triibal-sync-back-") as staging:
    ...
    def _infer_host_path(self, remote_path: str,
        ...
        For example, if the mapping has ``/root/.triibal/skills/a.md`` →
        ``~/.triibal/skills/a.md``, a new remote file at
        ``/root/.triibal/skills/b.md`` maps to ``~/.triibal/skills/b.md``.
        """
        mapping = file_mapping if file_mapping is not None else []
        for host, remote in mapping:
