...
One ``CDPSupervisor`` runs per Triibal ``task_id`` that has a reachable CDP
...
DIALOG_BRIDGE_HOST = "triibal-dialog-bridge.invalid"
...
(() => {
  if (window.__triibalDialogBridgeInstalled) return;
  window.__triibalDialogBridgeInstalled = true;
  const ENDPOINT = "http://triibal-dialog-bridge.invalid/";
