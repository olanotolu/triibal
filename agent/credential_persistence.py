...
``auth.json``.  They intentionally have no dependency on ``triibal_cli.auth`` so
...
# Sources Triibal owns and can intentionally persist in auth.json.  Everything
...
def sanitize_borrowed_credential_payload(
    ...
    Owned sources (manual entries and Triibal-owned OAuth/device-code state)
