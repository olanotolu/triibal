...
as ``kind: backend``) or ``~/.triibal/plugins/image_gen/<name>/`` (user, opt-in
via ``plugins.enabled``).
...
class ImageGenProvider(abc.ABC):
    ...
    def display_name(self) -> str:
        """Human-readable label shown in ``triibal tools``. Defaults to ``name.title()``."""
    ...
    def list_models(self) -> List[Dict[str, Any]]:
        """Return catalog entries for ``triibal tools`` model picker.
...
def _images_cache_dir() -> Path:
    """Return ``$TRIIBAL_HOME/cache/images/``, creating parents as needed."""
    from triibal_constants import get_triibal_home

    path = get_triibal_home() / "cache" / "images"
...
def save_b64_image(
    ...
    *,
    ...
    """Decode base64 image data and write it under ``$TRIIBAL_HOME/cache/images/``.
...
def save_url_image(
    ...
    *,
    ...
    """Download an image URL and write it under ``$TRIIBAL_HOME/cache/images/``.
