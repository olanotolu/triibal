...
from triibal_constants import get_triibal_home, get_skills_dir, is_wsl
...
_TRIIBAL_MD_NAMES = (".triibal.md", "TRIIBAL.md")


def _find_triibal_md(cwd: Path) -> Optional[Path]:
    """Discover the nearest ``.triibal.md`` or ``TRIIBAL.md``.
...
TRIIBAL_AGENT_HELP_GUIDANCE = (
    "If the user asks about configuring, setting up, or using Triibal Agent "
    "itself, load the `triibal-agent` skill with skill_view(name='triibal-agent') "
    "before answering. Docs: https://triibal.dev/docs"
)
...
KANBAN_GUIDANCE = (
    ...
    "the shared board at `~/.triibal/kanban.db`. Your task id is in "
    "`$TRIIBAL_KANBAN_TASK`; your workspace is `$TRIIBAL_KANBAN_WORKSPACE`. "
    ...
    "ground truth.\n"
    "2. **Work inside the workspace.** `cd $TRIIBAL_KANBAN_WORKSPACE` before "
    ...
    "## Do NOT\n"
    "\n"
    "- Do not shell out to `triibal kanban <verb>` for board operations. Use "
...
PLATFORM_HINTS = {
    ...
    "webui": (
        "You are in the Triibal WebUI, a browser-based chat interface. "
...
# container / remote host rather than on the machine where Triibal itself
...
def _probe_remote_backend(env_type: str) -> str | None:
    ...
    operate on a different machine than the host Triibal runs on.
    """
    ...
    if not is_remote_backend:
        ...
        if probe:
            hints.append(
                ...
                f"where Triibal itself is running. The host OS, home, and cwd "
                f"of the Triibal process are irrelevant; only the following "
            ...
            hints.append(
                f"Terminal backend: {backend}. Your `terminal`, `read_file`, "
                f"`write_file`, `patch`, and `search_files` tools all operate "
                f"inside {description} — NOT on the machine where Triibal "
...
def _skills_prompt_snapshot_path() -> Path:
    return get_triibal_home() / ".skills_prompt_snapshot.json"
...
def build_skills_system_prompt(
    ...
    scanned alongside the local ``~/.triibal/skills/`` directory.  External dirs
    ...
    _platform_hint = (
        os.environ.get("TRIIBAL_PLATFORM")
        or get_session_env("TRIIBAL_SESSION_PLATFORM")
        or ""
    )
    ...
    if not skills_by_category:
        ...
        result = (
            ...
            "or troubleshoot Triibal Agent itself — its CLI, config, models, providers, tools, "
            "skills, voice, gateway, plugins, or any feature — load the `triibal-agent` skill "
            "first. It has the actual commands (e.g. `triibal config set …`, `triibal tools`, "
            "`triibal setup`) so you don't have to guess or invent workarounds.\n"
...
def build_nous_subscription_prompt(valid_tool_names: "set[str] | None" = None) -> str:
    ...
    lines.extend(
        [
            ...
            "Useful commands: triibal setup, triibal setup tools, triibal setup terminal, triibal status.",
        ]
    )
    return "\n".join(lines)
...
def load_soul_md() -> Optional[str]:
    """Load SOUL.md from TRIIBAL_HOME and return its content, or None.
    ...
    """
    try:
        from triibal_cli.config import ensure_triibal_home
        ensure_triibal_home()
    except Exception as e:
        logger.debug("Could not ensure TRIIBAL_HOME before loading SOUL.md: %s", e)
...
def _load_triibal_md(cwd_path: Path) -> str:
    """.triibal.md / TRIIBAL.md — walk to git root."""
    triibal_md_path = _find_triibal_md(cwd_path)
    if not triibal_md_path:
        return ""
    try:
        content = triibal_md_path.read_text(encoding="utf-8").strip()
        if not content:
            return ""
        content = _strip_yaml_frontmatter(content)
        rel = triibal_md_path.name
        try:
            rel = str(triibal_md_path.relative_to(cwd_path))
        except ValueError:
            pass
        content = _scan_context_content(content, rel)
        result = f"## {rel}\n\n{content}"
        return _truncate_content(result, ".triibal.md")
    except Exception as e:
        logger.debug("Could not read %s: %s", triibal_md_path, e)
        return ""
...
def build_context_files_prompt(cwd: Optional[str] = None, skip_soul: bool = False) -> str:
    ...
    Priority (first found wins — only ONE project context type is loaded):
      1. .triibal.md / TRIIBAL.md  (walk to git root)
      2. AGENTS.md / agents.md   (cwd only)
      3. CLAUDE.md / claude.md   (cwd only)
      4. .cursorrules / .cursor/rules/*.mdc  (cwd only)

    SOUL.md from TRIIBAL_HOME is independent and always included when present.
    ...
    # SOUL.md from TRIIBAL_HOME only — skip when already loaded as identity
    if not skip_soul:
