...
Every Triibal request that hits the Nous Portal — main agent loop, auxiliary
client (compression / titles / vision / web_extract / session_search / etc.),
and any future code path — must carry the same product-attribution tags so
Nous can attribute usage to Triibal Agent and bucket it by client release.

Tag shape (sent in OpenAI-compatible ``extra_body['tags']``):

    [
        "product=triibal-agent",
        "client=triibal-client-v<__version__>",
    ]

The version is sourced live from ``triibal_cli.__version__`` so it auto-aligns
...
``triibal_cli.__version__`` is the canonical source of truth.
"""
...
def _triibal_version() -> str:
    """Return the current Triibal release version, e.g. ``"0.13.0"``.

    Falls back to ``"unknown"`` if ``triibal_cli`` cannot be imported (should
...
def triibal_client_tag() -> str:
    """Return the ``client=...`` tag for Nous Portal requests.

    Format: ``client=triibal-client-v<MAJOR>.<MINOR>.<PATCH>``.
    """
    return f"client=triibal-client-v{_triibal_version()}"
...
def nous_portal_tags() -> List[str]:
    ...
    """
    return ["product=triibal-agent", triibal_client_tag()]
