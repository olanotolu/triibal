...
class MemoryProvider(ABC):
    ...
    def initialize(self, session_id: str, **kwargs) -> None:
        ...
        kwargs always include:
          - triibal_home (str): The active TRIIBAL_HOME directory path. Use this
            for profile-scoped storage instead of hardcoding ``~/.triibal``.
        ...
        kwargs may also include:
          ...
          - agent_workspace (str): Shared workspace name (e.g. "triibal").
    ...
    def get_config_schema(self) -> List[Dict[str, Any]]:
        ...
        Used by 'triibal memory setup' to walk the user through configuration.
    ...
    def save_config(self, values: Dict[str, Any], triibal_home: str) -> None:
        """Write non-secret config to the provider's native location.

        Called by 'triibal memory setup' after collecting user inputs.
        ``values`` contains only non-secret fields (secrets go to .env).
        ``triibal_home`` is the active TRIIBAL_HOME directory path.
