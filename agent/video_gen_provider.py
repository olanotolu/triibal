...
as ``kind: backend``) or ``~/.triibal/plugins/video_gen/<name>/`` (user, opt-in
via ``plugins.enabled``).
...
class VideoGenProvider(abc.ABC):
    ...
    def capabilities(self) -> Dict[str, Any]:
        ...
        Used by the tool layer for soft validation and by ``triibal tools``
...
def _videos_cache_dir() -> Path:
    """Return ``$TRIIBAL_HOME/cache/videos/``, creating parents as needed."""
    from triibal_constants import get_triibal_home

    path = get_triibal_home() / "cache" / "videos"
...
def save_b64_video(
    ...
    *,
    ...
    """Decode base64 video data and write under ``$TRIIBAL_HOME/cache/videos/``.
