"""Centralized logging setup for Triibal Agent.

Provides a single ``setup_logging()`` entry point that both the CLI and
gateway call early in their startup path.  All log files live under
``~/.triibal/logs/`` (profile-aware via ``get_triibal_home()``).
...
from triibal_constants import get_config_path, get_triibal_home
...
def _install_session_record_factory() -> None:
    ...
    if getattr(current_factory, "_triibal_session_injector", False):
    ...
    _session_record_factory._triibal_session_injector = True  # type: ignore[attr-defined]
...
# Used by _ComponentFilter and exposed for ``triibal logs --component``.
COMPONENT_PREFIXES = {
    "gateway": ("gateway", "triibal_plugins"),
...
def setup_logging(
    *,
    ...
    """Configure the Triibal logging subsystem.
    ...
    *force* is ``True``.
    ...
    ----------
    triibal_home
        Override for the Triibal home directory.  Falls back to
        ``get_triibal_home()`` (profile-aware).
    log_level
...
def setup_verbose_logging() -> None:
    ...
    for h in root.handlers:
        if isinstance(h, logging.StreamHandler) and not isinstance(h, RotatingFileHandler):
            if getattr(h, "_triibal_verbose", False):
                return

    handler = logging.StreamHandler()
    handler.setLevel(logging.DEBUG)
    handler.setFormatter(RedactingFormatter(_LOG_FORMAT_VERBOSE, datefmt="%H:%M:%S"))
    handler._triibal_verbose = True  # type: ignore[attr-defined]
...
class _ManagedRotatingFileHandler(RotatingFileHandler):
    ...
    inherit the triibal group. However, both _open() (initial creation) and
    ...
    def __init__(self, *args, **kwargs):
        from triibal_cli.config import is_managed
