...
Auto-tracks and cleans up ephemeral files created during Triibal Agent
sessions — test scripts, temp outputs, cron logs, stale chrome profiles.
Scoped strictly to `$TRIIBAL_HOME` and `/tmp/triibal-*`.
...
| Hook | Behaviour |
|---|---|
| `post_tool_call` | When `write_file` / `terminal` / `patch` creates a file matching `test_*`, `tmp_*`, or `*.test.*` inside `TRIIBAL_HOME`, track it silently as `test` / `temp` / `cron-output`. |
...
- `is_safe_path()` rejects anything outside `TRIIBAL_HOME` or `/tmp/triibal-*`
- Windows mounts (`/mnt/c` etc.) are rejected
- The state directory `$TRIIBAL_HOME/disk-cleanup/` is itself excluded
- `$TRIIBAL_HOME/logs/`, `memories/`, `sessions/`, `skills/`, `plugins/`,
