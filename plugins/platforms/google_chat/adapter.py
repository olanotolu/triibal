...
# ``model_tools`` import time, so ``triibal status``, ``triibal chat``, etc.
...
# ``triibal_plugins.platforms__google_chat.adapter`` once the plugin
...
#     fresh "Triibal is thinking…" card during the small window between
...
def _mime_for_message_type(mime: str) -> MessageType:
    """Map a MIME string to a triibal MessageType.
...
class GoogleChatAdapter(BasePlatformAdapter):
    ...
    def __init__(self, config: PlatformConfig):
        ...
        # Persisted to ${TRIIBAL_HOME}/google_chat_thread_counts.json so
        ...
        try:
            ...
            _triibal_home = _get_triibal_home()
        except (ModuleNotFoundError, ImportError):
            _triibal_home = _Path.home() / ".triibal"
        self._thread_count_store = _ThreadCountStore(
            _triibal_home / "google_chat_thread_counts.json"
        )
        ...
        # they don't sit in the chat forever as "Triibal is thinking…".
    ...
    def _bot_id_cache_path(self) -> _Path:
        ...
        base = os.getenv("TRIIBAL_HOME", str(_Path.home() / ".triibal"))
    ...
    async def _dispatch_message(self, msg: Dict[str, Any], envelope: Dict[str, Any]) -> None:
        ...
        try:
            ...
            if text.startswith("/setup-files") and event.source is not None:
                ...
                # ${TRIIBAL_HOME}/google_chat_user_tokens/<sanitized>.json
    ...
    async def _build_message_event(
        ...
        """Parse a Chat API message into a triibal MessageEvent."""
        space = envelope.get("space") or msg.get("space") or {}
    ...
    async def send_typing(self, chat_id: str, metadata: Any = None) -> None:
        """Post a visible 'Triibal is thinking…' marker message.
        ...
        "Triibal is thinking…" stuck in chat forever, plus one card that
        ...
        body: Dict[str, Any] = {"text": "Triibal is thinking…"}
        if thread_id:
            body["thread"] = {"name": thread_id}
    ...
    async def stop_typing(self, chat_id: str) -> None:
        ...
        Google Chat has no separate typing API: the "Triibal is thinking…"
    ...
    async def on_processing_complete(
        ...
        see "Triibal is thinking…" stuck forever.
        """
    ...
    async def _consume_typing_card_with_text(
        ...
        creating a fresh "Triibal is thinking…" card during any subsequent
...
def interactive_setup() -> None:
    """Walk the user through Google Chat configuration via ``triibal setup``.

    The setup wizard at ``triibal_cli/gateway.py`` calls this for plugin
    ...
    prompt for env vars, persist them to ``~/.triibal/.env`` so the next
    ...
    print_info("  3. Create a Pub/Sub topic (e.g. triibal-chat-events) and a Pull subscription.")
    ...
    print()
    print_success("Google Chat configuration saved to ~/.triibal/.env")
...
def register(ctx) -> None:
    ...
    ctx.register_platform(
        ...
        install_hint="pip install 'triibal-agent[google_chat]'",
        ...
        # the "Triibal is thinking..." marker patches and edit overhead.
        ...
        platform_hint=(
            ...
            "are generating a response, a 'Triibal is thinking…' marker message "
