...
Output is saved as PNG under ``$TRIIBAL_HOME/cache/images/``.
"""
...
class OpenAICodexImageGenProvider(ImageGenProvider):
    ...
    def get_setup_schema(self) -> Dict[str, Any]:
        return {
            ...
            "env_vars": [],
            "post_setup_hint": (
                "Sign in with `triibal auth codex` (or `triibal setup` → Codex) "
        ...
        if not _read_codex_access_token():
            return error_response(
                error=(
                    ...
                    "`triibal auth codex` (or `triibal setup` → Codex) to sign in."
                ),
