...
_DEFAULT_CONTAINER_TAG = "triibal"
...
def _load_supermemory_config(triibal_home: str) -> dict:
    config = _default_config()
    config_path = Path(triibal_home) / "supermemory.json"
...
def _save_supermemory_config(values: dict, triibal_home: str) -> None:
    config_path = Path(triibal_home) / "supermemory.json"
    existing = {}
    if config_path.exists():
        try:
...
class SupermemoryMemoryProvider(MemoryProvider):
    ...
    def get_config_schema(self):
        # Only prompt for the API key during `triibal memory setup`.
        # All other options are documented for $TRIIBAL_HOME/supermemory.json
    ...
    def save_config(self, values, triibal_home):
        ...
        _save_supermemory_config(sanitized, triibal_home)

    def initialize(self, session_id: str, **kwargs) -> None:
        from triibal_constants import get_triibal_home
        self._triibal_home = kwargs.get("triibal_home") or str(get_triibal_home())
        self._session_id = session_id
        self._turn_count = 0
        self._config = _load_supermemory_config(self._triibal_home)
    ...
    def sync_turn(self, user_content: str, assistant_content: str, *, session_id: str = "") -> None:
        ...
        metadata = {"source": "triibal", "type": "conversation_turn"}
        ...
        def _run():
            try:
                self._client.add_memory(
                    content.strip(),
                    metadata={"source": "triibal_memory", "target": target, "type": "explicit_memory"},
