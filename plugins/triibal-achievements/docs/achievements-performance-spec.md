# Triibal Achievements Performance Spec (Post-Hackathon)

Status: Draft (no code changes yet)
Owner: triibal-achievements plugin
...
### `GET /api/plugins/triibal-achievements/achievements`
...
### `POST /api/plugins/triibal-achievements/rescan` (optional)
Manual refresh trigger.
Prefer async trigger + immediate status response.

### `GET /api/plugins/triibal-achievements/scan-status` (optional new)
Reports scan state for UX/ops.

### Removed
- `GET /api/plugins/triibal-achievements/overview`
...
- Persist plugin data under:
  - `~/.triibal/plugins/triibal-achievements/scan_snapshot.json`

### Phase 2
- Incremental scan checkpoints with per-session fingerprints.
- Persist checkpoint data under:
  - `~/.triibal/plugins/triibal-achievements/scan_checkpoint.json`
...
- `~/.triibal/plugins/triibal-achievements/`
