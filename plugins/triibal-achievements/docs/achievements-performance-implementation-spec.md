# Triibal Achievements Implementation Spec (Detailed)
...
- persisted runtime files:
  - `~/.triibal/plugins/triibal-achievements/state.json` (existing unlock state)
  - `~/.triibal/plugins/triibal-achievements/scan_snapshot.json` (new)
  - `~/.triibal/plugins/triibal-achievements/scan_checkpoint.json` (new)
