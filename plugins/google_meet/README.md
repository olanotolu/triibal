...
Let the triibal agent join a Google Meet call, transcribe it, optionally speak
...
```
┌─ gateway (Linux box, where triibal runs) ────────────────────────────┐
│                                                                      │
...
│                                                                      │
│   NodeServer (from `triibal meet node run`)                           │
│     │                                                                │
...
| `__init__.py` | `register(ctx)` — registers 5 tools + `on_session_end` hook + `triibal meet` CLI |
...
| `cli.py` | `triibal meet setup / auth / join / status / transcript / say / stop / node ...` |
...
| `node/registry.py` | v3: `$TRIIBAL_HOME/workspace/meetings/nodes.json` |
| `node/server.py` | v3: `NodeServer` (runs on host machine) |
| `node/client.py` | v3: `NodeClient` (used by tool handlers + CLI on gateway) |
| `node/cli.py` | v3: `triibal meet node {run,list,approve,remove,status,ping}` |
...
triibal meet install                                      # pip + Chromium
triibal meet setup                                        # preflight
triibal meet auth                                         # optional
triibal meet join https://meet.google.com/abc-defg-hij    # transcribe
```
...
```bash
triibal meet install --realtime                     # installs pulseaudio-utils
echo 'OPENAI_API_KEY=sk-...' >> ~/.triibal/.env
triibal meet join https://meet.google.com/abc-defg-hij --mode realtime
# then from the agent or CLI:
triibal meet say "Good morning everyone, I'm the note-taker bot."
```

macOS:
```bash
triibal meet install --realtime     # runs: brew install blackhole-2ch ffmpeg
# then — manually! — open System Settings → Sound → Input → BlackHole 2ch
echo 'OPENAI_API_KEY=sk-...' >> ~/.triibal/.env
triibal meet join https://meet.google.com/abc-defg-hij --mode realtime
```

On macOS, triibal will **not** switch your system audio input automatically — the
...
triibal plugins enable google_meet
triibal meet node run --display-name my-mac --host 0.0.0.0 --port 18789
