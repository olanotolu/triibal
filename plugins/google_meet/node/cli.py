"""`triibal meet node ...` subcommand tree.

Wired into the existing ``triibal meet`` parser by the plugin's top-level
...
def register_cli(subparser: argparse.ArgumentParser) -> None:
    ...
    *subparser* is the ``triibal meet node`` argparse object — typically
    ...
    run.add_argument("--display-name", default="triibal-meet-node")
...
def node_command(args: argparse.Namespace) -> int:
    """Dispatch for ``triibal meet node ...``.
    ...
    if cmd == "run":
        ...
        print(f"             triibal meet node approve <name> ws://<host>:{args.port} {token}")
        try:
