...
Usage in execute_code:
    exec(open(os.path.expanduser(
        os.path.join(os.environ.get("TRIIBAL_HOME", os.path.expanduser("~/.triibal")), "skills/red-teaming/godmode/scripts/auto_jailbreak.py")
    )).read())
...
try:
    ...
    _SKILL_DIR = Path(os.getenv("TRIIBAL_HOME", Path.home() / ".triibal")) / "skills" / "red-teaming" / "godmode"
...
# ═══════════════════════════════════════════════════════════════════
# Triibal config paths
# ═══════════════════════════════════════════════════════════════════

TRIIBAL_HOME = Path(os.getenv("TRIIBAL_HOME", Path.home() / ".triibal"))
CONFIG_PATH = TRIIBAL_HOME / "config.yaml"
PREFILL_PATH = TRIIBAL_HOME / "prefill.json"
...
MODEL_STRATEGIES = {
    ...
    # Nous/Triibal models — already uncensored, just needs clean prompt
    "triibal": {
        "order": ["prefill_only"],
        "system_templates": {},
    },
...
def _detect_model_family(model: str) -> str:
    ...
    if "grok" in model_lower or "x-ai" in model_lower:
        return "grok"
    if "triibal" in model_lower or "nous" in model_lower:
        return "triibal"
...
def _get_current_model() -> tuple:
    """Read current model and provider from Triibal config.yaml.
...
def _write_prefill(prefill_messages: list):
    """Write prefill messages to ~/.triibal/prefill.json."""
    ...
    if winning_strategy:
        ...
        if not dry_run:
            ...
            if verbose:
                ...
                print()
                print("[DONE] Jailbreak locked in. Restart Triibal for changes to take effect.")
        else:
            if verbose:
...
def undo_jailbreak(verbose=True):
    ...
    if verbose:
        print("[UNDO] Jailbreak removed. Restart Triibal for changes to take effect.")
