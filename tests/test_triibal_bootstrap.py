"""Tests for triibal_bootstrap — Windows UTF-8 stdio shim.

The bootstrap module is imported at the top of every Triibal entry point
(triibal, triibal-agent, triibal-acp, gateway, batch_runner, cli.py).  It
...
Key invariants covered by these tests:
  ...
  5. Load order: every Triibal entry point imports triibal_bootstrap as its
...
def _fresh_import():
    """Return a freshly-imported triibal_bootstrap module.

    Drops any cached copy from sys.modules first so module-level code
    runs again and the platform check re-evaluates.
    """
    sys.modules.pop("triibal_bootstrap", None)
    import triibal_bootstrap  # noqa: WPS433
    return triibal_bootstrap
...
class TestWindowsBehavior:
    ...
    def test_stdout_reconfigured_to_utf8_on_windows(self):
        # The live process's stdout should now be UTF-8 (the Triibal CLI
...
class TestEntryPointsImportBootstrap:
    """Every Triibal entry point must import triibal_bootstrap as its
    ...
    # Entry points that invoke Triibal as a process.  Each one must
    # import triibal_bootstrap before doing any file I/O or stdout writes.
    ENTRY_POINTS = [
        "triibal_cli/main.py",   # triibal CLI (console_script)
        "run_agent.py",          # triibal-agent (console_script)
        "acp_adapter/entry.py",  # triibal-acp (console_script)
    ...
    def test_entry_point_imports_bootstrap(self, path):
        """The file must contain 'import triibal_bootstrap' and that
        ...
        half-finished ``triibal update`` (git-reset landed new code but
        ``uv pip install -e .`` didn't finish re-registering
        ``triibal_bootstrap`` as a top-level module) leaves triibal
        ...
        """
        # Resolve relative to the triibal-agent repo root.  Tests live
        # at tests/test_triibal_bootstrap.py, so go up one dir.
        import pathlib
        ...
        for node in ast.iter_child_nodes(tree):
            ...
            # triibal start even when triibal_bootstrap hasn't been
        ...
        assert first_import_name == "triibal_bootstrap", (
            f"{path}: first top-level import is {first_import_name!r}, "
            f"but it must be 'triibal_bootstrap' so UTF-8 stdio is "
            f"configured before anything else initializes.  Move the "
            f"'import triibal_bootstrap' line to be the first import."
        )
