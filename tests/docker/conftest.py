...
Override the image with ``TRIIBAL_TEST_IMAGE`` env var to point at a pre-built
...
IMAGE_TAG = os.environ.get("TRIIBAL_TEST_IMAGE", "triibal-agent-harness:latest")
...
def built_image() -> str:
    ...
    Override with ``TRIIBAL_TEST_IMAGE`` env var to point at a pre-built
    image (faster local iteration).
    """
    if os.environ.get("TRIIBAL_TEST_IMAGE"):
        return IMAGE_TAG
...
# ---------------------------------------------------------------------------
# docker_exec — default to the unprivileged triibal user
# ---------------------------------------------------------------------------
#
# Background: every Triibal runtime path inside the container drops to UID
# 10000 (the ``triibal`` user) via ``s6-setuidgid triibal``. ``docker exec``
...
# fails (PermissionError swallowed) for triibal, so a test that ran as root
...
# helpers below run every probe under ``-u triibal`` unless a specific
...
def docker_exec(
    ...
    *args: str,
    ...
    """Run a command inside ``container`` as ``user`` (default: triibal).
