...
TRIIBAL_MAX_ITERATIONS, but a later per-turn load_dotenv(..., override=True)
can restore a stale .env TRIIBAL_MAX_ITERATIONS value before the next turn.
"""
...
def test_reload_runtime_env_keeps_env_max_iterations_when_config_omits_key(
    ...
    triibal_home = tmp_path / ".triibal"
    triibal_home.mkdir()
    (triibal_home / "config.yaml").write_text(yaml.safe_dump({"agent": {}}), encoding="utf-8")
    (triibal_home / ".env").write_text("TRIIBAL_MAX_ITERATIONS=123\n", encoding="utf-8")

    monkeypatch.setattr(gateway_run, "_triibal_home", triibal_home)
    monkeypatch.delenv("TRIIBAL_MAX_ITERATIONS", raising=False)

    gateway_run._reload_runtime_env_preserving_config_authority()

    assert os.environ["TRIIBAL_MAX_ITERATIONS"] == "123"
