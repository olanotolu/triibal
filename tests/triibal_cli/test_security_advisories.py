"""Tests for triibal_cli.security_advisories.
...
import triibal_cli.security_advisories as adv
...
def isolated_home(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> Path:
    """Redirect TRIIBAL_HOME so banner cache and config writes are sandboxed."""
...
class TestAck:
    ...
    def test_ack_advisory_persists_id(self, isolated_home, monkeypatch):
        ...
        # don't depend on the full triibal_cli.config bootstrap.
        store: dict = {"security": {}}
        monkeypatch.setattr(
            "triibal_cli.config.load_config", lambda: store
        )
        monkeypatch.setattr(
            "triibal_cli.config.save_config",
...
class TestRendering:
    def test_short_banner_lines_includes_id_and_version(self, fake_advisory):
        ...
        assert "triibal doctor" in joined
