"""Tests for the top-level `./triibal` launcher script."""
...
def test_launcher_delegates_to_argparse_entrypoint(monkeypatch):
    """`./triibal` should use `triibal_cli.main`, not the legacy Fire wrapper."""
    launcher_path = Path(__file__).resolve().parents[2] / "triibal"
    called = []

    fake_main_module = types.ModuleType("triibal_cli.main")

    def fake_main():
        called.append("triibal_cli.main")

    fake_main_module.main = fake_main
    monkeypatch.setitem(sys.modules, "triibal_cli.main", fake_main_module)
    ...
    assert called == ["triibal_cli.main"]
