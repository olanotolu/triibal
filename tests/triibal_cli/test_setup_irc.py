"""Tests for IRC gateway configuration via `triibal setup gateway` UI.

Covers the full plugin-platform discovery → status → configure flow so that
a fresh Triibal install (no state, no env vars) can set up IRC through the
...
class TestIRCFreshInstallDiscovery:
    """IRC appears in the setup menu on a brand-new Triibal install."""
    ...
    def test_irc_status_configured_when_env_set(self, monkeypatch):
        ...
        try:
            ...
            monkeypatch.setenv("IRC_CHANNEL", "#triibal")
            monkeypatch.setenv("IRC_NICKNAME", "triibal-bot")
...
class TestIRCGatewaySetupFreshInstall:
    """Simulate the full `triibal setup gateway` experience with IRC present."""
