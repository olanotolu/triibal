"""Unit tests for triibal_cli.pty_bridge — PTY spawning + byte forwarding.
...
class TestPtyBridgeIO:
    def test_reads_child_stdout(self):
        bridge = PtyBridge.spawn(["/bin/sh", "-c", "printf triibal-ok"])
        try:
            output = _read_until(bridge, b"triibal-ok")
            assert b"triibal-ok" in output
        finally:
            bridge.close()
...
class TestPtyBridgeEnv:
    ...
    def test_env_is_forwarded(self):
        bridge = PtyBridge.spawn(
            ["/bin/sh", "-c", "printf %s \"$TRIIBAL_PTY_TEST\""],
            env={**os.environ, "TRIIBAL_PTY_TEST": "pty-env-works"},
        )
        try:
