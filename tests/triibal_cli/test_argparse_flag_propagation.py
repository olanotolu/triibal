...
subparser, placing the flag BEFORE the subcommand (e.g. 'triibal --yolo chat')
...
def _build_parser():
    """Build the triibal argument parser from the real code.
...
class TestChatVerboseArg:
    ...
    def test_cmd_chat_forwards_none_when_verbose_is_absent(self, monkeypatch):
        ...
        fake_banner = types.ModuleType("triibal_cli.banner")
        ...
        monkeypatch.setitem(sys.modules, "triibal_cli.banner", fake_banner)
...
class TestYoloEnvVar:
    """Verify --yolo sets TRIIBAL_YOLO_MODE regardless of flag position.
...
class TestAcceptHooksOnAgentSubparsers:
    ...
    parser and `chat`, so `triibal gateway run --accept-hooks` failed
    ...
    def test_accepted_at_every_position(self, argv):
        """Invoking `triibal <argv>` must exit 0 (help) rather than
