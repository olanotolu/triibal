...
def container_env(tmp_path, monkeypatch):
    """Set up a fake TRIIBAL_HOME with .container-mode file."""
    triibal_home = tmp_path / ".triibal"
    triibal_home.mkdir()
    monkeypatch.setenv("TRIIBAL_HOME", str(triibal_home))
    monkeypatch.delenv("TRIIBAL_DEV", raising=False)

    container_mode = triibal_home / ".container-mode"
    container_mode.write_text(
        "# Written by NixOS activation script. Do not edit manually.\n"
        "backend=podman\n"
        "container_name=triibal-agent\n"
        "exec_user=triibal\n"
        "triibal_bin=/data/current-package/bin/triibal\n"
    )
    return triibal_home
...
def test_get_container_exec_info_returns_metadata(container_env):
    ...
    with patch("triibal_constants.is_container", return_value=False):
        info = get_container_exec_info()

    assert info is not None
    assert info["backend"] == "podman"
    assert info["container_name"] == "triibal-agent"
    assert info["exec_user"] == "triibal"
    assert info["triibal_bin"] == "/data/current-package/bin/triibal"


def test_get_container_exec_info_none_inside_container(container_env):
    """Returns None when we're already inside a container."""
    with patch("triibal_constants.is_container", return_value=True):
        info = get_container_exec_info()
...
def test_get_container_exec_info_skipped_when_triibal_dev(container_env, monkeypatch):
    """Returns None when TRIIBAL_DEV=1 is set (dev mode bypass)."""
    monkeypatch.setenv("TRIIBAL_DEV", "1")
...
def test_get_container_exec_info_not_skipped_when_triibal_dev_zero(container_env, monkeypatch):
    """TRIIBAL_DEV=0 does NOT trigger bypass — only '1' does."""
    monkeypatch.setenv("TRIIBAL_DEV", "0")
...
def test_get_container_exec_info_defaults():
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        triibal_home = Path(tmpdir) / ".triibal"
        triibal_home.mkdir()
        (triibal_home / ".container-mode").write_text(
            "# minimal file with no keys\n"
        )

        with patch("triibal_constants.is_container", return_value=False), \
             patch.dict(get_container_exec_info.__globals__, {"get_triibal_home": lambda: triibal_home}), \
             patch.dict(os.environ, {}, clear=False):
            os.environ.pop("TRIIBAL_DEV", None)
            info = get_container_exec_info()
...
def test_get_container_exec_info_docker_backend(container_env):
    ...
    (container_env / ".container-mode").write_text(
        "backend=docker\n"
        "container_name=triibal-custom\n"
        "exec_user=myuser\n"
        "triibal_bin=/opt/triibal/bin/triibal\n"
    )
    ...
    assert info["backend"] == "docker"
    assert info["container_name"] == "triibal-custom"
    assert info["exec_user"] == "myuser"
    assert info["triibal_bin"] == "/opt/triibal/bin/triibal"
...
def test_exec_in_container_calls_execvp(docker_container_info):
    ...
    from triibal_cli.main import _exec_in_container
    ...
    assert "-it" in cmd
    idx_u = cmd.index("-u")
    assert cmd[idx_u + 1] == "triibal"
    e_indices = [i for i, v in enumerate(cmd) if v == "-e"]
    e_values = [cmd[i + 1] for i in e_indices]
    assert "TERM=xterm-256color" in e_values
    assert "LANG=en_US.UTF-8" in e_values
    assert "triibal-agent" in cmd
    assert "/data/current-package/bin/triibal" in cmd
    assert "chat" in cmd
