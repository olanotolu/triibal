...
``triibal_cli.main`` skips eager plugin discovery at argparse-setup time
when the invocation is clearly targeting a known built-in subcommand.
This saves 500-650ms on ``triibal --help``, ``triibal version``,
``triibal logs``, etc., by not importing ``google.cloud.pubsub_v1``,
...
def _live_subcommand_names() -> set[str]:
    """Run ``triibal --help`` in-process and parse the subcommand block.
    ...
    built-in-only set.
    """
    from triibal_cli import main as _main

    argv_backup = sys.argv[:]
    sys.argv = ["triibal", "--help"]
    buf = io.StringIO()
    try:
...
@pytest.mark.parametrize(
    ...
    [
        ...
        (["triibal", "-w", "--tui"], None),
        (["triibal", "version"], "version"),
        (["triibal", "--tui", "chat"], "chat"),
        (["triibal", "-w", "logs"], "logs"),
        (["triibal", "chat", "hello world"], "chat"),
        (["triibal", "gateway", "run"], "gateway"),
        # Top-level value-taking flags: the value should be skipped.
        (["triibal", "-m", "gpt5", "chat"], "chat"),
        (["triibal", "--model", "gpt5", "chat", "hi"], "chat"),
        (["triibal", "-m", "gpt5", "--provider", "openai", "chat"], "chat"),
        (["triibal", "-z", "hello world"], None),
        (["triibal", "-z", "hello", "chat"], "chat"),
        (["triibal", "--model=gpt5", "chat"], "chat"),     # inline form
        (["triibal", "--", "chat"], "chat"),               # -- terminator
        (["triibal", "-w", "--"], None),
        # Unknown positional after skipped flags → plugin-cmd candidate.
        (["triibal", "some-plugin-cmd"], "some-plugin-cmd"),
        (["triibal", "-m", "gpt5", "some-plugin-cmd"], "some-plugin-cmd"),
    ],
)
...
@pytest.mark.parametrize(
    ...
    [
        ["triibal"],                          # bare → chat
        ["triibal", "--help"],                # top-level help
        ["triibal", "-h"],
        ["triibal", "version"],               # known built-in
        ["triibal", "logs"],
        ["triibal", "gateway", "run"],
        ["triibal", "--tui"],
        ["triibal", "-w", "--tui"],
        ["triibal", "chat", "hi"],
        ["triibal", "help"],                  # accepted built-in-ish
        ["triibal", "-m", "gpt5", "chat"],    # flag-value-skipping
    ],
)
...
@pytest.mark.parametrize(
    "argv",
    [
        ["triibal", "meet", "join"],          # potential google_meet plugin
        ["triibal", "honcho", "status"],      # potential memory plugin
        ["triibal", "unknown-subcmd"],
    ],
)
...
def test_builtin_set_covers_every_registered_subcommand():
    ...
    # in the set so ``triibal help <cmd>`` skips discovery; it won't show
    ...
    assert not missing_from_declaration, (
        ...
        f"triibal_cli/main.py::_BUILTIN_SUBCOMMANDS so plugin discovery "
