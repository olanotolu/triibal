"""Regression tests for TOCTOU-safe credential file writers in ``triibal_cli.auth``.
...
# ---------------------------------------------------------------------------
# _save_auth_store  (~/.triibal/auth.json — every native OAuth provider)
# ---------------------------------------------------------------------------
...
def test_shared_nous_store_writes_0o600_with_0o700_parent(tmp_path, monkeypatch):
    ...
    # home" reference — which currently tracks TRIIBAL_HOME via
    # get_default_triibal_root() — can't collide with our override and
    # falsely claim we're writing to the real user's shared store.
    monkeypatch.setenv("TRIIBAL_SHARED_AUTH_DIR", str(tmp_path / "shared_override"))
    old_umask = os.umask(0o022)
    try:
        from triibal_cli import auth as auth_mod
