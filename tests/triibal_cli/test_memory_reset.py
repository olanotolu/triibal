"""Tests for the `triibal memory reset` CLI command.
...
- Profile-scoped reset (uses TRIIBAL_HOME)
"""
...
def memory_env(tmp_path, monkeypatch):
    """Set up a fake TRIIBAL_HOME with memory files."""
    triibal_home = tmp_path / ".triibal"
    memories = triibal_home / "memories"
    memories.mkdir(parents=True)
    monkeypatch.setenv("TRIIBAL_HOME", str(triibal_home))

    # Create sample memory files
    (memories / "MEMORY.md").write_text(
        "§\nTriibal repo is at ~/.triibal/triibal-agent\n§\nUser prefers dark themes",
        encoding="utf-8",
    )
...
def _run_memory_reset(target="all", yes=False, monkeypatch=None, confirm_input="no"):
    ...
    Simulates what happens when `triibal memory reset` is run.
    """
...
class TestMemoryReset:
    ...
    def test_reset_no_files_exist(self, tmp_path, monkeypatch):
        ...
        triibal_home = tmp_path / ".triibal"
        (triibal_home / "memories").mkdir(parents=True)
        monkeypatch.setenv("TRIIBAL_HOME", str(triibal_home))
    ...
    def test_reset_profile_scoped(self, tmp_path, monkeypatch):
        """Reset should work on the active profile's TRIIBAL_HOME."""
    ...
    def test_reset_empty_memories_dir(self, tmp_path, monkeypatch):
        ...
        # The memories dir won't exist; get_triibal_home() / "memories" won't have files
        result = _run_memory_reset(target="all", yes=True)
        assert result == "nothing"
