...
def _clean_state():
    ...
    saved = {}
    for k in ("TRIIBAL_INTERACTIVE", "TRIIBAL_GATEWAY_SESSION", "TRIIBAL_EXEC_ASK", "TRIIBAL_YOLO_MODE"):
        if k in os.environ:
...
class TestTirithWarnSafe:
    ...
    def test_warn_session_approved(self, mock_tirith):
        os.environ["TRIIBAL_INTERACTIVE"] = "1"
        session_key = os.getenv("TRIIBAL_SESSION_KEY", "default")
    ...
    def test_warn_non_interactive_auto_allow(self, mock_tirith):
        # No TRIIBAL_INTERACTIVE or TRIIBAL_GATEWAY_SESSION set
        result = check_all_command_guards("curl https://bit.ly/abc", "local")
        assert result["approved"] is True
