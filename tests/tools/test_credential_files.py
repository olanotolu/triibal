...
class TestRegisterCredentialFiles:
    def test_dict_with_path_key(self, tmp_path):
        ...
        triibal_home.mkdir()
        (triibal_home / "token.json").write_text("{}")
        ...
        assert len(mounts) == 1
        assert mounts[0]["host_path"] == str(triibal_home / "token.json")
        assert mounts[0]["container_path"] == "/root/.triibal/token.json"
    ...
    def test_string_entry(self, tmp_path):
        triibal_home = tmp_path / ".triibal"
        triibal_home.mkdir()
        (triibal_home / "secret.key").write_text("key")
    ...
    def test_path_takes_precedence_over_name(self, tmp_path):
        ...
        triibal_home = tmp_path / ".triibal"
        triibal_home.mkdir()
        (triibal_home / "real.json").write_text("{}")
...
class TestSkillsDirectoryMount:
    def test_returns_mount_when_skills_dir_exists(self, tmp_path):
        ...
        assert mounts[0]["container_path"] == "/root/.triibal/skills"
    ...
    def test_custom_container_base(self, tmp_path):
        triibal_home = tmp_path / ".triibal"
        (triibal_home / "skills").mkdir(parents=True)

        with patch.dict(os.environ, {"TRIIBAL_HOME": str(triibal_home)}):
            mounts = get_skills_directory_mount(container_base="/home/user/.triibal")

        assert mounts[0]["container_path"] == "/home/user/.triibal/skills"
...
class TestIterSkillsFiles:
    def test_returns_files_skipping_symlinks(self, tmp_path):
        ...
        assert "/root/.triibal/skills/cat/myskill/SKILL.md" in paths
        assert "/root/.triibal/skills/cat/myskill/scripts/run.sh" in paths
...
class TestPathTraversalSecurity:
    ...
    def test_dotdot_traversal_rejected(self, tmp_path, monkeypatch):
        """'../sensitive' must not escape TRIIBAL_HOME."""
        monkeypatch.setenv("TRIIBAL_HOME", str(tmp_path / ".triibal"))
        (tmp_path / ".triibal").mkdir()

        # Create a sensitive file one level above triibal_home
    ...
    def test_deep_traversal_rejected(self, tmp_path, monkeypatch):
        ...
        # Create a fake sensitive file outside triibal_home
    ...
    def test_legitimate_file_still_works(self, tmp_path, monkeypatch):
        """Normal files inside TRIIBAL_HOME must still be registered."""
        triibal_home = tmp_path / ".triibal"
        triibal_home.mkdir()
        monkeypatch.setenv("TRIIBAL_HOME", str(triibal_home))
        (triibal_home / "token.json").write_text('{"token": "abc"}')
    ...
    def test_nested_subdir_inside_triibal_home_allowed(self, tmp_path, monkeypatch):
        """Files in subdirectories of TRIIBAL_HOME must be allowed."""
        triibal_home = tmp_path / ".triibal"
        triibal_home.mkdir()
        subdir = triibal_home / "creds"
        subdir.mkdir()
    ...
    def test_symlink_traversal_rejected(self, tmp_path, monkeypatch):
        """A symlink inside TRIIBAL_HOME pointing outside must be rejected."""
        triibal_home = tmp_path / ".triibal"
        triibal_home.mkdir()
        monkeypatch.setenv("TRIIBAL_HOME", str(triibal_home))

        # Create a sensitive file outside triibal_home
        sensitive = tmp_path / "sensitive.json"
        sensitive.write_text('{"secret": "value"}')

        # Create a symlink inside triibal_home pointing outside
        symlink = triibal_home / "evil_link.json"
        try:
        ...
        # The resolved path escapes TRIIBAL_HOME — must be rejected
        assert result is False
        assert get_credential_file_mounts() == []
...
class TestConfigPathTraversal:
    ...
    def _write_config(self, triibal_home: Path, cred_files: list):
        import yaml
    ...
    def test_config_traversal_rejected(self, tmp_path, monkeypatch):
        """'../secret' in config.yaml must not escape TRIIBAL_HOME."""
        triibal_home = tmp_path / ".triibal"
        triibal_home.mkdir()
        monkeypatch.setenv("TRIIBAL_HOME", str(triibal_home))
        ...
        sensitive.write_text("{}")
        self._write_config(triibal_home, ["../secret.json"])
    ...
    def test_config_absolute_path_rejected(self, tmp_path, monkeypatch):
        ...
        sensitive.write_text("{}")
        self._write_config(triibal_home, [str(sensitive)])

        mounts = get_credential_file_mounts()
        assert mounts == []

    def test_config_legitimate_file_works(self, tmp_path, monkeypatch):
        """Normal files inside TRIIBAL_HOME via config must still mount."""
        triibal_home = tmp_path / ".triibal"
        triibal_home.mkdir()
        monkeypatch.setenv("TRIIBAL_HOME", str(triibal_home))

        (triibal_home / "oauth.json").write_text("{}")
        self._write_config(triibal_home, ["oauth.json"])
...
class TestCacheDirectoryMounts:
    ...
    def test_returns_existing_cache_dirs(self, tmp_path, monkeypatch):
        ...
        triibal_home = tmp_path / ".triibal"
        triibal_home.mkdir()
        (triibal_home / "cache" / "documents").mkdir(parents=True)
        (triibal_home / "cache" / "audio").mkdir(parents=True)
        monkeypatch.setenv("TRIIBAL_HOME", str(triibal_home))

        mounts = get_cache_directory_mounts()
        paths = {m["container_path"] for m in mounts}
        assert "/root/.triibal/cache/documents" in paths
        assert "/root/.triibal/cache/audio" in paths
    ...
    def test_skips_nonexistent_dirs(self, tmp_path, monkeypatch):
        ...
        mounts = get_cache_directory_mounts()
        assert len(mounts) == 1
        assert mounts[0]["container_path"] == "/root/.triibal/cache/documents"
    ...
    def test_legacy_dir_names_resolved(self, tmp_path, monkeypatch):
        ...
        triibal_home = tmp_path / ".triibal"
        triibal_home.mkdir()
        # Use legacy dir name — get_triibal_dir prefers old if it exists
        (triibal_home / "document_cache").mkdir()
        (triibal_home / "image_cache").mkdir()
        monkeypatch.setenv("TRIIBAL_HOME", str(triibal_home))

        mounts = get_cache_directory_mounts()
        host_paths = {m["host_path"] for m in mounts}
        assert str(triibal_home / "document_cache") in host_paths
        assert str(triibal_home / "image_cache") in host_paths
        # Container paths always use the new layout
        container_paths = {m["container_path"] for m in mounts}
        assert "/root/.triibal/cache/documents" in container_paths
        assert "/root/.triibal/cache/images" in container_paths
...
class TestIterCacheFiles:
    ...
    def test_enumerates_files(self, tmp_path, monkeypatch):
        ...
        triibal_home = tmp_path / ".triibal"
        doc_dir = triibal_home / "cache" / "documents"
    ...
    def test_nested_files(self, tmp_path, monkeypatch):
        ...
        triibal_home = tmp_path / ".triibal"
        ss_dir = triibal_home / "cache" / "screenshots"
        ...
        assert entries[0]["container_path"] == "/root/.triibal/cache/screenshots/session_abc/screen1.png"
