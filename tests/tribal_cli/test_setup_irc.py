"""Tests for IRC gateway configuration via `tribal setup gateway` UI.

Covers the full plugin-platform discovery → status → configure flow so that
a fresh Tribal install (no state, no env vars) can set up IRC through the
...
class TestIRCFreshInstallDiscovery:
    """IRC appears in the setup menu on a brand-new Tribal install."""
    ...
    def test_irc_status_configured_when_env_set(self, monkeypatch):
        ...
        try:
            ...
            monkeypatch.setenv("IRC_CHANNEL", "#tribal")
            monkeypatch.setenv("IRC_NICKNAME", "tribal-bot")
...
class TestIRCGatewaySetupFreshInstall:
    """Simulate the full `tribal setup gateway` experience with IRC present."""
