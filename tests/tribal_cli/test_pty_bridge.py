"""Unit tests for tribal_cli.pty_bridge — PTY spawning + byte forwarding.
...
class TestPtyBridgeIO:
    def test_reads_child_stdout(self):
        bridge = PtyBridge.spawn(["/bin/sh", "-c", "printf tribal-ok"])
        try:
            output = _read_until(bridge, b"tribal-ok")
            assert b"tribal-ok" in output
        finally:
            bridge.close()
...
class TestPtyBridgeEnv:
    ...
    def test_env_is_forwarded(self):
        bridge = PtyBridge.spawn(
            ["/bin/sh", "-c", "printf %s \"$TRIBAL_PTY_TEST\""],
            env={**os.environ, "TRIBAL_PTY_TEST": "pty-env-works"},
        )
        try:
