"""Tests for the top-level `./tribal` launcher script."""
...
def test_launcher_delegates_to_argparse_entrypoint(monkeypatch):
    """`./tribal` should use `tribal_cli.main`, not the legacy Fire wrapper."""
    launcher_path = Path(__file__).resolve().parents[2] / "tribal"
    called = []

    fake_main_module = types.ModuleType("tribal_cli.main")

    def fake_main():
        called.append("tribal_cli.main")

    fake_main_module.main = fake_main
    monkeypatch.setitem(sys.modules, "tribal_cli.main", fake_main_module)
