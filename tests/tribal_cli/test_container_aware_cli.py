...
def container_env(tmp_path, monkeypatch):
    """Set up a fake TRIBAL_HOME with .container-mode file."""
    tribal_home = tmp_path / ".tribal"
    tribal_home.mkdir()
    monkeypatch.setenv("TRIBAL_HOME", str(tribal_home))
    monkeypatch.delenv("TRIBAL_DEV", raising=False)

    container_mode = tribal_home / ".container-mode"
    container_mode.write_text(
        "# Written by NixOS activation script. Do not edit manually.\n"
        "backend=podman\n"
        "container_name=tribal-agent\n"
        "exec_user=tribal\n"
        "tribal_bin=/data/current-package/bin/tribal\n"
    )
    return tribal_home
...
def test_get_container_exec_info_returns_metadata(container_env):
    ...
    with patch("tribal_constants.is_container", return_value=False):
        info = get_container_exec_info()

    assert info is not None
    assert info["backend"] == "podman"
    assert info["container_name"] == "tribal-agent"
    assert info["exec_user"] == "tribal"
    assert info["tribal_bin"] == "/data/current-package/bin/tribal"


def test_get_container_exec_info_none_inside_container(container_env):
    """Returns None when we're already inside a container."""
    with patch("tribal_constants.is_container", return_value=True):
        info = get_container_exec_info()
...
def test_get_container_exec_info_skipped_when_tribal_dev(container_env, monkeypatch):
    """Returns None when TRIBAL_DEV=1 is set (dev mode bypass)."""
    monkeypatch.setenv("TRIBAL_DEV", "1")
...
def test_get_container_exec_info_not_skipped_when_tribal_dev_zero(container_env, monkeypatch):
    """TRIBAL_DEV=0 does NOT trigger bypass — only '1' does."""
    monkeypatch.setenv("TRIBAL_DEV", "0")
...
def test_get_container_exec_info_defaults():
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        tribal_home = Path(tmpdir) / ".tribal"
        tribal_home.mkdir()
        (tribal_home / ".container-mode").write_text(
            "# minimal file with no keys\n"
        )

        with patch("tribal_constants.is_container", return_value=False), \
             patch.dict(get_container_exec_info.__globals__, {"get_tribal_home": lambda: tribal_home}), \
             patch.dict(os.environ, {}, clear=False):
            os.environ.pop("TRIBAL_DEV", None)
            info = get_container_exec_info()
...
def test_get_container_exec_info_docker_backend(container_env):
    ...
    (container_env / ".container-mode").write_text(
        "backend=docker\n"
        "container_name=tribal-custom\n"
        "exec_user=myuser\n"
        "tribal_bin=/opt/tribal/bin/tribal\n"
    )
    ...
    assert info["backend"] == "docker"
    assert info["container_name"] == "tribal-custom"
    assert info["exec_user"] == "myuser"
    assert info["tribal_bin"] == "/opt/tribal/bin/tribal"
...
def test_exec_in_container_calls_execvp(docker_container_info):
    ...
    from tribal_cli.main import _exec_in_container
    ...
    assert "-it" in cmd
    idx_u = cmd.index("-u")
    assert cmd[idx_u + 1] == "tribal"
    e_indices = [i for i, v in enumerate(cmd) if v == "-e"]
    e_values = [cmd[i + 1] for i in e_indices]
    assert "TERM=xterm-256color" in e_values
    assert "LANG=en_US.UTF-8" in e_values
    assert "tribal-agent" in cmd
    assert "/data/current-package/bin/tribal" in cmd
    assert "chat" in cmd
