"""Tests for ``tribal debug`` CLI command and debug utilities."""
...
def tribal_home(tmp_path, monkeypatch):
    """Set up an isolated TRIBAL_HOME with minimal logs."""
...
class TestUploadPasteRs:
    ...
    def test_upload_paste_rs_success(self):
        from tribal_cli.debug import _upload_paste_rs
        ...
        with patch("tribal_cli.debug.urllib.request.urlopen", return_value=mock_resp):
    ...
    def test_upload_paste_rs_network_error(self):
        ...
        with patch(
            "tribal_cli.debug.urllib.request.urlopen",
...
class TestUploadDpasteCom:
    ...
    def test_upload_dpaste_com_success(self):
        from tribal_cli.debug import _upload_dpaste_com
...
class TestUploadToPastebin:
    ...
    def test_tries_paste_rs_first(self):
        from tribal_cli.debug import upload_to_pastebin

        with patch("tribal_cli.debug._upload_paste_rs",
    ...
    def test_falls_back_to_dpaste_com(self):
        ...
        with patch("tribal_cli.debug._upload_paste_rs",
             ...
             patch("tribal_cli.debug._upload_dpaste_com",
...
class TestCaptureLogSnapshot:
    ...
    def test_reads_small_file(self, tribal_home):
        from tribal_cli.debug import _capture_log_snapshot
    ...
    def test_empty_primary_reports_file_empty(self, tribal_home):
        """Empty primary (no .1 fallback) surfaces as '(file empty)', not missing."""
        (tribal_home / "logs" / "agent.log").write_text("")
    ...
    def test_race_truncate_after_resolve_reports_empty(self, tribal_home, monkeypatch):
        """If the log is truncated between resolve and stat, say 'empty', not 'missing'."""
        log_path = tribal_home / "logs" / "agent.log"
        from tribal_cli import debug
    ...
    def test_truncates_large_file(self, tribal_home):
        """Files larger than max_bytes get tail-truncated."""
        from tribal_cli.debug import _capture_log_snapshot

        # Write a file larger than 1KB
        big_content = "x" * 100 + "\n"
        (tribal_home / "logs" / "agent.log").write_text(big_content * 200)

        snap = _capture_log_snapshot("agent", tail_lines=10, max_bytes=1024)
        assert snap.full_text is not None
        assert "truncated" in snap.full_text

    def test_keeps_first_line_when_truncation_on_boundary(self, tribal_home):
        ...
        (tribal_home / "logs" / "agent.log").write_text(line * num_lines)
    ...
    def test_drops_partial_when_truncation_mid_line(self, tribal_home):
    ...
    def test_unknown_log_returns_none(self, tribal_home):
        from tribal_cli.debug import _capture_log_snapshot
        snap = _capture_log_snapshot("nonexistent", tail_lines=10)
        assert snap.full_text is None

    def test_falls_back_to_rotated_file(self, tribal_home):
    ...
    def test_prefers_primary_over_rotated(self, tribal_home):
    ...
    def test_falls_back_when_primary_empty(self, tribal_home):
...
# ---------------------------------------------------------------------------
# Capture log redaction (force=True applies regardless of TRIBAL_REDACT_SECRETS)
# ---------------------------------------------------------------------------
...
class TestCaptureLogSnapshotRedaction:
    ...
    @pytest.fixture
    def tribal_home_with_secret(self, tmp_path, monkeypatch):
        """Isolated TRIBAL_HOME whose agent.log contains a vendor-prefixed token."""
        home = tmp_path / ".tribal"
        home.mkdir()
        monkeypatch.setenv("TRIBAL_HOME", str(home))
    ...
    def test_default_redacts_tail_and_full_text(self, tribal_home_with_secret):
        from tribal_cli.debug import _capture_log_snapshot
    ...
    def test_redact_false_passes_through(self, tribal_home_with_secret):
        from tribal_cli.debug import _capture_log_snapshot
    ...
    def test_force_true_works_when_redaction_disabled(
        self, tribal_home_with_secret, monkeypatch
    ):
        ...
        input unchanged when TRIBAL_REDACT_SECRETS=false, and the share-time
        ...
        monkeypatch.setenv("TRIBAL_REDACT_SECRETS", "false")

        from tribal_cli.debug import _capture_log_snapshot

        assert os.environ.get("TRIBAL_REDACT_SECRETS", "") == "false"
    ...
    def test_default_redacts_email_addresses_for_public_share(
        self, tribal_home_with_secret
    ):
        from tribal_cli.debug import _capture_log_snapshot

        log_path = tribal_home_with_secret / "logs" / "agent.log"
        log_path.write_text(
    ...
    def test_no_redact_preserves_email_addresses(self, tribal_home_with_secret):
        from tribal_cli.debug import _capture_log_snapshot
    ...
    def test_capture_default_log_snapshots_threads_redact(
        ...
        from tribal_cli.debug import _capture_default_log_snapshots
...
class TestCollectDebugReport:
    ...
    def test_report_includes_dump_output(self, tribal_home):
        from tribal_cli.debug import collect_debug_report

        with patch("tribal_cli.dump.run_dump") as mock_dump:
            mock_dump.side_effect = lambda args: print(
                "--- tribal dump ---\nversion: 0.8.0\n--- end dump ---"
            )
            report = collect_debug_report(log_lines=50)

        assert "--- tribal dump ---" in report
        assert "version: 0.8.0" in report

    def test_report_includes_agent_log(self, tribal_home):
        from tribal_cli.debug import collect_debug_report

        with patch("tribal_cli.dump.run_dump"):
            report = collect_debug_report(log_lines=50)

        assert "--- agent.log" in report
        assert "session started" in report

    def test_report_includes_errors_log(self, tribal_home):
        from tribal_cli.debug import collect_debug_report
    ...
    def test_report_includes_gateway_log(self, tribal_home):
        from tribal_cli.debug import collect_debug_report
...
class TestRunDebugShare:
    ...
    def test_share_sweeps_expired_pastes(self, tribal_home, capsys):
        ...
        with patch("tribal_cli.dump.run_dump"), \
             patch("tribal_cli.debug._sweep_expired_pastes", return_value=(0, 0)) as mock_sweep, \
             patch("tribal_cli.debug.upload_to_pastebin",
    ...
    def test_share_survives_sweep_failure(self, tribal_home, capsys):
        ...
        with patch("tribal_cli.dump.run_dump"), \
             patch(
                 "tribal_cli.debug._sweep_expired_pastes",
    ...
    def test_local_flag_prints_full_logs(self, tribal_home, capsys):
    ...
    def test_share_uploads_three_pastes(self, tribal_home, capsys):
        ...
        with patch("tribal_cli.dump.run_dump") as mock_dump, \
            ...
            mock_dump.side_effect = lambda a: print("--- tribal dump ---\nversion: test\n--- end dump ---")
            run_debug_share(args)
        ...
        assert "--- tribal dump ---" in agent_paste
        assert "--- full agent.log ---" in agent_paste
        gateway_paste = uploaded_content[2]
        assert "--- tribal dump ---" in gateway_paste
        assert "--- full gateway.log ---" in gateway_paste

    def test_share_keeps_report_and_full_log_on_same_snapshot(self, tribal_home, capsys):
        """A mid-run rotation must not make full agent.log older than the report."""
        from tribal_cli.debug import run_debug_share, collect_debug_report as real_collect_debug_report
        ...
        with patch("tribal_cli.dump.run_dump"), \
             patch("tribal_cli.debug.collect_debug_report", side_effect=_wrapped_collect_debug_report), \
             patch("tribal_cli.debug.upload_to_pastebin", side_effect=_mock_upload):
            run_debug_share(args)
    ...
    def test_share_continues_on_log_upload_failure(self, tribal_home, capsys):
    ...
    def test_share_exits_on_report_upload_failure(self, tribal_home, capsys):
...
class TestRunDebugShareRedaction:
    ...
    def test_default_share_redacts_uploaded_content(
        self, tribal_home_with_secret, capsys
    ):
        ...
        with patch("tribal_cli.dump.run_dump"), \
             patch("tribal_cli.debug._sweep_expired_pastes", return_value=(0, 0)), \
             patch("tribal_cli.debug.upload_to_pastebin", side_effect=fake_upload):
            run_debug_share(args)
...
class TestRunDebug:
    def test_no_subcommand_shows_usage(self, capsys):
        ...
        out = capsys.readouterr().out
        assert "tribal debug" in out
        assert "share" in out
        assert "delete" in out

    def test_share_subcommand_routes(self, tribal_home):
        from tribal_cli.debug import run_debug
...
class TestExtractPasteId:
    def test_paste_rs_url(self):
        from tribal_cli.debug import _extract_paste_id
...
class TestDeletePaste:
    def test_delete_sends_delete_request(self):
        from tribal_cli.debug import delete_paste
        ...
        with patch("tribal_cli.debug.urllib.request.urlopen",
...
class TestScheduleAutoDelete:
    ...
    ``~/.tribal/pastes/pending.json`` and lets ``_sweep_expired_pastes``
    handle the DELETE requests synchronously on the next ``tribal debug``
    invocation.
    """

    def test_does_not_spawn_subprocess(self, tribal_home):
        ...
        import ast
        import inspect
        from tribal_cli.debug import _schedule_auto_delete
    ...
    def test_records_pending_to_json(self, tribal_home):
        """Scheduled URLs are persisted to pending.json with expiration."""
        from tribal_cli.debug import _schedule_auto_delete, _pending_file
        import json
    ...
    def test_skips_non_paste_rs_urls(self, tribal_home):
    ...
    def test_merges_with_existing_pending(self, tribal_home):
        """Subsequent calls merge into existing pending.json."""
        from tribal_cli.debug import _schedule_auto_delete, _load_pending
    ...
    def test_dedupes_same_url(self, tribal_home):
...
class TestSweepExpiredPastes:
    ...
    def test_sweep_empty_is_noop(self, tribal_home):
    ...
    def test_sweep_deletes_expired_entries(self, tribal_home):
        ...
        with patch("tribal_cli.debug.delete_paste", side_effect=fake_delete):
            deleted, remaining = _sweep_expired_pastes()
    ...
    def test_sweep_leaves_future_entries_alone(self, tribal_home):
        from tribal_cli.debug import _sweep_expired_pastes, _save_pending
        import time
        ...
        with patch("tribal_cli.debug.delete_paste") as mock_delete:
            deleted, remaining = _sweep_expired_pastes()
    ...
    def test_sweep_survives_network_failure(self, tribal_home):
    ...
    def test_sweep_drops_entries_past_grace_window(self, tribal_home):
...
class TestRunDebugSweepsOnInvocation:
    ...
    def test_run_debug_calls_sweep(self, tribal_home):
        from tribal_cli.debug import run_debug

        args = MagicMock()
        args.debug_command = None  # default → prints help

        with patch("tribal_cli.debug._sweep_expired_pastes") as mock_sweep:
            run_debug(args)

        mock_sweep.assert_called_once()

    def test_run_debug_survives_sweep_failure(self, tribal_home, capsys):
        ...
        out = capsys.readouterr().out
        assert "Usage: tribal debug" in out


class TestRunDebugDelete:
    def test_deletes_valid_url(self, capsys):
        from tribal_cli.debug import run_debug_delete

        args = MagicMock()
        args.urls = ["https://paste.rs/abc"]

        with patch("tribal_cli.debug.delete_paste", return_value=True):
    ...
    def test_handles_delete_failure(self, capsys):
        ...
        with patch("tribal_cli.debug.delete_paste",
...
class TestShareIncludesAutoDelete:
    ...
    def test_share_schedules_auto_delete(self, tribal_home, capsys):
        from tribal_cli.debug import run_debug_share
        ...
        with patch("tribal_cli.dump.run_dump"), \
             ...
             patch("tribal_cli.debug._schedule_auto_delete") as mock_sched:
            run_debug_share(args)
    ...
    def test_share_shows_privacy_notice(self, tribal_home, capsys):
        from tribal_cli.debug import run_debug_share
    ...
    def test_local_no_privacy_notice(self, tribal_home, capsys):
        from tribal_cli.debug import run_debug_share
