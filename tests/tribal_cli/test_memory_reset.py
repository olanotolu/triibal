"""Tests for the `tribal memory reset` CLI command.
...
- Profile-scoped reset (uses TRIBAL_HOME)
"""
...
def memory_env(tmp_path, monkeypatch):
    """Set up a fake TRIBAL_HOME with memory files."""
    tribal_home = tmp_path / ".tribal"
    memories = tribal_home / "memories"
    memories.mkdir(parents=True)
    monkeypatch.setenv("TRIBAL_HOME", str(tribal_home))

    # Create sample memory files
    (memories / "MEMORY.md").write_text(
        "§\nTribal repo is at ~/.tribal/tribal-agent\n§\nUser prefers dark themes",
        encoding="utf-8",
    )
...
def _run_memory_reset(target="all", yes=False, monkeypatch=None, confirm_input="no"):
    ...
    Simulates what happens when `tribal memory reset` is run.
    """
...
class TestMemoryReset:
    ...
    def test_reset_no_files_exist(self, tmp_path, monkeypatch):
        ...
        tribal_home = tmp_path / ".tribal"
        (tribal_home / "memories").mkdir(parents=True)
        monkeypatch.setenv("TRIBAL_HOME", str(tribal_home))
    ...
    def test_reset_profile_scoped(self, tmp_path, monkeypatch):
        """Reset should work on the active profile's TRIBAL_HOME."""
    ...
    def test_reset_empty_memories_dir(self, tmp_path, monkeypatch):
        ...
        # The memories dir won't exist; get_tribal_home() / "memories" won't have files
        result = _run_memory_reset(target="all", yes=True)
        assert result == "nothing"
