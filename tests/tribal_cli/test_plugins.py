"""Tests for the Tribal plugin system (tribal_cli.plugins)."""
...
def _make_plugin_dir(base: Path, name: str, *, register_body: str = "pass",
    ...
    ``<tribal_home>/config.yaml`` under ``plugins.enabled``. Plugins are
    ...
    *base* is expected to be ``<tribal_home>/plugins/``; we derive
    ``<tribal_home>`` from it by walking one level up.
    """
    ...
    if auto_enable:
        # Write/merge plugins.enabled in <TRIBAL_HOME>/config.yaml.
        # Config is always read from TRIBAL_HOME (not from the project
        # dir for project plugins), so that's where we opt in.
        import os
        tribal_home_str = os.environ.get("TRIBAL_HOME")
        if tribal_home_str:
            tribal_home = Path(tribal_home_str)
        else:
            tribal_home = base.parent
...
class TestPluginDiscovery:
    ...
    def test_discover_user_plugins(self, tmp_path, monkeypatch):
        """Plugins in ~/.tribal/plugins/ are discovered."""
        plugins_dir = tmp_path / "tribal_test" / "plugins"
        _make_plugin_dir(plugins_dir, "hello_plugin")
        monkeypatch.setenv("TRIBAL_HOME", str(tmp_path / "tribal_test"))
    ...
    def test_discover_project_plugins(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setenv("TRIBAL_ENABLE_PROJECT_PLUGINS", "true")
        plugins_dir = project_dir / ".tribal" / "plugins"
...
class TestPluginLoading:
    ...
    def test_load_registers_namespace_module(self, tmp_path, monkeypatch):
        """Directory plugins are importable under tribal_plugins.<name>."""
        plugins_dir = tmp_path / "tribal_test" / "plugins"
        _make_plugin_dir(plugins_dir, "ns_plugin")
        monkeypatch.setenv("TRIBAL_HOME", str(tmp_path / "tribal_test"))

        # Clean up any prior namespace module
        sys.modules.pop("tribal_plugins.ns_plugin", None)

        mgr = PluginManager()
        mgr.discover_and_load()

        assert "tribal_plugins.ns_plugin" in sys.modules
...
class TestPluginHooks:
    ...
    def test_invalid_hook_name_warns(self, tmp_path, monkeypatch, caplog):
        ...
        with caplog.at_level(logging.WARNING, logger="tribal_cli.plugins"):
            mgr = PluginManager()
            mgr.discover_and_load()
...
class TestPreToolCallBlocking:
    ...
    def test_block_message_returned_for_valid_directive(self, monkeypatch):
        monkeypatch.setattr(
            "tribal_cli.plugins.invoke_hook",
...
class TestPluginCommands:
    ...
    def test_get_plugin_command_handler_found(self):
        ...
        with patch("tribal_cli.plugins._plugin_manager", mgr):
    ...
    def test_commands_in_list_plugins_output(self, tmp_path, monkeypatch):
        ...
        plugins_dir = tmp_path / "tribal_test" / "plugins"
        # Set TRIBAL_HOME BEFORE _make_plugin_dir so auto-enable targets
...
class TestPluginCommandResultResolution:
    ...
    def test_awaits_async_result_with_running_loop(self, monkeypatch):
        ...
        monkeypatch.setattr("tribal_cli.plugins.asyncio.get_running_loop", lambda: _Loop())
    ...
    def test_running_loop_timeout_does_not_hang_forever(self, monkeypatch):
        ...
        monkeypatch.setattr("tribal_cli.plugins.asyncio.get_running_loop", lambda: _Loop())
        monkeypatch.setattr("tribal_cli.plugins._PLUGIN_COMMAND_AWAIT_TIMEOUT_SECS", 0.1)
...
class TestPluginDispatchTool:
    ...
    def test_dispatch_tool_calls_registry(self):
        ...
        with patch("tribal_cli.plugins.PluginContext.dispatch_tool.__module__", "tribal_cli.plugins"):
...
class TestPluginDebugLogging:
    """TRIBAL_PLUGINS_DEBUG opt-in stderr handler for plugin developers."""
    ...
    def test_debug_handler_not_installed_when_env_var_absent(self, monkeypatch):
        ...
        monkeypatch.delenv("TRIBAL_PLUGINS_DEBUG", raising=False)
    ...
    def test_debug_handler_installed_when_env_var_set(self, monkeypatch):
        """With TRIBAL_PLUGINS_DEBUG=1, a DEBUG-level stderr handler is attached."""
        monkeypatch.setenv("TRIBAL_PLUGINS_DEBUG", "1")
        from tribal_cli import plugins as plugins_mod
