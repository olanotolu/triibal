...
``tribal_cli.main`` skips eager plugin discovery at argparse-setup time
when the invocation is clearly targeting a known built-in subcommand.
This saves 500-650ms on ``tribal --help``, ``tribal version``,
``tribal logs``, etc., by not importing ``google.cloud.pubsub_v1``,
...
def _live_subcommand_names() -> set[str]:
    """Run ``tribal --help`` in-process and parse the subcommand block.
    ...
    built-in-only set.
    """
    from tribal_cli import main as _main

    argv_backup = sys.argv[:]
    sys.argv = ["tribal", "--help"]
    buf = io.StringIO()
    try:
...
@pytest.mark.parametrize(
    ...
    [
        ...
        (["tribal", "-w", "--tui"], None),
        (["tribal", "version"], "version"),
        (["tribal", "--tui", "chat"], "chat"),
        (["tribal", "-w", "logs"], "logs"),
        (["tribal", "chat", "hello world"], "chat"),
        (["tribal", "gateway", "run"], "gateway"),
        # Top-level value-taking flags: the value should be skipped.
        (["tribal", "-m", "gpt5", "chat"], "chat"),
        (["tribal", "--model", "gpt5", "chat", "hi"], "chat"),
        (["tribal", "-m", "gpt5", "--provider", "openai", "chat"], "chat"),
        (["tribal", "-z", "hello world"], None),
        (["tribal", "-z", "hello", "chat"], "chat"),
        (["tribal", "--model=gpt5", "chat"], "chat"),     # inline form
        (["tribal", "--", "chat"], "chat"),               # -- terminator
        (["tribal", "-w", "--"], None),
        # Unknown positional after skipped flags → plugin-cmd candidate.
        (["tribal", "some-plugin-cmd"], "some-plugin-cmd"),
        (["tribal", "-m", "gpt5", "some-plugin-cmd"], "some-plugin-cmd"),
    ],
)
...
@pytest.mark.parametrize(
    ...
    [
        ["tribal"],                          # bare → chat
        ["tribal", "--help"],                # top-level help
        ["tribal", "-h"],
        ["tribal", "version"],               # known built-in
        ["tribal", "logs"],
        ["tribal", "gateway", "run"],
        ["tribal", "--tui"],
        ["tribal", "-w", "--tui"],
        ["tribal", "chat", "hi"],
        ["tribal", "help"],                  # accepted built-in-ish
        ["tribal", "-m", "gpt5", "chat"],    # flag-value-skipping
    ],
)
...
@pytest.mark.parametrize(
    "argv",
    [
        ["tribal", "meet", "join"],          # potential google_meet plugin
        ["tribal", "honcho", "status"],      # potential memory plugin
        ["tribal", "unknown-subcmd"],
    ],
)
...
def test_builtin_set_covers_every_registered_subcommand():
    ...
    # in the set so ``tribal help <cmd>`` skips discovery; it won't show
    ...
    assert not missing_from_declaration, (
        ...
        f"tribal_cli/main.py::_BUILTIN_SUBCOMMANDS so plugin discovery "
