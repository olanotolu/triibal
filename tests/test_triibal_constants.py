"""Tests for triibal_constants module."""
...
class TestGetDefaultTriibalRoot:
    """Tests for get_default_triibal_root() — Docker/custom deployment awareness."""

    def test_no_triibal_home_returns_native(self, tmp_path, monkeypatch):
        """When TRIIBAL_HOME is not set, returns ~/.triibal."""
        monkeypatch.delenv("TRIIBAL_HOME", raising=False)
        monkeypatch.setattr(Path, "home", lambda: tmp_path)

        assert get_default_triibal_root() == tmp_path / ".triibal"

    def test_triibal_home_is_native(self, tmp_path, monkeypatch):
        """When TRIIBAL_HOME = ~/.triibal, returns ~/.triibal."""
        native = tmp_path / ".triibal"
        native.mkdir()
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
        monkeypatch.setenv("TRIIBAL_HOME", str(native))
        assert get_default_triibal_root() == native

    def test_triibal_home_is_profile(self, tmp_path, monkeypatch):
        """When TRIIBAL_HOME is a profile under ~/.triibal, returns ~/.triibal."""
        native = tmp_path / ".triibal"
    ...
    def test_triibal_home_is_docker(self, tmp_path, monkeypatch):
        """When TRIIBAL_HOME points outside ~/.triibal (Docker), returns TRIIBAL_HOME."""
        docker_home = tmp_path / "opt" / "data"
        docker_home.mkdir(parents=True)
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
        monkeypatch.setenv("TRIIBAL_HOME", str(docker_home))
        assert get_default_triibal_root() == docker_home

    def test_triibal_home_is_custom_path(self, tmp_path, monkeypatch):
        """Any TRIIBAL_HOME outside ~/.triibal is treated as the root."""
        custom = tmp_path / "my-triibal-data"
        custom.mkdir()
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
        monkeypatch.setenv("TRIIBAL_HOME", str(custom))
        assert get_default_triibal_root() == custom

    def test_docker_profile_active(self, tmp_path, monkeypatch):
        """When a Docker profile is active (TRIIBAL_HOME=<root>/profiles/<name>),
        ...
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
        monkeypatch.setenv("TRIIBAL_HOME", str(profile))
        assert get_default_triibal_root() == docker_root
...
class TestIsContainer:
    ...
    def _reset_cache(self, monkeypatch):
        ...
        monkeypatch.setattr(triibal_constants, "_container_detected", None)
    ...
    def test_caches_result(self, monkeypatch):
        ...
        monkeypatch.setattr(triibal_constants, "_container_detected", True)
        assert is_container() is True
...
class TestSecureParentDir:
    ...
    def test_safe_path_calls_chmod(self, tmp_path, monkeypatch):
        ...
        safe_dir = tmp_path / "home" / "user" / ".triibal"
