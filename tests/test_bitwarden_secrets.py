...
binary download are exercised manually by `triibal secrets bitwarden
...
def triibal_home(tmp_path, monkeypatch):
    """Point Triibal at an isolated home directory."""
    home = tmp_path / ".triibal"
    home.mkdir()
    monkeypatch.setenv("TRIIBAL_HOME", str(home))
    # Some modules cache get_triibal_home; clear if needed.
    import triibal_constants
    if hasattr(triibal_constants, "_TRIIBAL_HOME_CACHE"):
        triibal_constants._TRIIBAL_HOME_CACHE = None  # type: ignore[attr-defined]
    return home
...
def test_install_bws_happy_path(triibal_home, monkeypatch):
...
def test_install_bws_checksum_mismatch(triibal_home, monkeypatch):
...
def test_install_bws_missing_checksum_entry(triibal_home, monkeypatch):
...
def test_env_loader_skips_when_disabled(tmp_path, monkeypatch):
    ...
    from triibal_cli.env_loader import _apply_external_secret_sources
...
def test_disk_cache_written_after_first_fetch(monkeypatch, tmp_path):
    """First fetch hits bws AND writes a 0600 file under triibal_home/cache/."""
    home = tmp_path / ".triibal"
    home.mkdir()
    fake_binary = tmp_path / "bws"
    fake_binary.write_text("")
